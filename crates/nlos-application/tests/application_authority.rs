...
/// Mock probe for activity-gate tests: returns a fixed count for every package.
...
/// 运行中 Task 门：fresh uninstall 在 probe 报告 active tasks > 0 时
/// typed 拒绝（ApplicationActiveTasksRunning）且零 durable 副作用；ungated
/// API 不受影响。
#[test]
...
/// 运行中 Task 门：durable uninstall replay 不受 active-task 门影响。
#[test]
...
/// 运行中 Task 门：fresh rollback 在 probe 报告 active tasks > 0 时
/// typed 拒绝且零 durable 副作用。
#[test]
...
fn background_task_registration_replays_idempotently() {
    ...
    assert_eq!(receipt.task_id, task_id(0xA1));

    let listed = authority
        .inspect_background_tasks(verified.package_id)
        .expect("inspect background tasks");
...
fn background_task_registration_refusals_are_typed_and_leave_zero_state() {
    ...
    let authority = open_authority(stack.root.root());
    uninstalled(&authority, verified.package_id, 0x0C, 5_000);
    let uninstalled_err = authority
        .register_background_task(RegisterBackgroundTaskRequest {
            registered_at_ms: 6_000,
            idempotency_key: key(0x0D),
            ..request
        })
        .expect_err("uninstalled must refuse");
    assert!(matches!(
        uninstalled_err,
...
fn background_task_registration_duplicate_and_conflict_refusals() {
    ...
    let duplicate = authority
        ...
        .expect_err("duplicate task at same generation");
    assert!(matches!(
        duplicate,
        ApplicationAuthorityError::BackgroundTaskAlreadyRegistered { .. }
    ));

    let too_early = authority
        .register_background_task(RegisterBackgroundTaskRequest {
            package_id: verified.package_id,
        ...
        .expect_err("registration must not precede install");
    assert!(matches!(
        too_early,
