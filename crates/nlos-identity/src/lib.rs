...
impl IdentityAuthority {
    ...
    /// Returns the count of durable trusted-local session ingress receipts.
    ///
    /// This is a read-only aggregate over immutable session rows; idempotent
    /// replays do not increase the count.
    ///
    /// # Errors
    ///
