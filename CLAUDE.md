...
- **最大宽度并行(2026-09-13 维护者偏好)**:在合理编排前提下,并行度取当前屏障约束下的最大值——写集不相交的任务必须同时派发,不得无故串行;只有真实数据依赖、共享文件或集成屏障才允许排队。每个并行车道用独立 `CARGO_TARGET_DIR`/隔离 worktree 避免锁竞争,失败只阻塞其依赖者
...
- **波次进行中状态跃迁必附 DAG**:任何任务 dispatched/过审/修复环/完成时,回复末尾自动附 `python3 plugins/dash/scripts/dash render panel` 的终端车道图(维护者 2026-09-13 要求,目的:大型 AI 开发中维护者能持续把握当前状态);无波次工作区时跳过,不刷屏
