...
### 6.16 Wake latency/fairness 确定性测试（2026-09-20 追加，W27-F / B-RUNTIME 开放项）
...
- **勾销**：§3 `B-RUNTIME` 行「wake latency/fairness」的**功能级确定性测试**（scheduler-step 级 latency bound + burst fairness 无饿死 + 交错 cancel 不丢 wake）→ 本 §6.16。
- **如实保留（不外推）**：
