...
## 6. 桥接 kill-window 故障注入矩阵（2026-08-24 增量）

- 日期：2026-08-24（Attempt `TASK-RESOURCE-BRIDGE-FAULT-01`）；基线 HEAD `52a7d32`（= origin/main，工作区 clean，已验证无漂移）。候选为工作区未提交变更（仅新增 `crates/nlos-task/tests/resource_bridge_fault_injection.rs` + 本 Evidence 增量）；本 attempt 不提交、不 push、不更新 `stage-b-progress.md`。
- 性质：**验证矩阵而非功能**——先对 HEAD 行为写矩阵测试，全绿即证明桥接在 HEAD 已正确。RED 仅在暴露真实缺陷时适用：本增量 **未发现缺陷，`crates/nlos-task/src/**` 零改动**。
- 遵循的先例：Task 侧故障 VFS 注入/kill-9 子进程（`READY` 管道同步，无 sleep）/`FAULT_LOCK` 进程序列化/WAL 尾部截断/错误链断言/逐场景 `integrity_check` → `tests/fault_injection.rs`（B-TASK-001）与 `tests/reconcile_fault_injection.rs`（B-TASK-003）；F1–F7 矩阵行式样与 Phase A/B 断电结构 → `nlos-resource/tests/finalize_fault_injection.rs`；桥接 fixture（owner/seal/permit/plan-READY）→ `tests/resource_commit.rs` 与 `tests/mixed_semantic_resource_commit.rs`。
- 故障目标与边界：Resource/Semantic owner 在桥接时已 FINALIZED/READY 且不可变，故全部注入均指向 **Task authority 连接**（`SqliteTaskAuthority::open_with_vfs`，VFS `nlos-task-resource-bridge-fault`）；owner 连接走普通 VFS 不受影响。矩阵证明的是 **verify-then-commit + 幂等收敛**，不是跨 authority 原子性（措辞固定，不做原子性声明）。
- fixture/故障原语清单：`nlos-store-fault` `FaultMode::FailWritesAfter{0, IoErr}` / `FailWritesAfter{0, Full}` / `PowerLossAfter{0}`（进程级 xWrite 门）；kill-9 子进程（`Child::kill` 强杀，模拟进程崩溃；页面缓存仍在，非真实断电——矩阵头部 disclaimer 沿先例逐字保留）；文件级 WAL 尾部截断（最后一个 commit 帧半帧处截断 + 删 `-shm`）；`PRAGMA integrity_check` 逐场景复核。fixture：双 Reservation（R1 upper 100，consume 30→37，finalize 37/refund 63；R2 upper 25，consume 10，finalize 10/refund 15）或单 Reservation（upper 60，consume 12，refund 48）+ 1 条 semantic append；plain 与 authority-lease 两条 permit 路径均已覆盖（lease 变体承载两个重放风暴行）。
- 矩阵表（场景 × rung；命令均为 `cargo test -p nlos-task --test resource_bridge_fault_injection`，11/11 全绿）：

  | 窗口 | resource-only rung 测试 | mixed rung 测试 | 预期 | 实际 |
  |---|---|---|---|---|
  | W1 pre-commit IOERR | `bridge_fault_resource_rung_ioerr_precommit_fails_typed_and_converges` | `bridge_fault_mixed_rung_ioerr_precommit_fails_typed_and_converges` | `Sqlite` typed 错误（链含 i/o）；permit Issued、head 0、四终结表 0 行（mixed 另证 plan 保持 READY、READY 前缀 publications 1 行保留）；disarm 后同请求重试 `Committed` 且嵌套集合与 owner 聚合逐字段相等 | 一致（PASS） |
  | W2 pre-commit ENOSPC | `bridge_fault_resource_rung_enospc_precommit_fails_typed_and_converges` | `bridge_fault_mixed_rung_enospc_precommit_fails_typed_and_converges` | 同 W1，链含 full | 一致（PASS） |
  | W3 PowerLossAfter commit-point | `bridge_fault_resource_rung_power_loss_commit_point_converges_both_ways` | `bridge_fault_mixed_rung_power_loss_commit_point_converges_both_ways` | Phase A（`PowerLossAfter{0}`）：finalize "报告成功"但重开后**完全不可见**（permit Issued、head 0、四表 0 行；mixed plan 保持 READY）；同请求重做 → `Committed` 与幻影 receipt 逐字节相等（确定性 `derive_commit_receipt_id`）。Phase B（kill-9 after commit）：**完全可见**（receipt + 全部嵌套行 + permit Closed + head 1；mixed plan FINALIZED）；同请求重放 → `Replayed` 逐字节相等、无重复行。两方向都绝无部分可见 | 一致（PASS） |
  | W4 torn WAL tail | `bridge_fault_resource_rung_torn_wal_tail_discards_and_converges` | `bridge_fault_mixed_rung_torn_wal_tail_discards_both_sides_together` | 子进程提交完整桥接后被杀，父进程截断最后 commit 帧一半；重开桥接事务整体隐藏（已提交前缀 task/attempt/write-set/permit 保留），同请求重做 `Committed`、聚合完整；重开后重放逐字节相等、行数恰一套 | 一致（PASS） |
  | W5 kill-window 重放幂等（lease 变体） | `bridge_fault_resource_rung_replay_storm_after_commit_is_idempotent` | `bridge_fault_mixed_rung_replay_storm_keeps_both_sets_single_and_byte_equal` | commit 后同请求连放 3 次 + 重开（对空 owner）再放 1 次 → 全部 `Replayed` 逐字节相等；receipt/嵌套父行/子行（mixed 另含 publications）各恰 1 套，风暴无重复 | 一致（PASS） |
  | W6 mixed 双侧同生同灭 | （resource-only 无双侧语义，由 W3/W4 单侧覆盖） | `bridge_fault_mixed_rung_power_loss_commit_point_converges_both_ways` Phase A/B + `bridge_fault_mixed_rung_torn_wal_tail_discards_both_sides_together` | 不可见方向：receipt、resource 嵌套行、plan FINALIZED 翻转**一起消失**（publications 行属 READY 前缀、合法保留）；可见方向：三者**一起出现**。"只有一侧"的中间态被双向证明不可能（同一 `BEGIN IMMEDIATE` 事务：`insert_receipt` → nested 行 → permit close → head advance → `finalize_plan` CAS） | 一致（PASS） |

- 精确计数（均 fmt 后运行）：`cargo test -p nlos-task --test resource_bridge_fault_injection` → **11 通过 / 0 失败 / 0 忽略**（10 场景 + `crash_child_helper` 子进程入口）；`cargo test -p nlos-task --quiet` → **204 通过 / 0 失败 / 0 忽略**（= 193 基线 + 11 新增，24 个 test 二进制）；`cargo check -p nlos-task` 通过；`cargo clippy -p nlos-task --all-targets --all-features -- -D warnings` 通过（0 warning）；`cargo fmt --all -- --check` 通过；`git diff --check` 通过。
- LSP 诊断：daemon 请求超时（与前两增量同状）；以 `cargo check` + `clippy -D warnings`（更强的编译器诊断门）替代，二者均通过。新增测试代码 0 `unsafe`、0 生产 `unwrap/expect`（仅测试内 `expect`，沿先例）、确定性 fixture（固定字节种子、无 wall-clock、无 sleep、kill-9 以管道 `READY` 同步）、每场景独立临时目录 RAII 清理。
- 测试文件 2039 纯 LOC，超出 250 行天花板（`allow: SIZE_OK` 于文件头声明）：单一故障矩阵单二进制是本仓库既有形态（此前七个 `*_fault_injection.rs` 均为 950–1846 行单体），fixture 按仓库惯例随矩阵文件复制。
- 发现并修复的缺陷：**无**（矩阵全绿；`crates/nlos-task/src/**` 本增量零改动）。
- 明确缺口（本增量不解决，PARTIAL_PASS 维持）：① 仍无真实断电硬件证据——kill-9 是进程崩溃（页面缓存存活），`PowerLossAfter`/WAL 截断是模型化丢写，均非 fsync 级断电复现；② 仍无跨 authority 原子性——verify-then-commit 边界未变，owner 读仍在 Task 事务外，矩阵证明的是收敛性而非原子性；③ 无 CI 结果（本地 macOS/arm64）；④ 不据此宣称 DONE 或 H4+；`stage-b-progress.md` 未由本 attempt 更新。
