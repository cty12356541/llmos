...
- `cargo test -p nlos-task --test authority_lease -- --nocapture`：4 项通过；新增覆盖新 term live lease 的 takeover-fence CAS、control epoch 单调推进、旧 lease 拒绝、冻结后新 permit 拒绝、receipt immutable trigger 与 restart/replay。
