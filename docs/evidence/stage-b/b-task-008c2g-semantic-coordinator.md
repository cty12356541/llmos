...
### 5.4 Mixed Effect + Semantic fault-prefix/restart matrix（B-TASK-008C2G-COORD-SEM-MIXED-FAULT）

- 新增 `crates/nlos-commit-coordinator/tests/semantic_mixed_convergence_fault_injection.rs`，在同一 mixed Effect + Semantic sealed write set 上覆盖三个跨 authority durable boundary：Semantic owner publication 写失败后重开、owner publication receipt 已提交但 Task consumer 写失败后重开、READY 后 mixed terminal finalize 写失败后重开。
- 每个场景都关闭 optional Effect slot，再丢弃/重开对应 authority；恢复只调用公开 `SemanticCommitCoordinator::converge_pending(1, now_ms)`，断言单条 Semantic publication、单条 TaskCommitReceipt、`FINALIZED`、Task head=1 与空 incomplete scan。测试二进制共 7 项（4 项既有 baseline + 3 项新增）通过。
- 本地复验：`cargo test -p nlos-commit-coordinator --test semantic_mixed_convergence_fault_injection -- --nocapture`（7 项）、`cargo test -p nlos-commit-coordinator --test semantic_convergence_fault_injection --quiet`（9 项）、`cargo test -p nlos-commit-coordinator --test semantic_pending_mixed_restart_scan --quiet`（5 项）、targeted Clippy、fmt/diff 与 workspace 均通过。Rust cross-platform/MSRV CI [32627556597](https://github.com/cty12356541/llmos/actions/runs/32627556597) 与 Pages [32627556621](https://github.com/cty12356541/llmos/actions/runs/32627556621) 均成功。
- 故障模型限定为测试专用 SQLite/VFS 的 `IOERR`/`FULL` 写失败与 durable-prefix restart；不外推真实硬件掉电、跨机器原子提交、外部 provider proof/attestation、Trust View/vector checkpoint 或完整 TaskWriteSet。
