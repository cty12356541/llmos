...
## 9. IPC 面接入 ADR-0011 认证（2026-08-30 增量，无 commit——本 Attempt 不做 git 操作）

- `nlos-wait-control` 新增 opt-in 认证服务变体（feature `authenticated-server`，`#[cfg(all(unix, feature = "authenticated-server"))]` 模块 `src/authenticated.rs`）：`AuthenticatedWaitControlServer` 绑定 `(WaitAuthority, WaitControlAuthorizer, IdentityAuthority, AuthorityClock, ServerHandshakeContext)`，`serve_one` 按 W2-A 设施的 fail-closed 顺序组装（OS pre-gate → challenge → attestation → `serve_one` 既有语义），verified_at_ms 取 `AuthorityClock::inspect_wall()`（无副作用 wall 验证读；host 若要实时过期检查应在自身 bootstrap 时 `wall_now` 推进 wall 域）。
- **principal 绑定幂等键**（ADR-0011「跨 Principal 等待登记」解锁点）：共享 WaitAuthority 的 durable replay 身份是原始 key 字节，跨 principal 可碰撞/重放；认证路径下五个方法中三个 mutation（register/notify/cancel）的幂等键在进入 plain service 前改写为 `SHA-256("llmos/wait-control/auth-idem/v1" ‖ principal_id(16B) ‖ original_key(16B))[..16]`（domain 分离 + 确定性 ⇒ 同 principal 同 key 重放同一 durable 回执、跨 principal 派生不同 key）；query 与畸形请求原样透传，失败语义与本地信任域路径逐字相同（无第二套拒绝词汇）。notify/cancel 回执引用绑定后 key 字节；register 回执仍为 WaitId。
- **零改动面**：`WaitControlService`/`handle`/`handle_for_ipc`/`WaitControlAuthorizer` 本地信任域路径与 `src/bin/wait-control-conformance.rs`（CI 的 TS/Python 客户端依赖）字节不动；additive 仅有 lib.rs 的模块声明、crate 文档段、`&T` blanket impl（供服务器按借用注入 policy）。
- 验证（feature-on 面）：新增 5 项集成测试（`tests/authenticated_wait_control.rs`，真 Unix socket）：认证 roundtrip（register 走认证路径 + durable 行携带 principal 绑定 key ≠ 原始 key + 二连接同请求 replay 同一行）、过期 key 对 clock wall fail-closed（KeyExpired，证明 verified_at 来自 clock 权威）、坏签名（SignatureInvalid + registry 零触碰）、attestation 重放（第二连接 NonceRejected）、未知 principal（PrincipalUnknown）；既有 13 项零回归。
- 验证门：`cargo test -p nlos-wait-control` → 13 passed / 0 failed（gate 命令下认证模块按 cfg 编译为空，新测试不运行）；`cargo test -p nlos-wait-control --features authenticated-server` → 5 + 13 全部 passed / 0 failed；`cargo clippy -p nlos-wait-control --all-targets -D warnings`（stable + nightly-2026-08-01 × default/authenticated-server 两特征面）→ 全部 0 warning；`cargo fmt` 双工具链 `--check` → 干净；`--features conformance-server --no-run` → bin 照常编译（bin 零改动核实）。
- 已知限制与偏差：**写集偏差申报**——`crates/nlos-wait-control/Cargo.toml`（任务写集未列）必须 additive 修改：新增 `authenticated-server` feature（optional nlos-clock/nlos-identity/nlos-ipc/sha2/tokio）与 dev-deps（ed25519-dalek/nlos-clock/nlos-identity，测试需真 Ed25519 签名与双权威），否则任务要求的认证 roundtrip 测试无法编译；Cargo.lock 随之机械更新（与并行 system/takeover 车道同文件加条目，无冲突）；该 crate manifest 无其他车道触碰。Windows 交叉 clippy 未跑通：`libsqlite3-sys` cc 交叉编译在 macOS host 失败，与 b-task-006l §「未运行」先例同因（环境限制，非本车道 cfg 缺陷；本模块 cfg(unix) 本就不在 Windows 面内），按先例由三平台 CI 背书。fresh clock 库 `inspect_wall()` 返回 0（迁移种子水位）——认证在 key 窗口含 0 时仍可用，实时性由 host bootstrap 推进 wall 域保证。
