...
| `B-OP-FENCE` | Operation 状态机、callback identity、cancel/generation fence | `PARTIAL_PASS` | [PoC-0002](../evidence/stage-b/poc-0002-operation-callback-fence.md)；[B-OP-FENCE-002](../evidence/stage-b/b-op-fence-002-operation-endpoint-proof.md)：owner-derived Operation endpoint proof/readback、generation fence 与重启回读 | Driver authentication、EffectPermit、progress/stream callback、TaskWriteSet/participant registry 接线；Tokio wake 集成已随 `B-OUTBOX`（PoC-0004）补齐 |
...
| `B-OP-FENCE-002` | Operation owner endpoint proof/readback | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-op-fence-002-operation-endpoint-proof.md) | Operation prepare→activate、跨进程签名/租约/attestation、Task participant registry 接线与 per-effect TaskWriteSet linkage 仍未实现 |
...
- 仍为 `PARTIAL_PASS`：Operation prepare→activate、跨进程签名/租约/attestation、Task participant registry 接线、per-effect TaskWriteSet linkage 与 Channel endpoint 仍未完成。
