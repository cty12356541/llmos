...
- 本切片当时尚无 durable dedup/result；该缺口的三平台 authority 已由 4.18/B-SCHEMA-010 补齐，真实 IPC 三平台接线由 4.19/B-SCHEMA-011 补齐。server restart 组合已在本地通过但远程回执待补；deadline/cancel propagation、Receipt authority、Capability authorization 和完整 server fault matrix 仍未完成，因此仍是 `SDK-2 CANDIDATE / PARTIAL`。
...
- 真实 SABI server 接线已由 4.19/B-SCHEMA-011 补齐并通过三平台；server restart 组合本地通过、远程三平台待验证。deadline/cancel/uncertain 完整 fault matrix、Receipt body 和 retention/GC 仍未完成，因此不升级 SDK 等级。
...
- 接线明确了 durable `result_wire` 与每次重建的 transport envelope 必须分层；同进程重连已由本地与[三平台 run 30740180511](https://github.com/cty12356541/llmos/actions/runs/30740180511) 验证。新增组合把 commit/recovery 拆为两个独立 Rust 进程，恢复进程重绑目录和业务 endpoint、重开同一 SQLite authority，TS/Python 重新协商后回放原结果且恢复进程零次新 dispatch；本地通过，远程三平台待验证。详见 [B-SCHEMA-011](../evidence/stage-b/b-schema-011-durable-idempotency-ipc.md)。
...
Protobuf envelope + Rust generation + registry + first golden       DONE
  ...
  → deadline/cancel/uncertain state machine                        NEXT
