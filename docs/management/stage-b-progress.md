...
> 本轮（2026-08-23 第十九增量）推进 `B-TASK-008C2G-IPC-EXTERNAL-TIMING-KILL`：新增不依赖 server crash hook 的 TypeScript/Python 外部 supervisor 入口；客户端用 OS CSPRNG 采样 0–64ms 延迟，在请求发出后直接以 SIGTERM/SIGKILL（Windows 为 TerminateProcess 等价路径）终止服务，再用同一 authority/identity 与同一 idempotency key 重启，macOS 本地各 8 trials 均恢复为单行 signed/`LocallyCovered` record 且 replay 逐字节一致。该证据不定位 commit phase，也不等同真实断电。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §5.3。

> 本轮（2026-08-23 第二十增量）推进 `B-TASK-008C2G-COORD-SEM-FAULT-1`：Semantic publication coordinator 新增 named SQLite VFS 测试入口和 9 项本地故障前缀/重启矩阵，覆盖 owner `IOERR`/`ENOSPC`、`PowerLossAfter`、WAL 撕裂尾部、Task consumer 写失败及 READY 后终态 finalize 写失败；所有场景均保持 durable prefix、单 publication receipt 和单次 TaskCommitReceipt。该证据不外推真实硬件掉电、跨机器原子提交或 Principal attestation。详见 [B-TASK-008C2G-COORD](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md) §5.1。
...
>
> 当前进度同步（2026-08-23）：第十九增量外部 supervisor TS/Python 各 8 trials 在本地 macOS 通过；第二十增量 Semantic coordinator 故障矩阵 9 项、既有 coordinator 4 项、Semantic crate、workspace、Clippy/fmt 全部通过。两条新验收门尚待本次提交后的三平台手动 workflow/常规 CI 复验；它们仍保持 `PARTIAL_PASS`，不改变真实 Capability authority、Principal peer attestation、trusted-clock anti-replay、真实断电或完整跨 authority TaskWriteSet 的未决状态。
...
- 本地 macOS TS/Python 各 8 trials 均恢复为单行 signed/`LocallyCovered` record，第二次 replay 逐字节一致；该门仍不定位 commit phase，也不等同真实断电。三平台手动 workflow 待本次提交后运行。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC 外部 supervisor CSPRNG 时序终止 PARTIAL PASS（B-TASK-008C2G-IPC-EXTERNAL-TIMING-KILL，本地 macOS TS/Python 各 8 trials 通过；三平台 workflow 待复验）
  → Semantic publication coordinator 故障前缀/重启矩阵 PARTIAL PASS（B-TASK-008C2G-COORD-SEM-FAULT-1，本地 9 项通过；仅本地 SQLite/VFS 证据）
```
