...
> 本轮（2026-09-04 第七十增量）波次 13 编排者救回中止车道后五车道 canonical 落地、一车道未启动。(1) [W12-J] ROAD-B-006 structured join/detach（commit `0782403`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §5 追加）：`RuntimeAdapter::join_fiber`/`detach_fiber`、`FiberExit` 终态 Condvar、generation fence、implicit detach 槽位回收；spawn 后 Running 可见性竞态修复；`join_detach` 6 项 + runtime-tokio 全 crate 70 passed；Process crash/Activation meter 未做。(2) [W12-U] ROAD-B-001 uninstall 前片（commit `2211521`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md)）：schema v3 `uninstall_application` installed/disabled→uninstalled CAS + immutable receipt 幂等；31 测试；rollback/GC/运行中 Task 门未做。同波次已落地：W12-K `0a7260f`、W12-C `0690d9b`、W12-S `c82ca7a`（见第六十九增量）。W12-P process crash-propagation 子 Agent 取消/未启动——不得冒充 DONE。本地定向测试通过；CI 未在本增量后复跑。
...
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach 已有单节点 H3（[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)）；Process crash propagation、Activation meter 未做；仍为 `PARTIAL_PASS` |
