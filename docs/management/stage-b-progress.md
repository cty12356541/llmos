...
> 本轮（2026-08-19 第十增量，远端待复验）推进 `B-TASK-008C2G-IPC-FAULT-MAPPING`：在实际 `TakeoverControl` IPC handler 上通过 fault VFS 注入 SQLite `IOERR` 与 `ENOSPC` 写失败；两种故障均映射为 typed `DURABILITY + RETRY_SAME_IDEMPOTENCY_KEY`，失败时不产生 durable row，解除注入后以同一 idempotency key 重试恢复为已验签、`LocallyCovered` 的单行记录。本地 TakeoverControl 9 项、workspace 447 项、feature clippy/fmt 已通过；三平台 + MSRV 与 Pages 待推送后回填。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.93。
...
>
> 当前进度同步（2026-08-19）：第十增量已完成 IPC `IOERR`/`ENOSPC` typed durability 映射与同 key 恢复的本地证明，远端复验待推送；第九增量已完成 TakeoverControl 两 caller 并发线性化与单 row 证明，三平台 + MSRV run 32194086430 已通过，Pages run 32194086576 成功；第八增量的 TypeScript/Python payload/response conformance、双连接 replay 与 Windows named-pipe 已由 run 32192662820 三平台通过。
>
...
| `B-TASK-008C2G-IPC-FAULT-MAPPING` | TakeoverControl IPC 故障映射与同 key 恢复 | `IN_PROGRESS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.93：fault VFS `IOERR`/`ENOSPC` → typed `DURABILITY + RETRY_SAME_IDEMPOTENCY_KEY`，失败无 durable row，解除后同 key 恢复为 signed/`LocallyCovered` 单行；本地 9 项、workspace 447 项、feature clippy/fmt 通过，远端待复验 | 进程崩溃/PowerLoss/WAL 撕裂尾部、真实 Unix/named-pipe 多连接压力、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
...
- 本地 `cargo test -p nlos-takeover-control --features conformance-server` 9 项、`cargo test --workspace --quiet` 447 项（另 2 项既有 scale probe ignored）、feature clippy/fmt 与 diff 检查已通过；三平台 + MSRV 与 Pages 待本增量推送后回填。该证据不外推为进程崩溃、真实断电、WAL 撕裂尾部或真实 Unix/named-pipe 多连接压力。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC `IOERR`/`ENOSPC` 故障映射与同 key 恢复 IN_PROGRESS（B-TASK-008C2G-IPC-FAULT-MAPPING，本地 9 项通过，远端待复验）
```
