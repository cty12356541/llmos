...
| `B-TASK-008C2G-COORD` | Semantic publication cross-authority restart coordinator | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md)；[B-TASK-008C2G-FAULT](../evidence/stage-b/b-task-008c2g-takeover-fault-matrix.md)：schema v27–v35 lease/takeover 表组 F1–F4 故障注入矩阵（kill-9 中断/commit 后崩溃/IoErr/ENOSPC/静默丢写与 WAL 撕裂尾部/解除后继续）；[B-TASK-008C2G-SUCCESSOR-REGISTRY](../evidence/stage-b/b-task-008c2g-successor-registry.md)：successor registry generation/assignment rotation + 新 permit | schema v26 mixed finalize envelope、单机 bounded convergence、本地 slot/Receipt-bound proof binding、schema v27 durable lease/term/fencing、schema v28 opt-in CommitPermit/terminal binding、schema v29 same-term adoption/reconcile lease guard、schema v30 immutable local `FROZEN_FOR_TAKEOVER` fence receipt/exact local root、schema v31 lease-bound local assignment baseline、schema v32 pending takeover receipt prefix、schema v33 per-endpoint barrier observation、schema v34 canonical exact-fence member manifest、schema v35 barrier digest、schema v37 completion 与 successor assignment 激活、successor registry local reopen 已接线；NLOS principal→participant 授权绑定、远端 barrier/物理 cleanup 证明、旧 permit cross-term adoption、lease-binding fault matrix 三平台复验、rotation 专属故障矩阵、外部 provider proof/attestation、Trust View/vector checkpoint、多 Cell 与完整 TaskWriteSet 仍未实现 |
...
- 详见 [B-TASK-008C2G-SUCCESSOR-REGISTRY](../evidence/stage-b/b-task-008c2g-successor-registry.md)；限制：只复制本地 participant tuple，未重新取得跨 authority endpoint proof/attestation；旧 permit cross-term adoption、TS/Python conformance、Windows named pipe round-trip 与 rotation 专属 VFS 故障矩阵仍未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Cross-term adoption（旧 quarantined permit）+ TS/Python conformance + Windows named pipe round-trip NEXT（B-TASK-008C2G）
```
