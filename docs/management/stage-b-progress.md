...
| `B-IDENTITY` | Principal、ControlDomain、版本化 identity snapshot 与 signing key authority | `IN_PROGRESS` | [B-IDENTITY-001](../evidence/stage-b/b-identity-001-principal-key-authority.md)：authority-assigned identity、Ed25519 key validity/revocation、Semantic signature verification PARTIAL PASS；[B-IDENTITY-002](../evidence/stage-b/b-identity-002-key-rotation.md)：signing-key rotation 最小前缀 + kill-window 故障矩阵（IOERR/ENOSPC/PowerLoss/torn WAL/replay storm，commit `9777031`）PARTIAL PASS；[B-IDENTITY-003](../evidence/stage-b/b-identity-003-custody.md)：trusted-local key custody 最小前缀（commit `2a607b5`）PARTIAL PASS | 多 Principal domain merge/split、HSM/跨机 session、认证 session/attestation ingress、grace-period 双签、三平台 CI/真实掉电 |
...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall/rollback 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)、[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md)、[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md)、[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md) commit `5e5fae5`）；Slice K 纵切面已接线 uninstall（[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §9，commit `fc5d7f1`）；GC/运行中 Task 门与完整 `[PKG-UPDATE-001]` 生命周期策略未做 |
...
| `ROAD-B-005` Task Manager 多层手动控制与 NL/GUI/CLI 同路 | 局部推进：NL 受限语法四命令同义词白名单 + ExportMetrics（[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md)，commit `e66167e`）已有单节点 H3；Trusted GUI 与多层手动调度未做 |
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach、process crash propagation 合同层、Activation meter 最小前缀（`external_wait`/`active_cpu`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.6，commit `c3b2a10`）、阻塞 I/O 负向证明（§6.7，commit `9b262f3`）已有单节点 H3；100K 探针下分维 metering、runtime 联动、`backpressure_wait`/`suspended` 未做；仍为 `PARTIAL_PASS` |
