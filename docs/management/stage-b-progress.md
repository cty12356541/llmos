...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall/rollback 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)–[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md)）；运行中 Task activity-gate 拒绝 fresh uninstall/rollback（commit `e709753`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §W16-001）PARTIAL PASS；Slice K uninstall（commit `fc5d7f1`）+ **uninstall 后显式 orphan GC**（commit `daafa75`，§W17-001）PARTIAL PASS；**`[PKG-UPDATE-001]` SameMajor compatibility window**（commit `69aab73`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md) §7）+ **SameMinor compatibility window**（commit `82e4eb7`，§8，W19-001）PARTIAL PASS；自动 GC 触发/真实 Task 探针接线与 migration runner/health check/原子切换未做 |
