...
>
> 最后更新：2026-08-16（已纳入 `B-TASK-008C2G-SEM` Semantic publication consumer、schema v25、nested receipt、本地混合 Effect + Semantic v3 终结 hook、slot/Receipt-bound success proof，以及 `B-TASK-008C2G-COORD` Semantic-only 跨 authority restart coordinator、schema v27 durable lease/term/fencing 原语与 schema v28 opt-in CommitPermit binding；此前的 TaskWriteSet/authority 增量保持有效）
>
...
- TaskAuthority 写失败后的 owner publication exact replay 与 prefix 收敛已有 SQLite abort/VFS 局部证据；schema v26 又持久化 mixed finalize envelope，使 coordinator 可在重启后重建 v3 request；EffectClosedSuccess 也已有本地 slot/Receipt-bound proof binding；schema v27 增加单 authority durable lease/term/fencing primitive，schema v28 再把 opt-in binding 接入 CommitPermit 签发、plain v3 finalize、pre-effect close 和 mixed Effect + Semantic persisted-envelope finalize/replay，并以测试证明接管后旧 lease 在签发/终结两处拒绝。详见 [B-TASK-008C2G-COORD](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md)。仍为 `PARTIAL_PASS`：跨进程 auth、Semantic-only high-level lease path、adoption、完整 kill-9/ENOSPC/torn-write 矩阵、外部 provider proof/attestation、Trust View/vector checkpoint、多 Cell 与完整 TaskWriteSet 仍未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Semantic-only high-level lease path + cross-process auth/adoption/fault matrix       NEXT（B-TASK-008C2G）
```
