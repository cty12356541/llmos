...
| `B-TASK-008C2G-IPC-LONG-SOAK` | TakeoverControl 分钟级长时 IPC soak | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §5.2：TS/Python 独立入口默认各运行 60 秒目标压力、8 轮、32 个真实 Unix/named-pipe 连接；每语言 8 个 fresh fixture session、2048 次调用，所有 response context/原始 payload 逐字节一致，server 最终单行 `LocallyCovered`；本地默认及短回归通过；该入口未接入常规远端 workflow，暂无三平台长时 CI 证明 | 随机外部时刻注入、真实断电、时间窗 anti-replay、真实 Capability authorizer、principal-level peer attestation 与长期 SDK facade 仍未实现 |
...
- 该长时入口当前未接入常规 Rust cross-platform workflow；本 Evidence 只证明本地 macOS/Linux-compatible transport 的分钟级压力，不外推为 Windows 远端、真实断电、随机外部时刻崩溃、时间窗 anti-replay 或真实 Capability/principal attestation。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC 分钟级长时 soak PARTIAL PASS（B-TASK-008C2G-IPC-LONG-SOAK，本地 TS/Python 默认 60 秒目标压力、8 轮 × 32 连接、每语言 2048 调用已过；常规远端 workflow 尚未接入）
```
