...
>
> 最后更新：2026-08-16（已纳入 `B-TASK-008C2G-SEM` Semantic publication consumer、schema v25、nested receipt、本地混合 Effect + Semantic v3 终结 hook、slot/Receipt-bound success proof，以及 `B-TASK-008C2G-COORD` Semantic-only 跨 authority restart coordinator、schema v27 durable lease/term/fencing 原语、schema v28 opt-in CommitPermit binding、Semantic-only high-level lease finalize、schema v29 same-term adoption/reconcile lease guard、local `FROZEN_FOR_TAKEOVER` takeover-fence pre-gate、schema v30 immutable local fence receipt/exact local root、schema v31 lease-bound local `TaskAuthorityAssignment` baseline、schema v32 pending `TaskAuthorityTakeoverReceipt` local prefix、schema v33 per-endpoint barrier receipt observation、`B-SCHEMA-005` exact OS peer credential pre-gate；此前的 TaskWriteSet/authority 增量保持有效）
>
...
| `B-TASK-008C2G-COORD` | Semantic publication cross-authority restart coordinator | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md) | schema v26 mixed finalize envelope、单机 bounded convergence、本地 slot/Receipt-bound proof binding、schema v27 durable lease/term/fencing、schema v28 opt-in CommitPermit/terminal binding、schema v29 same-term adoption/reconcile lease guard、schema v30 immutable local `FROZEN_FOR_TAKEOVER` fence receipt/exact local root、schema v31 lease-bound local assignment baseline、schema v32 pending takeover receipt prefix 与 schema v33 per-endpoint barrier observation 已接线；NLOS principal/签名 peer auth、远端 barrier 验证/完成、successor assignment 激活、跨 term adoption、完整故障矩阵、外部 provider proof/attestation、Trust View/vector checkpoint、多 Cell 与完整 TaskWriteSet 仍未实现 |
...
- 仍为 `PARTIAL_PASS`：TaskAuthority consumer 已由 `B-TASK-008C2G-SEM` 接线，单 authority lease 已覆盖 mixed、Semantic-only high-level 与 same-term adoption/reconcile terminal path，并增加新 term 的 local `FROZEN_FOR_TAKEOVER` fence pre-gate、schema v30 immutable local fence receipt/exact local fence-set root、schema v31 lease-bound local assignment baseline、schema v32 pending takeover receipt prefix 与 schema v33 per-endpoint barrier observation；跨 authority prepare/consume/recovery、NLOS principal/attestation、远端 barrier 验证/完成、successor assignment 激活、跨 term adoption、Trust View/vector checkpoint 与多 Cell 仍未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Cross-process principal/attestation + cross-term adoption + complete fault matrix NEXT（B-TASK-008C2G）
```
