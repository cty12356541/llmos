...
| `B-PARTICIPANT` | TaskAuthority participant registry generation/root/freeze 与 endpoint proof | `IN_PROGRESS` | [B-TASK-007A](../evidence/stage-b/b-task-007a-self-participant-registry.md)：TaskStore self/freeze；[B-TASK-007B1/007B2](../evidence/stage-b/b-task-007b2-verified-participant-registration.md)：Artifact/Semantic proof + verified registration CAS；[B-TASK-007C1](../evidence/stage-b/b-task-007c1-resource-endpoint-proofs.md)：Driver/Resource durable endpoint proofs PARTIAL PASS | Driver/Resource Task registration、Channel endpoint、operation prepare→activate、EffectPermit/Receipt binding、takeover fence、fault matrix |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TaskAuthority verified Driver/Resource registration CAS                               NEXT（B-TASK-007C2）
