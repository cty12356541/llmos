...
> 本轮继续：`B-RESOURCE-005` 增量——QUARANTINED Reservation 在 caller 随后提供 effect-closed proof 时经同一 `finalize_reservation` 直接结算（reconciliation 解冻：冻结 high-water 为基线、同事务清除 QUARANTINED overlay 指针并写 FINALIZED overlay/receipt/退款，immutable quarantine receipt 行保留为证据）；6 测试全绿、`nlos-resource` 15 测试全过、workspace 396 测试全过；`B-TASK-008C2G-FAULT` 两轮矩阵文件均已通过三平台 CI（run 31962738904 / 31963113968）。三平台 CI（本轮 resource 变更）待运行。
...
- 详见 [B-RESOURCE-005](../evidence/stage-b/b-resource-005-finalize-refund.md)；限制：proof digest 为 caller-asserted opaque 摘要、无 endpoint 签名、risk/rebate ledger、late rebate、多维资源与跨 authority resource/cost receipt 及 TaskCommitReceipt resource consumption 接线仍未实现；三平台 CI 待运行。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Effect-closed 证明下双重记账 finalize/refund 结算 + immutable receipt             PARTIAL PASS（B-RESOURCE-005，含 QUARANTINED→FINALIZED reconciliation 解冻）
