...
| `B-TASK-008C2G-COORD-ARTIFACT-RESTART-SCAN` | Artifact publication pending restart scan | `PARTIAL_PASS` | [B-TASK-006E](../evidence/stage-b/b-task-006e-cross-authority-coordinator.md) §6.1：两个 Artifact owner publication receipt 已 durable 后重开两 authority，仅调用公开 `converge_pending` 消费两条原始 receipt，完成单条 TaskCommitReceipt、两 Artifact head=1 与 `FINALIZED`；本地 1 项、targeted Clippy/fmt/diff 通过；Rust cross-platform/MSRV run [32627556597](https://github.com/cty12356541/llmos/actions/runs/32627556597)、Pages [32627556621](https://github.com/cty12356541/llmos/actions/runs/32627556621) 成功 | 仍是单机 bounded recovery；更广泛 Artifact/Resource/Operation/Channel 接线、完整 TaskWriteSet、跨机器原子提交与真实掉电仍未实现 |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Artifact publication pending restart scan PARTIAL PASS（B-TASK-008C2G-COORD-ARTIFACT-RESTART-SCAN，本地 1 项；双 owner receipt 重开后公开 `converge_pending` 收敛；Rust cross-platform/MSRV 32627556597、Pages 32627556621 均成功）
```
