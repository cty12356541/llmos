...
> 九十修订（同日，补登 2026-09-13 波次转向裁定）：维护者 2026-09-13 晚已裁定 dash 插件与 llmos 无关，抽离为独立公开项目 [cty12356541/claude-dash](https://github.com/cty12356541/claude-dash)（单提交导入 `05e7a68`，spec/plan 权威副本随迁 docs/history）；`feat/dash-plugin` 为**证据分支**（21 提交 `d90cac9..595d2a1` 完整保留），按裁定**不并入 llmos main、不删除**——上段「dash 分支并入 main 时点未定」据此修正为「不合并」；llmos main 保留自有轻量工具（render_dag.py/dag_watch.sh/dag_session.sh/statusline 配置，不迁移），statusline 已于本日切 agentdash CLI（`77ceb18`）。`feat/dash-plugin-design` 携带 llmos 相关改动（`b81e655` render_dag mtime 修复对 main 仍有效、`89c466d`/`062b139`/`77ceb18` statusline/忽略目录 chores、dash spec/plan 历史副本），已于同日经维护者决策合入 main（merge `52ee1c3`；test_render_dag 6/6 绿、render_dag --oneline 实跑 exit 0）——上段「已完全包含于 feat/dash-plugin（并入 main 后可删）」据此作废，该分支合并后已删。波次任务状态本就完整于 `.superpowers/sdd/2026-09-13-dash-plugin/progress.md`（13/13 complete + 终审 + 实机验证战役全绿；gitignored 运行时台账，git 侧此前不可见，本补登即消解）。本日对 feat/dash-plugin 追加的三个收敛提交（`29fc86f`/`420bc7d`/`38c3910`）系在发现该裁定前所做，不改变证据完整性（`595d2a1` 仍可达），分支已推 origin。流程教训登记：**重大裁定不得只存 gitignored 运行时台账**——转向裁定若当日入账，本日的误诊与多余收敛提交可避免。

> 本轮（2026-09-13 第八十九增量）波次 26 四车道 + 集成收尾全部落地（worktree 分支 `feat/unified-recovery-plane`，基线 `1a14ae0`（main）→`2b51d28`，设计定稿 spec 先行提交 `4322a17`（W26-000），三实现车道十提交 + 3 个 chore 集成合并 + 1 个基线 lint 修复；Evidence：[B-TASK-008C2G-UNIFIED-RECOVERY](../evidence/stage-b/b-task-008c2g-unified-recovery.md)）。(1) [W26-000] 统一恢复面设计定稿 spec（commit `4322a17`，`docs/superpowers/specs/2026-09-13-unified-recovery-plane-design.md`，维护者批准方向）：把 ADR-0013 有界收敛半边从「仅 Artifact 自动、Semantic 靠调用方」补齐为两域均自动 + 统一 TaskCommitReceipt 读侧聚合，15 条规范语句（SEM-RECOV-001..007 / UNIFIED-WORKER-001..004 / RECEIPT-UNIFY-001..004）逐条对应实现位置见 Evidence §2。(2) [W26-001] Semantic 恢复台账六提交（`e5203c7` schema v42 表组（`task_semantic_recovery` + due 索引 + `task_semantic_recovery_alert_receipts` + 双不可变触发器，逐列镜像 Artifact v8/v9）→ `3ffa7e7` 迁移事务化并补部分态守卫（审查裁定 Important-1 修复环；Ruling R6/R6a：`BEGIN IMMEDIATE` 包裹 + 5 命名部件计数守卫，部分态返回 `CorruptRecord`）→ `43e3128` 台账 CAS 记录与回读（`expected_total_failures` CAS 无双记）→ `dfb9fc3` 到期扫描/resume/汇总 + 无台账重扫 → `49fd9b4` 告警面 + finalize 置 `Resolved`（三个 finalize 成功事务内接线，失败路径零台账写）+ 台账自愈 → `b07c8fa` F1–F4 故障注入矩阵）：定向门 semantic_recovery_schema 2 + semantic_recovery_ledger 9 + semantic_recovery_fault_matrix 5 全过（kill-9 中断事务/commit 后崩溃/IoErr/静默丢写四行全 PASS、零缺陷、每行 `integrity_check` ok）；`nlos-task` 41 test targets 全 ok 零回归。(3) [W26-002] worker 双域三提交（`60517bb` `durable_cycle` 拆分 artifact/semantic 半轮 + 按域健康（9 个 `RecoveryWorkerHealth` 新字段，既有 43 测试逐位不变）→ `467a213` nlos-system-control 6 处穷举字面量机械适配（Ruling R5 写集扩展，行为中性）→ `7360698` semantic 域真接线 + 域故障隔离）：`start_with_semantic_authority` 增量入口（旧 `start` 委托 `None`，semantic 半轮按构造静默），单轮线性「扫描→converge→失败记台账→退避→gauge」；`TaskStoreError` = infra 走域退避、`CoordinatorError` = 计划级进台账；单域故障位粘滞不牵连另一域，阈值沿用单一 `failure_threshold` 按域独立累计；定向门 unified_worker_dual_domain 7/7（核心 `worker_converges_pending_semantic_plan_without_caller` 无 caller 重启收敛唯一终态；一域 Faulted 后另一域继续收敛；混合 infra/plan 失败预算分离）、coordinator 50/50 + system-control 59/59 零回归。(4) [W26-003] 统一 TaskCommitReceipt（commit `4cfb91b`）：`receipt.rs` 五变体穷举 enum（Plain/Artifact/Semantic/Resource/SemanticResource）+ `commit_receipt_digest()` SHA-256 canonical 确定性摘要（公式文档钉死、变体扩展必须同步），纯读侧聚合零持久化零新表、既有 receipt 写入路径不动；定向门 unified_task_commit_receipt 4/4，`nlos-task` 39 targets 零回归。(5) [W26-004] 集成收尾（本提交）：三车道 3 个 chore 合并零冲突（`7ccbc43` 屏障 1 / `7aa4c4c`+`9333784` 屏障 2）→ 全仓门 `cargo test --workspace --no-fail-fast` **218 测试二进制 1177 passed / 0 failed / 11 ignored**（W25 基线 213/1122/11，+5 二进制恰为 5 个新套件共 27 项新测试，零 flaky）、fmt 0、clippy 0（唯一 error `effect.rs:1372` doc_markdown 经基线字节级验证为 `1a14ae0` 既有非 W26 引入，一词 backticks 修复 `2b51d28`）；spec 状态行补「已实现 + Evidence 指针」。已知限制如实登记：单机本地 SQLite/VFS H3（kill-9 页缓存存活非断电）；两台账退避分布不对称（artifact per-plan jitter vs semantic 纯函数）与 escalation 阈值不对称（artifact 走 `config.failure_threshold` vs semantic 固定 8）已在 worker.rs 注释与测试钉死；health 面观测裂隙（Semantic 失败显示为 Coordinator、semantic 条目 `plan_id=None`、故障位置位后计数冻结在阈值）；统一 receipt 不改变持久化事实，complete TaskWriteSet 不因本波次晋升；deferred minors（resume 不重读 plan 终态可留惰性孤儿 Retrying 行、部分态守卫按表名计数、NotFound 混杂、嵌套 receipt 翻转敏感性未测、故障 fixture ~215 行双份维护）见 Evidence §6。议题 35 未启动；CI 待 push 后触发；本增量未 push（控制器统一收尾）。终审补登（同日，W26 修复波）：semantic 域对 system-control 运维面不可见——metrics 全为 `nlos_artifact_recovery_*`、Acknowledge 走 artifact 命令、CLI 无 semantic 通道，`Escalated` 后的人工 resume 无任何 metric/IPC/CLI 通道可达，worker 的 9 个 semantic health 字段在生产消费方中被丢弃；接线（SABI 契约扩展 + 三语言 conformance）登记为 W27 首选车道（Evidence §6）。

> 本轮（2026-09-12 第八十八增量）波次 25 四车道 + 集成收尾全部 canonical 落地（worktree 分支 `feat/runtime-fiber-lifecycle-reaping`，基线 `b6e15c6`（PR #19 合并后 main）→`96de833`，设计定稿 spec 先行提交 `43dcf86`，四车道五提交）。(1) [W25-000] fiber/scopes 生命周期回收设计定稿 spec（commit `43dcf86`，`docs/superpowers/specs/2026-09-12-fiber-lifecycle-reaping-design.md`，维护者批准方向）：join 一次性消费、detach 声明回收、FIFO 墓碑环窗口内保留防重、scope 按 id 索引修 O(N²)、orphan 缓冲上界；§3 内存账（稳态总上界 ~10MB 量级、配置可调）。(2) [W25-001] fiber 记录 join 即回收与有界墓碑环（commit `1c28bcc`）+ [W25-001R] FiberReaped 外延错误文档与钉死测试（commit `78e1338`，审查裁定 Important-1 修复环）：三回收点全落既有 terminal 锁临界区（join 消费、detach 已终态、detach 未终态标记 `reap_on_terminal` 终态转换回收），新增锁边仅 terminal→registry 且 registry 为叶子无环；`TerminalOutcome::Consumed` 封闭并发双 joiner 至多一次消费；`FiberRegistry`（map+墓碑环同锁）使 spawn「活记录或墓碑命中」检查与 reap 原子互斥无 TOCTOU；契约层 `nlos-runtime` 新增 `RuntimeError::FiberReaped{fiber_id,generation}`（Display 穷举）并把 join/detach/spawn 契约文档改一次性消费+墓碑窗口语义；墓碑 FIFO 环默认 65_536、满则挤出、容量 0=纯消费无窗口（零容量下重 join 退化为 InvalidGeneration——FiberReaped 需墓碑命中，测试期许修正记档）；新套件 lifecycle_reap 6 用例先红后绿；兼容更新 6 处 join 重放断言（含 2 改名）+ 19 文件 config 字面量机械适配，cancel_late_callback_matrix 场景 5 零改动保持绿（防重由活记录承担，墓碑环只在记录被消费后接管窗口）；R1 补齐 lib.rs×5/wake/channel_wait/snapshot/replay 的 # Errors 文档与 durable 侧 API 外延清单 + 3 条钉死测试；runtime 双 crate 定向门 101 passed / 0 failed / 8 ignored。(3) [W25-002] scope 注册表按 id 索引与引用计数回收（commit `d1425c4`）：`ScopeRegistry`（map + `by_id` 二级索引 + 墓碑环同锁），`scope_for` 持锁线性 `any()` 扫描 → O(1) 索引查询（修 O(N²)）；引用计数 = 未回收 fiber 记录数 + 在途准入份（失败准入在 fibers 临界区外归还，fibers↔scopes 两锁从不嵌套）；三回收点统一经 `Inner::reap_fiber` 恰一次递减；归零移除条目+入 scope 墓碑环（默认 65_536）；墓碑口径经审查裁定：环内存 `(id,generation)`、窗口内同 id 异代拒（维持既有 InvalidGeneration）、被回收 `(id,generation)` 本身可立即重建新实例、FIFO 挤出/容量 0 放行——已取消 scope 的 cancelled 态不随墓碑存活（有界内存显式代价）；新增 `registered_scopes()` 观测 + 6 条 scope 用例；定向门 107 passed / 0 failed / 8 ignored。(4) [W25-003] orphaned channel_waits 上界与健康计数（commit `96de833`）：`orphan_buffer_capacity` 默认 1_024，deliver 早期投递分支在既有 channel_waits 锁同一临界区内钳制、`VecDeque<WaitId>` 伴随 FIFO 队列丢最老（HashMap 迭代序不定需真 FIFO；队列不变式：队列内 id 必有存活孤儿条目，挤出即真最老）；fiber 绑定等待不受界（ORPHAN-002，随终态 purge 天然受准入上界约束）；`RuntimeHealth::orphan_buffer_dropped_total` 单调计数器（AtomicU64 无锁读，shutdown 清空条目不动计数）+ `TokioRuntimeAdapter::health()` 新增（适配器此前无 health 通道）；容量 0=纯拒绝（投递仍成功、`DeliveryReport.buffered` 仍计数、丢弃另计 health——口径两处 doc 写明并钉死测试）；lifecycle_reap 追加 3 用例（18/18）。(5) [W25-004] 集成收尾（本提交）：Windows 实机全仓门 `cargo test --workspace --no-fail-fast` 两轮——首轮 213 测试二进制 1121 passed / 1 failed / 11 ignored（唯一失败 `activation_meter::operation_wait_accumulates_external_wait_not_active_cpu` 为 10s 状态轮询在 212+ 二进制并行负载下饥饿超时；隔离复跑 5/5 绿各 0.20s、复跑轮全绿不可复现，与 W24 登记 effect_fault_injection 环境性 flake 同型记档）、复跑轮 213 二进制 **1122 passed / 0 failed / 11 ignored**（对 W24 基线 1104：+1 测试二进制 lifecycle_reap、+18 测试）；`cargo fmt --check` FMT_EXIT=0、`cargo clippy --workspace --all-targets -- -D warnings` CLIPPY_EXIT=0（双 0）；`b-runtime-002` §6.1「终态占位」表述追加 §6.15 登记性说明（历史原文不删：占位由永久改为有界窗口，窗口内语义与原文逐字一致，场景 5 测试零改动保持绿）。已知限制如实登记：join 从可无限重放改为一次性消费（生产代码无多 join 调用方，审查证实）；durable 侧 API 对已回收句柄 fail-closed 报 `FiberReaped`——外延清单：`snapshot_handler_entry`/`resume_from_snapshot`/`gc_handler_entry_snapshot`/`resume_binding`（`ChannelWaitError::Runtime(FiberReaped)`）、`wait_for_channel`/`rearm_channel_waits`/`wait_for_operation`/`inspect_lifecycle_phase`/`begin|resume_backpressure_wait`/`begin|resume_suspended`/`inspect`/`activation_usage`（`RuntimeError::FiberReaped`），均在触碰 durable 侧之前失败、零持久化副作用，仍注册的终态 fiber 行为不变；契约层 `inspect`/`activation_usage` trait 文档仍只列 InvalidGeneration（对回收型 adapter 非穷举，留后续）；墓碑默认 65_536/65_536/1_024（`TokioRuntimeConfig` 三新字段，0=零容量环纯消费语义）；scope cancelled 态不越墓碑窗口存活；fiber/scope 墓碑 `tombstoned`/环 `contains` 为环上线性扫描 O(容量)（spec 规定 VecDeque 形态取舍）；scope acquire-失败-释放循环可累积重复墓碑条目（有界无害）；`by_id` Vec≤1 首代不变式无 debug_assert 钉死；spec §3 内存账为设计目标兑现（稳态上界 ~10MB 量级配置可调），未跑 100K 级实测内存探针；join_fiber 异步化（RT-A2）留后续波次；join/detach 回收后该 fiber 的 metering 从 `inspect_lifecycle_meter_aggregate` 聚合中消失（聚合总量可因回收下降）；`gc_handler_entry_snapshot` 须先于 join/detach 调用，否则 durable 快照行滞留（fail-closed 方向，树内无此调用序）。议题 35 未启动；CI 未在本增量复跑；本增量未 push。

> 本轮（2026-09-12 第八十七增量）波次 24 四车道 + 集成收尾全部 canonical 落地（worktree 分支 `w24`，fix 基线 `2f0b29d`（PR #18 合并后 main）→`9eacc4e`）。(1) [W24-001] nlos-takeover-control 租约获取全量迁移至时钟锚定入口（commit `14e8128`，兑现第八十六增量登记的「时钟锚定入口零生产接线」欠账）：crate 内全部 6 个 `acquire_authority_lease` 调用点迁移至 `acquire_authority_lease_anchored`（3 文件各 2 处，未锚定入口词边界 grep 零命中）；wall 来源按决策树逐点——tests/authenticated_ipc.rs（2 处）经 fixture 既有 `AuthorityClock` `inspect_wall()` 读 durable wall 高水位（时钟权威路径），conformance bin 与 takeover_control.rs 测试（各 2 处）无时钟句柄传 `SystemTime::now()` 观测值（行内/doc 注释注明 bin/test 层观测非权威时钟，不可表示读数 0 = fail-closed）；既有断言全部保留原语义（诚实观测下 min 判定与原判定一致）；takeover-control 定向门 9 passed / 0 failed。(2) [W24-002] nlos-task 补 W23 停放的三条边界与次序钉死测试（commit `69ace55`）：effect_lease_fence 钉「过期未接管租约按 Expired 拒绝而非 Fenced」、lease_clock_trust 钉「锚定边界等值判死可接管」（expires<=now 即死、相等可接管，直钉 src `>` 运算语义；分诊裁定计划括注『200==200 不接管』为计划笔误）、permit_expiry 钉「permit TTL 拒绝优先于过期租约围栏」；nlos-task 38 测试二进制独立复验 298 passed / 0 failed + clippy 双 0。(3) [W24-003] 三处语义注释补强（commit `49cfe38`+`2d25510`，纯注释零行为改动）：runtime-tokio spawn 关闭门（门只消确定性 panic/静默死记录向量，与并发执行中的 shutdown() 竞态时 spawn 仍可能返回 Ok）、nlos-task lease.rs 零锚定 Observed(0) 分支（真死租约+同 holder 由 legacy takeover(term+1) 变为 renew（term 不进位仅 lease_epoch+1），fencing token 派生仍绑定 lease_epoch 照常轮换）、effect.rs 铸造路径 TK-B1 门（PermitExpired 载荷 permit_id 为确定性预派生 id、拒绝不落盘仅诊断用）。(4) [W24-004] nlos-capability 无签名 TCB 入口收敛为强制签名（commit `9eacc4e`）：删除 deprecated 无签名 `issue_root`/`delegate`/`revoke`（ADR-0010 签名版 `*_signed` 为唯一入口、签名必填），`verify_command_signature`/`require_signed_by` 去 Option 化并删除 None 直通分支——签名验证成为全部命令路径的强制前置门；nlos-capability/nlos-semantic 共 10 个测试文件迁移至签名版入口（capability 36→33、semantic 35 不变），净删 3 个 signed↔unsigned 双向 replay 等价测试（测试主体随无签名入口消失，缺签名已成编译错误、强于运行时拒绝；底层不变量「replay 永不复验签」仍由 `replay_does_not_reverify_signature_after_key_revocation` 钉死）；全仓旧入口 grep 0 残留；capability 33 passed / semantic 35 passed / clippy 双 0。(5) [W24-005] 集成收尾（本提交）：Windows 实机全仓门 `cargo test --workspace --no-fail-fast` 212 测试二进制 1104 passed / 0 failed / 11 ignored；`cargo fmt --check` FMT_EXIT=0、`cargo clippy --workspace --all-targets -- -D warnings` CLIPPY_EXIT=0（双 0）；§3/§6 评估：W24 四车道均为存量前缀上的纯修复+测试+文档，无新工作包前缀、无状态语义迁移，按第八十六增量先例维持纯追加（评估结论登记于本行）。已知限制如实登记：TS/Py conformance 未跑（buf 未装，node v24.19.0 在位但套件未执行）；unix+feature 门控的 tests/authenticated_ipc.rs 变更在 Windows 编译为空（逐行核对通过），靠 CI unix 侧编译/执行验证；capability 双向等价测试净删 3 个（如上说明）；ADR-0011 时钟版验签迁移（identity 侧 `verify_capability_command_signature_at_clock` 已在，capability 入口仍用 caller-supplied time 旧变、identity 文档自述迁移窗口内保留）与 capa 占位主体/自报时间全量切换留后续波次（W25 安全面第二批）；effect_fault_injection.rs 全量并行下偶发 1 次失败为既有环境性（隔离复跑 5/5 绿 + 全量复跑 4 次全绿未能复现，记档供 CI flake 参考）。议题 35 未启动；CI 未在本增量复跑；本增量未 push。

> 本轮（2026-09-12 第八十六增量）波次 23 五修复车道 + 集成收尾全部 canonical 落地（worktree 分支 `w23-fixes`，fix 基线 `1083f1d`→`436228e`）。(1) [W23-001] nlos-process 平台条件收敛 `pid_map` 字段消除 Windows workspace clippy dead_code（commit `7c08586`）：两 platform kill adapter 字段各加 `#[cfg(unix)]`/`#[cfg(windows)]`、构造器签名跨平台不变、删除非读取平台假读占位；platform_kill 7 passed（Windows 实测，含新增跨平台两用例）；Linux 侧代码审查级确认、以 ubuntu CI 为准。(2) [W23-002] nlos-runtime-tokio spawn/join 跨 shutdown 窗口 fail-closed（commit `a644c80`）：`spawn_fiber` 顶部 shutdown 门 typed 返回 `RuntimeError::ShuttingDown`（消 panic 向量），`join` 改 fail-closed 循环 + `shutdown()` 持 terminal 锁 notify 唤醒 parked joiner（消永久挂死），终态 join 幂等优先于标志；新增 `shutdown_semantics` 3 测试；runtime-tokio 独立复验 92 passed / 0 failed / 8 ignored。(3) [W23-003] nlos-task `EffectPermit.valid_until_ms` TTL 执行（commit `d1a2aae`）：铸造（`requested_at_ms`）与派发（`dispatched_at_ms`）两入口过期（严格大于）typed 拒绝 `PermitExpired`，零部分状态、拒绝不落盘稳定重算，幂等重放分支优先级不变，`NoEffectReason::ExpiredBeforeDispatch` 既有语义保留；新增 `permit_expiry` 4 测试；nlos-task 独立复验 287 passed / 0 failed / 2 ignored。(4) [W23-004] nlos-task 效果面五个写路径接入 authority lease 围栏（commit `686832d`）：`validate_permit_authority_lease` 仅提 `pub(crate)` 语义零改动，mint/dispatch/outcome/no-effect/fiber 注册五入口各加 `*_with_authority_lease` 梯级（原公共签名一字不改，无租约绑定 permit 直通不扩大围栏），重放分支与 W23-003 TTL 优先级钉死；新增 `effect_lease_fence` 4 测试；nlos-task 37 测试二进制 0 failed。(5) [W23-005] nlos-task authority lease 接管判定新增时钟权威锚定入口（commit `436228e`）：`acquire_authority_lease_anchored(request, clock_wall_ms)` 以 `min(自报 requested_at_ms, wall)` 判活（自报时间只能更活不能更死）、`wall=0` fail-closed 拒绝接管；存量入口原签名原语义逐字保留；新增 `lease_clock_trust` 4 测试（红→绿）；nlos-task 38 测试二进制 0 failed。(6) [W23-006] 集成收尾（本提交）：Windows 实机全仓门 `cargo test --workspace --no-fail-fast` 212 测试二进制 1104 passed / 0 failed / 11 ignored；`cargo fmt --check` FMT_EXIT=0、`cargo clippy --workspace --all-targets -- -D warnings` CLIPPY_EXIT=0；CLAUDE.md 项目约定速查（Git 工作法，权威仍为 AGENTS.md 与渐进式披露规范）收录入库。已知限制如实登记：时钟锚定入口零生产接线——nlos-takeover-control 6 个调用点仍走未锚定存量入口（迁移 + 真实时钟取 wall 转 W24 车道）；`EffectPermitAuthorities` 结构束无 lease 槽（纯租约梯级已可覆盖，将来「租约 + owner 复验」组合可仿 `FinalizeSpec.authority_lease` 加槽）；W23-003 三个 deferred minor（铸造路径错误载荷的 permit_id 为预派生未落盘 id、派发入口 TTL 检查先于 token 校验、valid_until/observed 双负值角落——均已文档化取舍）。各车道以隔离 worktree 实跑定向验证门；议题 35 未启动；CI 未在本增量复跑；本增量未 push（7c08586 已先行推送 PR #17 分支，其余提交待合并窗口同步）。

> 本轮（2026-09-11 第八十五增量）波次 22 六车道全部 canonical 落地（base `d039cd0`；W22-R 车道超时报告后自行恢复提交 `43d4014`，其 nlos-task 依赖修补由收尾 integrator 补齐 `5c9d9dd`）。(1) [W22-001] ROAD-B-001 Slice K install 自动 orphan GC 前缀（commit `048d9ab`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §14）：install 入口默认先行 install-scoped GC pass（独立幂等 key 21/22，与手动 GC 19/20 不别名），`AutoOrphanGc` 显式旁路；`cargo test -p nlos-slice-k` → 15 passed。(2) [W22-004] ROAD-B-004 `max_task_nodes` admission gate 前缀（commit `e6ec3ef`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §11）：`register_task` 净新增路径事务内 consult `ScaleProfile::admits_task_nodes`（inclusive），同 generation 幂等 replay 绕过；100K probe 改绑 `TASK_PROFILE_100K`；scale_profile 13 passed + pressure 11 passed。(3) [W22-005] ROAD-B-005 NL inspect 同义词 `task status`/`任务状态`/`任务 状态` + `resource status`/`资源状态`/`资源 状态`（commit `e09ff64`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W22-005）：零新 ControlCommand 变体，NL↔直接构造 receipt 逐字节相等；`cargo test -p nlos-system-control` → 59 passed。(4) [W22-006] ROAD-B-006 `LifecycleMeterAggregate` OpenMetrics 文本 exposition 最小前缀（commit `f81e57f`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.14）：纯函数 `to_open_metrics_text()` 三指标（counter×2+gauge）、零新依赖、无 scrape/auth（prefix 如实登记）；runtime-tokio 55 passed。(5) [W22-P] ROAD-B-006 supervisor pid 注册表最小前缀（commit `a1da2e0`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §10）：`SupervisorPidRegistry`（同代次同 pid 幂等重放、同代次异 pid `OsPidRebind` 拒绝、严格新代次显式 supersede、旧代次 stale fail-closed；`pid_map()` 直喂两 kill adapter）；`cargo test -p nlos-process` → 35 passed。(6) [W22-R] B-RESOURCE Reservation 多维 demand 谓词最小前缀（commit `43d4014`，[B-RESOURCE-001](../evidence/stage-b/b-resource-001-driver-reservation-binding-authority.md) §W22-R）：`ResourceDemand` 固定三维（cpu_shares/memory_mib/io_weight）binding 时逐维 fail-closed，schema v5→v6 幂等迁移、零值=旧单维兼容档；nlos-resource 35 passed（+5 demand_admission）。收尾：nlos-task 5 测试文件构造点补 `ResourceDemand::default()`（`5c9d9dd`，44 passed 复验）；全仓 `cargo check --workspace --all-targets` 零错误。多车道共享 target 锁竞争，各车道以独立 `CARGO_TARGET_DIR`/隔离 worktree 实跑验证门并如实登记；W22-004 满形 `--all-targets` 门当时被 W22-R 未提交字段阻塞（scoped 门全绿，evidence 如实登记）。议题 35 未启动；CI 未在本增量复跑；本增量未 push。

> 本轮（2026-09-10 第八十四增量）波次 21 六车道全部 canonical 落地（S/002/004/005 四车道子 Agent 已提交，P/006 两车道工作区遗留由收尾 integrator 复验后提交 `1a8baea`/`ba9e3c9`）。(1) [W21-S] B-SEMANTIC-008 TrustView retraction 观察（commit `43a95f3`，[B-SEMANTIC-008](../evidence/stage-b/b-semantic-008-trust-view.md)）：`TrustViewSnapshot` 暴露 `retracted`/`retraction`，append-only 读路径、无 schema 变更、不推导 Gate disposition；`cargo test -p nlos-semantic` → 35 passed。(2) [W21-002] ROAD-B-002 Slice K 双 process binding inspect 读回（commit `10719a3`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §13）：同 package 双 binding 登记稳定排序聚合读回；`cargo test -p nlos-slice-k` → 13 passed。(3) [W21-004] ROAD-B-004 reclaim execution outcome 最小前缀（commit `c83c6c2`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §10）：`WorkingSetReclaimOutcome` 仅 `PermitDecision::Issued` 填充读回，replay/denied/conflict 均为 `None`；scale_profile 10 passed + pressure 10 passed + clippy PASS。(4) [W21-005] ROAD-B-005 NL inspect 同义词 `health status`/`健康状态`/`健康 状态`（commit `76cf285`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W21-005）：NL 与直接构造 receipt 逐字节相等；`cargo test -p nlos-system-control` → 59 passed。(5) [W21-P] ROAD-B-006 Windows platform kill adapter 前缀（commit `1a8baea`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §9）：`WindowsPlatformKillAdapter` 经 `taskkill /F /T`（workspace forbid unsafe），非 Windows cfg stub；`cargo test -p nlos-process` → 25 passed。(6) [W21-006] ROAD-B-006 lifecycle meter aggregate inspect 前缀（commit `ba9e3c9`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.12）：`LifecycleMeterAggregate` O(n) 聚合 + 10K/100K 探针联动；meter_aggregate 2 passed + lifecycle_phase 4 passed + clippy exit 0。收尾复验修正：W21-006 车道初记 lifecycle 名过滤 8/8 系误记，实测 2 passed / 2 ignored（evidence §6.12.1 已改）；车道新增 hunk 3 处 fmt 违规已修复，`activation_meter_scale.rs:142` 与 `lifecycle_scale.rs:72` 先在漂移非车道写集、保留登记。工作区遗留 `crates/nlos-identity/src/*.w11s-bak` 与 `llm-proxy/`（内含硬编码密钥，绝不入库）已本地 exclude。议题 35 未启动。收尾全仓门复跑（2026-09-10，macOS arm64）：rustfmt 1.9.0 stable 对历史车道代码 34 处版本敏感换行漂移经 `cargo fmt --all` 机械重排（16 文件，无 token 变更）+ 2 处 `too_many_lines` 按仓库惯例补 allow（commit `e75679a`）后，`cargo fmt --all -- --check` 与 `cargo clippy --workspace --all-targets -- -D warnings` 全绿；全仓测试两段 388 + 703 passed / 0 failed / 11 ignored（第二段含 nlos-runtime-tokio 完整重跑；fmt 后 14 crate 复跑语义验证 + 其余 4 crate `cargo check`）。已知问题如实登记：`activation_meter::compute_fiber_records_active_cpu_against_elapsed_wall` 存在 active_cpu>elapsed_wall 终态竞态（[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.13，W14-M 先在、非 W21 引入，根因待修）。已 push `e75679a`→`origin/main`。同日收口：§6.13 根因修复（commit `c84c91a`，§6.13.1）——`finish_terminal` 以同临界区同时间戳闭合 `elapsed_wall`+`finalize`，压力回归旧代码 6/6 红 → 新代码 10/10 绿，runtime-tokio 55 passed / 0 failed。

> 本轮（2026-09-09 第八十三增量）波次 20 六车道全部 canonical 落地 push `c4567c2`→`origin/main`。(1) [W20-004] ROAD-B-004 working-set reclaim **执行**最小前缀（commit `dfdf6b0`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §9）：issued permit + advisory 时规划 `RebuildableCache` 首 phase `WorkingSetReclaimExecution`；`cargo test -p nlos-task --test scale_profile` → 8 passed。(2) [W20-006] ROAD-B-006 lifecycle backpressure/suspended **100K** ignore 探针（commit `aa71d6c`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.11）：`lifecycle_scale --include-ignored` 2 passed ~1.2s（macOS arm64，rss≈156MB，threads=4）。(3) [W20-P] ROAD-B-006 POSIX platform kill 真实 OS 最小前缀（commit `207ddd2`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §8）：`PosixPlatformKillAdapter` 注入 ProcessId→pid 映射 + SIGTERM/ESRCH→AlreadyTerminated；`cargo test -p nlos-process` → 23 passed。(4) [W20-002] ROAD-B-002 Slice K 全链 registration inspect 接线（commit `689d557`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §12）：`end_to_end` happy chain 断言 background_tasks/process_bindings；`cargo test -p nlos-slice-k` → 12 passed。(5) [W20-005] ROAD-B-005 NL inspect 同义词 `检查健康`/`检查 健康`（commit `c4567c2`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W20-005）：`cargo test -p nlos-system-control` 全 crate 绿。(6) [W20-I] B-IDENTITY trusted-local session count 读回（commit `46286e5`+`d896225`，[B-IDENTITY-004](../evidence/stage-b/b-identity-004-session-ingress.md) §5）：`count_trusted_local_sessions()`；`cargo test -p nlos-identity` → 25 passed。W20-005 子 Agent 超时后部分 commit 已由 `c4567c2` 救回；W20-I 误暂存 `.w11s-bak` 已由 `d896225` 自 index 移除（不得冒充 canonical 产物）。议题 35 未启动；CI/workspace 全仓门未在本增量复跑。

> 本轮（2026-09-08 第八十二增量）波次 19 五车道 + W19-PATCH 全部 canonical 落地 push `3c12370`→`origin/main`。(0) [W19-PATCH] W18-006 10K 编排者收尾复跑 + 100K activation-meter `--include-ignored` 实跑登记（commit `2236e26` + evidence §6.9.4 追加，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)）：10K 1 passed ~12.6s；100K 1 passed ~13.4s（macOS arm64，threads=4，rss≈203MB）。(1) [W19-001] ROAD-B-001 `CompatibilityWindow::SameMinor` PKG-UPDATE 前缀（commit `82e4eb7`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md) §8）：pre-mutation major+minor fail-closed + replay bypass；`cargo test -p nlos-application --test application_authority update_compat` → 6 passed。(2) [W19-006] ROAD-B-006 lifecycle backpressure/suspended 10K ignore 探针（commit `8425be7`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.10）：`lifecycle_scale` `--include-ignored` 1 passed ~0.23s。(3) [W19-004] ROAD-B-004 working-set reclaim advisory 执行前缀（commit `1b41afb`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §8）：`WorkingSetReclaimAdvisory` on issued permit + `inspect_working_set_pressure`；`cargo test -p nlos-task --test scale_profile` → 6 passed。(4) [W19-P] ROAD-B-006 platform kill `AlreadyTerminated` + inspect 读回（commit `0ca7251`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §7）：`cargo test -p nlos-process` → 22 passed。(5) [W19-005] ROAD-B-005 NL inspect 同义词 `查看 健康`（commit `3c12370`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W19-005）：`cargo test -p nlos-system-control` 全 crate 绿。W19-P/W19-004/W19-005 子 Agent 首轮未提交，编排者验证后三车道自行 commit+push 救回。议题 35 未启动；CI/workspace 全仓门未在本增量复跑。

> 本轮（2026-09-07 第八十一增量）波次 18 收尾员救回最后两车道 canonical 落地 push `69aab73`→`origin/main`。(1) [W18-001] ROAD-B-001 `[PKG-UPDATE-001]` compatibility window 最小前缀（commit `69aab73`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md) §7）：`CompatibilityWindow::SameMajor` pre-mutation fail-closed + idempotent replay bypass；收尾员 `cargo test -p nlos-application --test application_authority update_compat` → 3 passed、`--lib compatibility_window` → 1 passed。(2) [W18-004] ROAD-B-004 working-set admission gate 最小前缀（commit `7e01ce7`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §7）：net-new permit consult `ScaleProfile::max_active_working_set` + replay bypass；`cargo test -p nlos-task --test scale_profile working_set_admission` → 3 passed。波次 18 六车道 + W18-R 全部 canonical；议题 35 未启动；CI/workspace 全仓门未在本增量复跑。

> 本轮（2026-09-07 第八十增量）波次 18 收尾员续收：第七十九增量后三车道 canonical 落地 push `4849cd3`→`origin/main`。(1) [W18-002] ROAD-B-002 Slice K background task + process binding inspect 接线（commit `4849cd3`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §11）：demo + `application_registrations` 2 passed；收尾员 `cargo test -p nlos-slice-k` → 10 passed / 0 failed。(2) [W18-005] B-CONTROL NL 同义词扩展（commit `ee7363a`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W18-005）：六命令 EN/ZH 变体 fail-closed；收尾员 `cargo test -p nlos-system-control` 全 crate 绿。(3) [W18-006] ROAD-B-006 Activation meter 10K ignore 探针实跑登记（commit `a82cd5b`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.9.3）：evidence 登记 macOS arm64 `--include-ignored` 1 passed；100K tier 未跑、收尾员未复跑 10K。**仍中止**（无 canonical commit）：[W18-001] PKG-UPDATE compatibility window（`nlos-application` 编译失败：`pack_package_version` const `u64::from`）；[W18-004] working-set admission gate（候选写集未提交）。议题 35 未启动；CI/workspace 全仓门未在本增量复跑。

> 本轮（2026-09-07 第七十九增量）波次 18 编排 resume 无进展中止、收尾员串行验收：[W18-R] B-RESOURCE cost-receipt 零 consumption 闭合边界 canonical 落地（commit `8b32d82`，[B-RESOURCE-006](../evidence/stage-b/b-resource-006-cost-receipt-aggregate.md) §6）；`cargo test -p nlos-resource` → 30 passed / 0 failed；push `8b32d82`→`origin/main`。补记波次 17 遗漏：[W17-006] 探针 race-free 修复（commit `8d98b78`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.9 追加）。W18-002/W18-005/W18-006 见第八十增量。

> 本轮（2026-09-06 第七十八增量）波次 17 收尾 integrator：补交 [W17-006] ROAD-B-006 Activation meter 规模探针骨架（commit `24001d0`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.9）：10K quick + 100K full 两档 `#[ignore]` 探针编译入库；`cargo test -p nlos-runtime-tokio` → 79 passed / 6 ignored；10K/100K `--include-ignored` 实跑、背压/挂起维 100K 规模验证、CI/workspace 全仓门未在本增量复跑。race-free 修复见第七十九增量 `8d98b78`。第七十七增量其余车道状态不变。

> 本轮（2026-09-06 第七十七增量）波次 17 部分车道 canonical 落地（编排/resume 中止后收尾员串行提交 push `daafa75`）。(1) [W17-004] ROAD-B-004 pressure/reclaim 声明式骨架（commit `8c73def`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md) §6）：`WorkingSetPressure`/`ReclaimPolicy` 谓词；未强制 admission。(2) [W17-002] ROAD-B-002 多 Process binding 登记（commit `8c6dd00`/`4313a2e`，[B-APPLICATION-006](../evidence/stage-b/b-application-006-process-binding.md)）。(3) [W17-005] B-CONTROL InspectTask socket parity + InspectResource NL/CLI（commit `b75d8c5`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W17-005）。(4) [W17-P] B-PROCESS platform kill 合同层最小前缀（commit `89b14a5`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §6）：stub adapter + 3 测试；真实 OS kill 未做。(5) [W17-001] ROAD-B-001 Slice K uninstall 后显式 orphan GC（commit `daafa75`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §10）。(6) [W17-006] Activation meter 100K 规模探针——见第七十八增量 `24001d0`；探针未实跑，不得冒充 DONE。(7) 议题 35 TaskPlan 声明面——波次 17 未启动。已落地五车道定向测试通过；clippy/fmt/CI/workspace 全仓门未在本增量后复跑。

> 本轮（2026-09-05 第七十六增量）波次 16 七车道全部 canonical 落地（W16-001/W16-002 子 Agent 首提交 lib 不完整，编排者救回 `e709753`/`7225313`；W16-005 子 Agent 未 push，编排者提交 `e40de48`+clippy 修复）。(1) [W16-004] ROAD-B-004 `TASK_PROFILE_100K` 常量 + `#[ignore]` 100K 注册探针（commit `1affa6b`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md)）。(2) [W16-I] B-IDENTITY trusted-local session ingress（commit `281871d`，[B-IDENTITY-004](../evidence/stage-b/b-identity-004-session-ingress.md)）。(3) [W16-P] B-PROCESS 批量 cancel 传播（commit `520cfce`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) §5）：`propagate_cancel_to_fibers` + terminal/crash 联动；3 新测试；仍为 PARTIAL_PASS（runtime 批量 cancel、platform kill 未做）。(4) [W16-006] ROAD-B-006 `backpressure_wait`/`suspended` 生命周期 + 分维 metering（commit `2332ce7`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.8）：`lifecycle_phase` 4 passed。(5) [W16-001] ROAD-B-001 运行中 Task 拒绝 uninstall/rollback（commit `e709753`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §W16-001）：`ActiveTaskActivityProbe` + gated API。(6) [W16-002] ROAD-B-002 后台 Task 登记（commit `7225313`，[B-APPLICATION-005](../evidence/stage-b/b-application-005-background-task-registration.md)）：schema v5 + `register_background_task`/`inspect_background_tasks`；29+7 测试。(7) [W16-005] ROAD-B-005 `InspectProcess` NL/CLI（commit `e40de48`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W16-005）+ clippy 修复 `e7f021e`/`68e214d`。各车道定向测试 + clippy/fmt 通过；CI/workspace 全仓门未在本增量后复跑；GitHub push 部分提交曾超时后重试。
...
> 本轮（2026-09-05 第七十三增量）波次 15 七车道中六车道 canonical 落地、一车道（W15-P）子 Agent 首次取消后编排者重委派。(1) [W15-I] B-IDENTITY trusted-local key custody 最小前缀（commit `2a607b5`，[B-IDENTITY-003](../evidence/stage-b/b-identity-003-custody.md)）。(2) [W15-U] ROAD-B-001 rollback 最小前缀（commit `5e5fae5`，[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md)）。(3) [W15-N] B-CAPABILITY kill-window 故障矩阵（commit `349fe9d`，[B-CAPABILITY-004](../evidence/stage-b/b-capability-004-fault-matrix.md)）。(4) [W15-S] B-SEMANTIC admission 故障矩阵（commit `5c8f2de`，[B-SEMANTIC-009](../evidence/stage-b/b-semantic-009-fault-matrix.md)）。(5) [W15-A] B-ARTIFACT per-revision provenance（commit `446730f`，[B-ARTIFACT-006](../evidence/stage-b/b-artifact-006-provenance.md)）。(6) [W15-B] ROAD-B-006 阻塞 I/O 负向证明（commit `9b262f3`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.7）：`blocking_io_negative` 2 passed / 1 ignored；32→256 fiber 线程次线性 + 绝对上界；runtime-tokio 75 passed / 4 ignored（`--test-threads=1`）；10K ignore 探针未本地实跑。(7) [W15-P] runtime crash 联动（runtime-tokio + process）——首次子 Agent 取消（`cbf2740f`），重委派中，不得冒充 DONE。已落地六车道 push `9b262f3`→`origin/main`；CI/workspace 全仓门未在本增量后复跑。

> 本轮（2026-09-05 第七十二增量）波次 14 八车道全部 canonical 落地（W14-C 子 Agent 取消，编排者救回未提交候选后提交）。(1) [W14-M] ROAD-B-006 Activation meter 最小前缀（commit `c3b2a10`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.6）：`UsageAccumulator`/`WaitingIo` 边界累计 `external_wait`、Running 段累计 `active_cpu`；`activation_meter` 3 项 + runtime-tokio 73 passed；100K 探针/阻塞 I/O 负向/`backpressure_wait`/`suspended` 未做。(2) [W14-I] B-IDENTITY rotation kill-window 故障矩阵（commit `9777031`，[B-IDENTITY-002](../evidence/stage-b/b-identity-002-key-rotation.md) §5）：IOERR/ENOSPC/PowerLoss/torn WAL/replay storm 五窗；20 测试（+6）；HSM/custody/session 未做。(3) [W14-C] B-CONTROL NL 同义词白名单（commit `e66167e`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W13-C）：四命令 EN/ZH 变体 fail-closed 编译 + socket receipt 等价；55 测试；GUI 未接。(4) [W14-T] B-SEMANTIC Trust View 最小前缀（commit `0092707`，[B-SEMANTIC-008](../evidence/stage-b/b-semantic-008-trust-view.md)）：`inspect_trust_view` 只读派生 taint/verification/judgment/retraction；30 测试（+4）；Gate/vector checkpoint 未做。(5) [W14-R] B-RESOURCE-006 cost-receipt 故障矩阵（commit `0eac704`，[B-RESOURCE-006](../evidence/stage-b/b-resource-006-cost-receipt-aggregate.md) §4）：finalize 四窗零 src 改动；29 测试（+4）。(6) [W14-F] B-ARTIFACT Windows 目录 fsync（commit `27b205f`，[B-ARTIFACT-001](../evidence/stage-b/b-artifact-001-content-addressed-store.md) §Windows fsync）：`FILE_FLAG_BACKUP_SEMANTICS`+`FlushFileBuffers`；Windows 实机单测未跑。(7) [W14-K] ROAD-B-001 Slice K 接 uninstall（commit `fc5d7f1`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §9 / [B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §6）：demo 09c + 2 集成测试；9 passed；Task/Process teardown 未做。(8) [W14-2] 掉电层 2 首次实跑（commit `5c1ce6a`，[B-STORE-FS-SEMANTICS-001](../evidence/stage-b/b-store-fs-semantics-001-apfs-calibration.md) §8.5）：workflow_dispatch run [33895972272](https://github.com/cty12356541/llmos/actions/runs/33895972272) success（dm-flakey 44/44 baseline+power-cut）；M4/M6/M8 模型校准仍无专项数据。各车道定向测试 + clippy/fmt 通过；CI/workspace 全仓门未在本增量后复跑。

> 本轮（2026-09-05 第七十一增量）波次 13 六车道全部 canonical 落地。(1) [W12-P] B-PROCESS crash propagation 最小前缀（commit `833ee82`，[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md)）：schema v3 `mark_process_terminated`/`propagate_crash`、terminal 后 fiber incarnation 注册/resume/active readback fail-closed、重启幂等 replay；22 测试（+5）；仍为 PARTIAL_PASS（runtime 侧传播、平台 kill、Activation meter 未做）。同波次已落地：W12-J `0782403`、W12-U `2211521`、W12-K `0a7260f`、W12-C `0690d9b`、W12-S `c82ca7a`（见第七十/六十九增量）。本地 `nlos-process` 定向测试 + clippy/fmt 通过；CI/workspace 全仓门未在本增量后复跑。

> 本轮（2026-09-04 第七十增量）波次 13 编排者救回中止车道后五车道 canonical 落地、一车道未启动。(1) [W12-J] ROAD-B-006 structured join/detach（commit `0782403`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §5 追加）：`RuntimeAdapter::join_fiber`/`detach_fiber`、`FiberExit` 终态 Condvar、generation fence、implicit detach 槽位回收；spawn 后 Running 可见性竞态修复；`join_detach` 6 项 + runtime-tokio 全 crate 70 passed；Activation meter 未做。(2) [W12-U] ROAD-B-001 uninstall 前片（commit `2211521`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md)）：schema v3 `uninstall_application` installed/disabled→uninstalled CAS + immutable receipt 幂等；31 测试；rollback/GC/运行中 Task 门未做。同波次已落地：W12-K `0a7260f`、W12-C `0690d9b`、W12-S `c82ca7a`（见第六十九增量）。W12-P process crash-propagation 子 Agent 取消/未启动——不得冒充 DONE。本地定向测试通过；CI 未在本增量后复跑。

> 本轮（2026-09-04 第六十九增量）波次 13 六车道启动、编排者中止后三车道 canonical 落地、三车道留未提交中止候选。(1) [W12-K] B-IDENTITY signing-key rotation 最小前缀（commit `0a7260f`，[B-IDENTITY-002](../evidence/stage-b/b-identity-002-key-rotation.md)）：`rotate_key` schema v2、双 fence CAS、immutable receipt、旧 generation 验签 fail-closed；11 测试（+1）；仍为 PARTIAL_PASS。(2) [W12-C] B-CONTROL ExportMetrics NL/CLI 前缀（commit `0690d9b`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W11-C 追加）：`export metrics`/`导出指标` 白名单 + CLI `export-metrics`，OpenMetrics 投影与 `export_metrics` parity；55 测试；仍为 PARTIAL_PASS。(3) [W12-S] B-SEMANTIC declassification receipt 最小前缀（commit `c82ca7a`，[B-SEMANTIC-007](../evidence/stage-b/b-semantic-007-declassification.md)）：schema v6、`issue_declassification_receipt`、Assertion admission taint 降标；26 测试（+4 declassification bin）；仍为 PARTIAL_PASS。W12-J join/detach（`join_detach` 6 项中 1 项失败）、W12-U uninstall（`application_authority.rs` 测试体未闭合、编译失败）、W12-P process crash prefix（写集未启动）——不得冒充 DONE。CI 未在本增量后复跑。

> 本轮（2026-09-04 第六十八增量）波次 11 六车道启动、用户中止后仅两车道 canonical 落地。(1) [W11-N] B-CAPABILITY Namespace hierarchy narrowing（commit `45c5341`，[B-CAPABILITY-003](../evidence/stage-b/b-capability-003-namespace-hierarchy.md)）：zero-padded 前缀子树 delegate/authorize/consume；Task exact match 不变；32 测试（+7）；仍为 PARTIAL_PASS。(2) [W11-U] ROAD-B-001 update 前片（commit `1cfb359`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md)）：`update_application` installed 状态内 CAS 推进代际 + manifest 变更门；27 测试（+7）；uninstall/rollback 未做。W11-J/W11-K/W11-C/W11-S 用户中止，工作区留未提交候选（runtime join/detach、identity rotation、NL export metrics、semantic declassification）——不得冒充 DONE。CI 未在本增量后复跑。

> 本轮（2026-09-01 第六十七增量）波次 10 四车道「最大并行」落地（W10-T/W10-R 超时被遗留 cargo 进程卡死拖累，产物经编排者救回；一次跨 crate API 连锁修复）。(1) [W10-T] B-TOPIC「扣费/信用」admission 执行（commit `30b6080`，b-topic-001 §追加）：payer 级信用账户（schema v8 immutable journal、身份域分隔派生、CHECK 武装余额）、publish 同事务扣减不足 typed 拒绝、republish 记子 payer、重放不双扣、verified readback 全量不变量（恒等式+身份重推导+spend→publication 证据解析）；attribution ledger 零改动；95 测试（+9）。(2) [W10-R] B-ARTIFACT「retention policy」（commit `a448fee`，[B-ARTIFACT-005](../evidence/stage-b/b-artifact-005-retention-policy.md)）：per-artifact retention_ms 显式设置 + 读取路径过期 fail-closed；与 GC 哲学定案（过期仅影响可读性不进删除集）；52 测试（+5）；其 resolve_head 加 now_ms 参数引发跨 crate 连锁，适配 nlos-task（sealed_at_ms 派生）与 commit-coordinator/slice-k（u64::MAX 保持原语义），三 crate 全绿（commit `6027f11`）。(3) [W10-K] B-CAPABILITY「call-limit 消耗账本」（commit `171d261`，[B-CAPABILITY-002](../evidence/stage-b/b-capability-002-call-limit-ledger.md)）：行使消耗 durable immutable 计数、幂等防双扣、耗尽 typed 拒绝；25 测试（+4）。(4) [W10-2] 掉电层 2 CI 车道（commit `f42d3fb`，b-store-fs-semantics-001 §层2 追加）：dm-flaky 虚拟化掉电 workflow（workflow_dispatch 手动触发，含 SIGKILL 时序降级分支）；workflow 交付未实跑如实登记。CI 三平台 success（`6027f11`）。

> 本轮（2026-09-01 第六十六增量）三车道落地（W9-V 经一次接管续建）。(1) [W9-C] ROAD-B-006 cancel/late-callback 矩阵（commit `6aab77c`，b-runtime-002 §追加）：durable wait 挂起中取消、唤醒/取消双序竞态终态唯一、取消后重 spawn 代次守卫、late-callback 不复活不 panic——§4 勾销 cancel/late-callback 项；structured join/detach API 未落地如实登记；64 测试（+6）。(2) [W9-V] B-SEMANTIC Judgment/Verification/Retraction admission（commit `b4fa406`，[B-SEMANTIC-006](../evidence/stage-b/b-semantic-006-judgment-verification-retraction.md)）：五类事件推进至四类落地（§17.2/§17.3/§17.4 最小子集），Retraction append-only 撤回行；22 测试（+3）；Trust View 集成等裁剪如实登记。(3) [W9-D] [议题 35](../discussions/35-TaskPlan声明面设计.md) 开档（commit `73dd232`）：TaskPlan/TaskNode 声明面四候选 + 推荐倾向 + **需用户决策清单**（manifest 扩 task 模板字段与 schema 冻结纪律交互等）——ROAD-B-004 剩余与 Task 模板桥的实现铺路。CI 慢核竞态复发一次并修复（`06718cc`：fiber replay settle 轮询加 async sleep 兜底，thread::sleep 死锁陷阱注释钉住）。CI 三平台 success（`06718cc`）。

> 本轮（2026-08-31 第六十五增量）「阶段 B 退出门冲刺」三轨道落地（本波次五车道因并行 cargo 构建锁互锁连环超时，产物经编排者串行收尾与双重实跑验证；根因与纪律已复盘）。(1) [W8-F] ROAD-B-006 前片（commit `e03ea91`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)）：100K durable-wait fiber 承载 probe——2 tokio worker 恒定 4 线程承载 100K（spawn 202ms、RSS ~2KiB/fiber、唤醒子集精确、register_settle ~190s 为 fsync 瓶颈与 task probe 一致量级）；双独立实跑数据互相印证（register 190/211s、RSS 198/200MB）；cancel/late-callback 等其余门条款如实登记。(2) [W8-S] ROAD-B-004 前片（commit `0bb6095`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md)）：ScaleProfile 骨架 + TASK_PROFILE_10K 档 + 10K 实跑（注册 mean 1.77ms、10K 库上 permit p50 346µs 不退化、惰性查询面 key-scoped 断言）；TaskPlan/TaskNode 声明面未落地如实映射登记。(3) [W8-L] 生命周期补全（commit `f6b2a30`）：disable_application（schema v2 immutable 幂等回执、状态机 CAS、异键拒绝）+ Slice K demo 09b lifecycle 演示（重装推进代际→停用→fail-closed，实跑 EXIT=0）；修复 demo seed 冲突与 CI 新 lint（unused_async/fmt）。CI 三平台 success（`a54990e`）。

> 本轮（2026-08-30 第六十四增量）三车道并行落地下列成果。(1) [W7-M] B-TASK-006L 多入口 parity 收尾（commit `5490b75`，b-task-006l §追加）：全面勘察结论如实登记——生产 IPC 路径已全部收敛（三服务 lib+认证入口均走 handle_for_ipc，唯二裸 handle 在 conformance 冻结面与 nlos-ipc 设施 bin 不动）；补真正缺口：同一命令经 in-process/plain-socket/authenticated 三入口的 typed 失败（Rights/NotFound）与成功 Receipt 逐字节相等，52 测试——B-TASK-006L 遗留至此仅剩外部场景项。(2) [W7-A] B-ARTIFACT GC 执行最小前缀（commit `832c58a`，[B-ARTIFACT-004](../evidence/stage-b/b-artifact-004-gc-orphan-collection.md)）：collect_orphan_blobs 保守 GC——引用集三层防御（全 revision+全 staged 含已发布+head），只删可证明孤儿；delete-then-receipt 排序保证 committed 回执永真；schema v5 immutable 幂等回执；cache 保留域分离；kill-window 精简矩阵双向；47 测试——单写者纪律前提与无自动触发/retention 如实登记。(3) [W7-Z] ADR-0014 冻结标记机械化（commit `7b2e357`，[B-SCHEMA-015](../evidence/stage-b/b-schema-015-registry-freeze-marker.md)）：REGISTRY frozen 字段（前 6 项 true/PrincipalHandshake false）+ registry_frozen() 查询；wire 零扰动实证（check-generated 零 diff）；21 测试。证据补提交 `b563d17`。CI 三平台 success（`7b2e357`）。

> 本轮（2026-08-30 第六十三增量）三车道并行落地下列成果。(1) [W6-R] ROAD-B-003 剩余可做项补全（commit `b4de16e`，[B-TASK-ROAD-B003-GAPS](../evidence/stage-b/b-task-road-b003-gaps.md)）：跨 Task handle 泄漏（T1 permit/attempt 身份在 T2 全部 task-scoped 路径 typed 拒绝零部分状态，双向）与 snapshot 漂移（root-only 与 head 前进两种漂移均 Conflicted 终态 fence 不可复活）——ROAD-B-003 六项中可做四项（双 Attempt 唯一提交/cancel 竞态/handle 泄漏/snapshot 漂移）全部落地，provider cache 降级与投机副作用 fence 因 provider 面未落地登记不适用；接管车道（前代理超时中断于验证门与证据环节，补完 clippy 修复与证据落档）；nlos-task 全量 253 passed 零失败。(2) [W6-P] Slice K Process 物化接线（commit `e66411b`，b-slice-k-001 追加）：fiber spawn 前经 ProcessAuthority 注册 durable process binding（generation/fence），fiber 字段改取注册结果；crash recovery 断言绑定行存活代次不变重放幂等；demo 增 materialize-process 步骤——缺口清单再勾销一项。(3) [W6-G] C# 探针补全 4/4（commit `a6ad75b`，b-sdk-csharp-001 追加）：Envelope 家族全四条 golden byte-equal + roundtrip，16/16 PASS 与 Go 探针对齐。CI 三平台 success（`a6ad75b`）。

> 本轮（2026-08-30 第六十二增量）三车道并行落地下列成果。(1) [W5-D] 双 Attempt 竞争与 cancel/commit 竞态场景（commit `3500c9c`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §竞争追加）：两种请求顺序均恰好一个 CommitPermit Issued、败者 typed Superseded 终态且重试 fail-closed（InvalidAttemptState），胜者唯一提交 head=1 且二次 converge 幂等空；cancel/commit 双向竞态按 TaskAuthority 实测语义断言（permit-first 线性化：cancel 不清 permit 收敛仍完成唯一提交；cancel 先发：双 Attempt 闭 + CancelledBeforeEffect 回执重放）——议题 31 §6 条 2-3「理念空白的构造性证明」在纵切面首次落地，ROAD-B-003 前片证据。(2) [W5-N] 受限语法自然语言控制前缀（commit `9eb868d`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md)）：双语白名单解析器编译为既有 ControlCommand（编译器前端非特权路径），ack 句法显式携带 CAS 期望（NL-AMBIG-001），NL 句与直接构造经真 Unix socket dispatch 的 Receipt 逐字节相等——ROAD-B-005 等价路径首片构造性证据；9 新测试。(3) [W5-A] [ADR-0015](./adrs/0015-dual-layer-untrusted-execution-isolation.md)（commit `76d04c8`）：exp8 candidate 晋升 canonical，双层隔离（独立 host Process + 进程内 Wasmtime 按信任档分层）七条决定逐条对齐实测，GuaranteeTier 挂接按 §10.4 + [TCB-DOWNGRADE-001]，B-SANDBOX 结论定案。CI 三平台 success（`76d04c8`）。

> 本轮（2026-08-30 第六十一增量）Slice K 纵切面首次端到端贯通。新 crate `nlos-slice-k`（commit `43b7d37`）：SliceKRuntime 统一组装七权威（identity/artifact/application/task/clock/operation/runtime），[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) 落档管理 README 第一纵切面全链——signed Package → verify → install Application → Task/Attempt → CommitPermit → Fiber 执行 Driver Operation（register/dispatch/complete）+ stage + plan → converge → TaskCommitReceipt → cancel 路径（CancelledBeforeEffect + runtime scope 取消）→ crash recovery（drop 全部权威 → reopen → converge_pending 收敛至 head=1，二次 converge 幂等空）→ inspect；demo bin（slice-k-demo）全链可 grep 输出，3 集成测试全绿；只用已落地公开 API 组装零新语义，缺口如实登记（TaskSpec 无自由字段致 application↔task 关联在 slice 层、NL 路径未接、多 Attempt 竞争未含）。同波次：B-TASK-006M 未决项首片 OpenMetrics 文本 renderer（commit `16c0fc0`，42 测试，sink 消费者 + fail-closed label 验证）；C# golden 探针（commit `380931d`，dotnet 8.0.424 官方 SHA512 校验 + 手写 codec 零依赖，两条 golden byte-equal + roundtrip 14/14 PASS）。CI 三平台 success。议题 31 的 Slice K 证据门自此有实体——后续沿缺口清单推进（Task 模板桥、多 Attempt 竞争、NL 控制）。

> 本轮（2026-08-30 第六十增量）CI 修复链 + 两波次并行实现。CI 链（7 commit，`da16081`→`baf86fa` 三平台复绿，外部遗留 Windows 编译错 + CI stable 新 lint chunks_exact + cfg 盲区群 + SQLite URI 平台坑 + create_dir_all code 123 + fiber_replay CI 慢核竞态断言，确立 nightly 双工具链 clippy 与 file:/// URI 归一等纪律并注入后续车道指令）。波次 2（3 车道，commit `9177f1c`/`4c1387f`/`2c424da`）：[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md) 新 crate nlos-application（verified package → durable installation，七项绑定守卫 + 状态机 trigger + clock 镜像故障矩阵，20 测试）；nlos-clock schema v2 wall 域（v1→v2 additive 迁移、双域论证分离、WallSource 注入源、回拨/重启不回退）+ nlos-identity at_clock 验签变体（取代上层传时间占位，34 测试）；nlos-ipc handshake::transport 认证传输设施（authenticated_serve_one/connect + endpoint binding 派生 + nonce 单次性，32 测试）。波次 3（3 车道，commit `d005f15`/`8d3da50`/`885ef23`）：三服务认证接线全部落地——SystemControl（envelope caller 与 verified principal 绑定 + CLI --auth，31 测试）、WaitControl（principal 域分隔幂等键派生 + inspect_wall 零副作用读，18 测试）、TakeoverControl（两层签名关系文档化 + authenticated-ipc feature，14 测试）——ADR-0011 的「10+ 工作包跨 Principal 尾部」至此全部接线。补 Windows CI 句柄时序修复（`bdbed11`：application 测试清理 best-effort）。验证：各车道定向门 + 双工具链 clippy/fmt 全绿，CI 三平台 success；B-TASK-006L 遗留仅全量生产 caller 迁移与 NL 路径。详见各 evidence 追加（b-task-006l/b-wait-001 §9/b-task-008c2g-ipc §5.5/b-control/b-clock/b-identity/b-application-001）。

> 本轮（2026-08-29 第五十九增量）按用户「最宽宽度并行」指令以 8 车道波次落地七项决策的首批实现（写集两两不相交，逐车道原子提交；FIBER/CTRL/PKG 三车道因队列/无活动超时由接管代理勘察后续建）：CLOCK 新 crate `nlos-clock`（AuthorityClock 单调水位 CAS + 幂等 tick 回执 + 7 项故障矩阵，11 测试，commit `57e6197`）；FS 掉电层 1 APFS 语义校准矩阵（fault VFS 写边界丢失 ≡ 同长度真实 WAL 截断恢复，11 测试，commit `1a40c37`；全仓跑暴露 oracle 对 xWrite 粒度过规格的间歇性失败，改为从实测字节推导，commit `1239a35`，8 连跑验证）；AUTH ADR-0011 握手最小前缀（`nlos.sabi.PrincipalHandshake` 入 REGISTRY 第 7 项 additive + golden + 三语言生成物同步 + nonce 一次性注册表 + 真实 Unix socket roundtrip，41 测试，commit `b0badd5`）；PKG Package manifest 签名验证（Ed25519 经 identity 当前 key generation、内容寻址绑定同事务防 TOCTOU、schema v4 immutable receipt + 故障注入，43 测试，commit `90e0420`）；CTRL B-CONTROL 控制面最小前缀（ControlCommand 统一 dispatch 走 SystemControl 既有 handler + CLI 真实 Unix socket、两路径 Receipt 逐字节等价钉住，23 测试，commit `ba9ff9e`，含 harness accept 空闲超时竞态修复与 HEAD 快照隔离复跑）；SANDBOX exp8 Wasmtime/WASI vs host process 四维实测（fuel/epoch 精度、内存 enforcement、capability 面收敛、故障隔离；结论为 candidate 建议后续 ADR，commit `3df1603`）；GOLANG Go golden 探针（路线 B 手写 wire codec——protoc/buf 缺失；四条 golden 逐字节一致 + roundtrip + 边界值 + unknown 字段保留；Go 1.27.0 临时工具链镜像 SHA256 校验，commit `b8fd98c`）；FIBER ADR-0012 全量（channel v3/task v41 binding+fiber 代次列幂等迁移、消费侧登记入口镜像 register_wait、投影扩展 effect/queue、入口快照 latest-only+终态 GC+SnapshotUnavailable 接线、incarnation CAS+supersede sweep，四 crate 350 测试，修复前代理 state_seq BLOB/INTEGER CAS 类型 bug，commit `1497220`）。验证矩阵：全仓 test fail-fast 前半 395 passed + 下游缺口定向补跑 178 passed（commit-coordinator/takeover-control/topic/wait/wait-control）+ 各车道门，零失败；clippy `--workspace --all-targets --all-features -D warnings` 与 fmt --all 全绿。详见 [B-CLOCK-001](../evidence/stage-b/b-clock-001-authority-clock.md)、[B-STORE-FS-SEMANTICS-001](../evidence/stage-b/b-store-fs-semantics-001-apfs-calibration.md)、[B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md) §握手追加、[B-ARTIFACT-003](../evidence/stage-b/b-artifact-003-package-signature.md)、[B-CONTROL-001](../evidence/stage-b/b-control-001-control-command-cli.md)、[B-SANDBOX-001](../evidence/stage-b/b-sandbox-001-isolation-comparison.md)、[B-SDK-GO-001](../evidence/stage-b/b-sdk-go-001-golden-probe.md)、[B-PROCESS-002](../evidence/stage-b/b-process-002-fiber-replay.md) §ADR-0012 追加。

> 本轮（2026-08-29 第五十八增量）决策批次落档：用户在决策会话中定案七项悬置决策，以四个 ADR 与排程变更持久化。(1) Slice K 纵切面立即全量并行（控制面/沙箱/打包车道与地基收尾同跑，B-CONTROL/B-SANDBOX 车道开放）；(2) 跨进程 Principal 认证采用签名贯穿（challenge-response 握手 + ADR-0010 签名命令 wire 化）并将 AuthorityClock 纳入设计范围，解锁 10+ 工作包跨 Principal 尾部（[ADR-0011](./adrs/0011-ipc-principal-auth-signature-passthrough.md)）；(3) B-PROCESS-002 三未决定案：effect/queue 登记式投影、handler 入口快照 latest-only、fiber 代次借 B-PROCESS-001 generation 权威（[ADR-0012](./adrs/0012-fiber-projection-registration-and-entry-snapshot.md)）；(4) verify-then-commit + 有界收敛升格为跨 authority 提交正式契约，coordinator 家族长期未决关闭（[ADR-0013](./adrs/0013-cross-authority-verify-then-commit-contract.md)）；(5) 真实掉电验证采用分层阶梯（层 1 文件系统语义矩阵优先开道，层 2 虚拟化掉电进 CI，层 3 硬件抽验 channel/wait/task 三权威）；(6) schema 注册表通道冻结：现有 6 canonical 条目即时冻结 v1-beta additive-only，认证/时钟面作 v2 additive 落表（[ADR-0014](./adrs/0014-schema-channel-freeze-v1-beta.md)）；(7) B-SDK-LANG-EVAL 解封末位排队（Go/C# golden 探针随波次携带）。全部为决策/排程落档，无实现发生，各工作包实现状态不变。

> 本轮（2026-08-29 第五十六/五十七增量）落地用户定案的两条线。schema 线：`nlos.sabi.WaitControl` canonical 化入 nlos-schema REGISTRY 第 6 项（canonical proto 源 + 10 对 bounded codec，wire 字节不变，wait-control 局部校验单层化、13 项测试零修改，gen/ 生成物补齐 `schema:check-generated` 恢复通过，commit `020c0bb`）+ conformance server bin 与 TS/Python 客户端（三 scene、失败 envelope 形状、跨进程重启 durability，`npx tsx`/`python3` 端到端 exit 0，commit `afa26c2`）。fiber 线（B-PROCESS-002 开张）：ADR-0009 最小前缀——BindingEventProjection 按 binding 投影 wait 事件流、ResumableBinding 契约（框架在任何 arming 前 fail-closed 校验计划）、resume_binding 复用 arm_durable_row（satisfied/pending/already_woken/cancelled 分桶、二次 resume 幂等重放）、SnapshotResumable 占位，commit `c5144a8`；runtime-tokio 55 passed / 1 ignored、nlos-wait 26、nlos-wait-control 13 全部无回归。commits：`020c0bb`/`afa26c2`/`c5144a8`。effect/queue 投影、B 快照实现、conformance CI job 接入未决。详见 [B-PROCESS-002](../evidence/stage-b/b-process-002-fiber-replay.md) 与 [B-WAIT-001](../evidence/stage-b/b-wait-001-durable-wait-registry.md) §8。

> 本轮（2026-08-29 第五十五增量）将「跨进程等待」从 blocked-by 降级为**分级解阻塞**：新 crate `nlos-wait-control` 按 SystemControl 先例提供本地信任域传输前缀——WaitAuthority 五操作经 SABI envelope IPC 暴露（handle/handle_for_ipc、authorizer 注入、mutation 幂等键绑定、durable receipt 证据、失败映射 envelope），13 项测试含真实 Unix socket roundtrip；**跨 Principal 认证继承上游未决**（B-TASK-006L 真实 Capability/Principal 仍开放）。同轮以讨论议题 34 开档 fiber 执行状态重建设计（事件溯源续跑倾向/受控快照兜底，ADR 定案前不实现，commit `99d75f6`）。commit `f5fed22`；nlos-wait-control 13 passed / 0 failed，workspace clippy/fmt 全绿。详见 [B-WAIT-001](../evidence/stage-b/b-wait-001-durable-wait-registry.md) §7 与 [讨论 34](../discussions/34-fiber执行状态重建设计.md)。

> 本轮（2026-08-29 第五十三/五十四增量）按依赖顺序关闭 `B-TOPIC-001` 与 `B-WAIT-001` 的剩余可推进项（语义均经 ADR 补记代行，含被否候选与复审触发器）。波次 A 双车道并行：匹配谓词最小前缀（精确名/尾通配 `prefix*`，subscribe_pattern 枚举 + create_topic attach，attached_by 溯源，投递零改动，schema 对象存在性追踪迁移，commit `852098a`，74 passed）∥ wait 侧 rehydration（rearm_channel_waits 把 PENDING durable waits 重挂新 fiber，satisfied/pending 双桶携 future，placeholder 早到消费，durable 零副作用，commit `d3cb9a5`，runtime-tokio 45+1）。波次 B 串行（同 crate）：payer 计量 ledger（immutable ledger 行 identity 自校验 + UNIQUE 防双记，advance 首次跨过记账/compact 无覆盖前缀记 Unallocated，三向对账恒等式 attributed+unallocated+unsettled==total 修正任务书两向式结构漏洞，旁路 trim 协议违例钉住，commit `fc64a20`，86 passed）。**跨进程等待登记 blocked-by B-TASK-006L**（真实 Capability/Principal 认证权威未决），不实现。workspace clippy/fmt 全绿。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §10–§11、[B-WAIT-001](../evidence/stage-b/b-wait-001-durable-wait-registry.md) §6。

> 本轮（2026-08-29 第五十二增量）关闭 `B-TOPIC-001` 已知限制「retention 保守上界」：schema v5 为 publication 行补 payload 长度元数据（不复制消息体），retention 字节检查恢复 ADR-0007 补记精确公式——订阅者落后 channel consume 点的遮蔽窗口不再退化为全量保守值，消除对发布者的误拒；legacy 哨兵行混合模式取 max(精确和, retained) 永不低估，backlog_precision 如实上报。commit `fa62a00`；nlos-topic 63 passed / 0 failed（新增 4 项），workspace clippy/fmt 全绿。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §9。

> 本轮（2026-08-29 第五十一增量）落地 `B-WAIT-001` 已登记后续「runtime-tokio 消费接线」，commit + wakeup 契约在单机内闭环：`TokioRuntimeAdapter::wait_for_channel`（durable 注册 + 内存等待同入口，gate 顺序镜像 wait_for_operation；durable WOKEN/CANCELLED 立即解析；PENDING 且高水位已满足自 notify 翻转后立即 Woken，显式 notify 无轮询；同 key 取代者 Cancelled）与 `TokioChannelWakeSink::deliver`（commit 侧 notify 后非阻塞投递，早到缓冲/重复幂等）；run_fiber 终态 purge 与 shutdown 覆盖新注册表；runtime 取消不清 durable 行（显式 cancel_wait 专属）。nlos-wait 增 `channel_high_water` helper。commit `3b005e6`；nlos-runtime-tokio 37 passed / 1 ignored（新增 9 项）、nlos-wait 26 / nlos-channel 27 无回归，workspace clippy/fmt 全绿。fiber rehydration（等待方崩溃后重建等待）与跨进程等待仍为后续。详见 [B-WAIT-001](../evidence/stage-b/b-wait-001-durable-wait-registry.md) §5。

> 本轮（2026-08-29 第五十增量）开张 `B-WAIT-001`（B-PROCESS 家族 durable wait registry 前缀，commit+wakeup 锚点）：按用户 2026-08-29 架构选择（[ADR-0008](./adrs/0008-durable-wait-registry-authority.md)：独立 crate 归属 + enqueue/commit 后显式幂等 notify、无轮询）两波次落地。波次 1 双车道：新 crate `nlos-wait`（schema v1：register_wait 经 channel owner 回读绑定 fence 快照、WaitId 域分隔派生、notify_commits 单事务批量 CAS `PENDING→WOKEN` + 回执重放不重复翻转、cancel 按 WaitId、binding_digest trigger 冻结 + 读时重derive、重启 replay）与 ADR-0008（被否候选：扩展 process/outbox；TokioWakeSink 接线与 fiber rehydration 登记后续）；波次 2：13 项 kill-window 故障矩阵——notify 跨权威窗口双向证明（翻转与回执同生同灭无部分翻转）、三入口 IOERR/ENOSPC、PowerLoss 双向、torn WAL、replay storm、cancel CAS 窗口、注入后 trigger 守卫仍有效，零缺陷钉住。三个 canonical commit：`401c817` / `04839cc` / `7acded5`；nlos-wait 26 passed / 0 failed，nlos-channel 27 无回归，workspace clippy/fmt 全绿。仍为单机 H3 / PARTIAL_PASS：runtime 接线、fiber rehydration、跨进程等待、真实掉电未决。详见 [B-WAIT-001](../evidence/stage-b/b-wait-001-durable-wait-registry.md)。

> 本轮（2026-08-29 第四十八增量）关闭 `B-TOPIC-001` 未决项「retention 执行」（语义经 [ADR-0007 补记](./adrs/0007-topic-service-single-log-fanout.md)，延续代行决策）：retained_bytes/retention_ms 以发布侧 pre-write 背压执行——未消费 backlog + 新 payload 超字节上界、或最老被滞后持有 entry 超时间上界 → `TopicRetentionExhausted` typed 拒绝零部分状态；QUARANTINED 滞差不计入（与 delivery attempts 正交）；零删除语义不变（compact 仍为唯一删除路径）；schema 零变更。commit `504a311`；nlos-topic 59 passed / 0 failed（新增 9 项），workspace clippy/fmt 全绿。仍为单机 H3 / PARTIAL_PASS：无后台驱逐/跨 topic 聚合、真实 payer 计量、匹配谓词、wakeup、跨进程/多机、真实掉电未决。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §8。

> 本轮（2026-08-29 第四十七增量）关闭 `B-TOPIC-001` 未决项「delivery attempts 执行」（语义经 [ADR-0007 补记](./adrs/0007-topic-service-single-log-fanout.md)，用户未即时应答按推荐语义执行）：publish 成功 enqueue 的同一事务内对滞后 ACTIVE 订阅者按条计费（首条无积压免费、追平不清零），达到策略 delivery_attempts 翻转 `QUARANTINED`——poll 对其 `DeliveryQuarantined` fail-closed，advance/unsubscribe 权利保留，peer 与发布者不受影响，零消息删除；`reinstate_with_token` 显式恢复（token 校验、清零计数、cursor 保持、幂等重放）。schema v4 幂等迁移；3 项既有测试按计费语义最小修正（意图保留）；新增 topic_delivery_attempts 9 项。commit `c74e5f4`；nlos-topic 50 passed / 0 failed，workspace clippy/fmt 全绿。仍为单机 H3 / PARTIAL_PASS：计数无运行时投递循环、恢复非自动、无后台驱逐/跨 topic 聚合、已 trim 哨兵行长度不可恢复、真实 payer 计量、匹配谓词、wakeup、跨进程/多机、真实掉电未决。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §7。

> 本轮（2026-08-28 第四十六增量）关闭 `B-TOPIC-001` 已登记后续项「token-free 旧入口弃用」：`advance`/`unsubscribe` 加 `#[deprecated]`（note 指引 `*_with_token`），生产代码零内部调用（薄封装委托非弃用 inner），既有测试机械 allow 零逻辑改动，新增 2 项双向行为等价抽测（弃用入口 ↔ token 入口 receipt 逐字节相等）。commit `a356f97`；nlos-topic 41 passed / 0 failed，workspace clippy/fmt 全绿。注意：增量 45/46 的提交（`3d42dc6`/`178f25e`/`a356f97` + 本文档提交）push 时遇 github.com 网络不可达，先落地本地待恢复后统一推送，本地领先 origin/main。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §6。

> 本轮（2026-08-28 第四十五增量）关闭 `B-TOPIC-001` 未决项「ack 消费身份绑定」：advance/unsubscribe 原只凭调用者报名 subscriber_key 即可冒充推进游标（SubscriptionId 为公开派生值）——subscribe 现签发 authority 派生 consumption token（subscription_id + subscription_generation 域分隔派生，镜像 channel fencing token），新增 advance_with_token / unsubscribe_with_token additive 入口，token 不匹配零写入 fail-closed；重订代次 +1 换发、旧 token fail-closed；schema v3 幂等迁移既有行重推导；token-free 旧入口保留（channel 先例），弃用为已登记后续。commit `3d42dc6`；nlos-topic 39 passed / 0 failed（新增 7 项），workspace clippy/fmt 全绿。仍为单机 H3 / PARTIAL_PASS：token 为对称证明非跨进程认证，delivery attempts、自动 republish、真实 payer 计量、匹配谓词、retention 执行、wakeup、跨进程/多机、真实掉电未决。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md) §5。

> 本轮（2026-08-28 第四十四增量）开张 `B-TOPIC-001`：按用户 2026-08-28 架构选择（[ADR-0007](./adrs/0007-topic-service-single-log-fanout.md)：Topic 服务层单 log + per-subscriber cursor，否决 per-recipient 拷贝）落地 Topic fanout 最小前缀。波次 1 两车道并行：新 crate `nlos-topic`（schema v2，自有 SQLite authority，Channel 保持唯一队列事实源、Topic 不复制消息体）——create_topic 经 Channel owner 回读绑定 fence 快照、RSM-FANOUT-001 全参数策略 fail-closed、subscribe max_recipients admission、publish 恰好一次 enqueue + PENDING_ENQUEUE 崩溃窗口补投收敛（verify-then-commit）、poll/advance per-subscriber 单调 CAS 慢消费者隔离、compact 钳制 min-live-cursor；波次 2 两车道并行：republish cascade 预算（父行 guarded UPDATE CAS 递减耗尽 fail-closed、深度上界 pre-write、父链环检测 CorruptRecord、预算 = 初始 depth − durable 子行数审计不变量、同 key replay 不双扣不双投）与 14 项 kill-window 故障矩阵（PENDING_ENQUEUE 跨权威 PowerLoss 双向收敛、topic/channel 双侧 torn WAL、advance CAS 窗口、compact 钳制；无缺陷钉住，悬空 ENQUEUED 态文档化且检测路径已断言）。四个 canonical commit：`adb552c`（ADR）/ `89f966e`（crate 核心）/ `345a959`（cascade）/ `05ff1ff`（故障矩阵）；nlos-topic 32 passed / 0 failed，workspace clippy/fmt 全绿。仍为单机 H3 / PARTIAL_PASS：delivery attempts、自动 republish、真实 payer 计量、匹配谓词、retention 执行、wakeup、跨进程/多机、真实掉电未决。详见 [B-TOPIC-001](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md)。
...
> 最后更新（当前检查点）：2026-09-13（波次 26 统一恢复面登记；代码 HEAD 基线 `2b51d28`：全量 218 测试二进制 / 1177 passed / 0 failed / 11 ignored，fmt/clippy 双 0，CI 待 push 后触发）；旧的“最后更新：2026-08-23/2026-08-21”检查点行保留为上一检查点历史摘要。
...
| `B-TYPES` | Rust workspace 与稳定 nominal ID / Generation / CancelEpoch | `DONE` | `crates/nlos-types`；`ADR-0001`；[B-TASK-006P](../evidence/stage-b/b-task-006p-shared-nominal-identity-spine.md) 扩展 TaskWriteSet 共享 identity spine | public schema 已定 v1-beta 通道冻结（[ADR-0014](./adrs/0014-schema-channel-freeze-v1-beta.md)，additive-only）；生成约束冻结标记写入 REGISTRY 属后续实现切片 |
| `B-IDENTITY` | Principal、ControlDomain、版本化 identity snapshot 与 signing key authority | `IN_PROGRESS` | [B-IDENTITY-001](../evidence/stage-b/b-identity-001-principal-key-authority.md)：authority-assigned identity、Ed25519 key validity/revocation、Semantic signature verification PARTIAL PASS；[B-IDENTITY-002](../evidence/stage-b/b-identity-002-key-rotation.md)：signing-key rotation 最小前缀 + kill-window 故障矩阵（commit `9777031`）PARTIAL PASS；[B-IDENTITY-003](../evidence/stage-b/b-identity-003-custody.md)：trusted-local key custody 最小前缀（commit `2a607b5`）PARTIAL PASS；[B-IDENTITY-004](../evidence/stage-b/b-identity-004-session-ingress.md)：trusted-local session ingress 最小前缀（commit `281871d`）+ **session count 读回**（commit `46286e5`，W20-I）PARTIAL PASS | 多 Principal domain merge/split、HSM/跨机 session、attestation ingress 扩展、grace-period 双签、三平台 CI/真实掉电 |
| `B-CAPABILITY` | Capability issue、attenuation、delegation、revoke 与 reference monitor | `IN_PROGRESS` | [B-CAPABILITY-001](../evidence/stage-b/b-capability-001-durable-attenuation-authority.md)：durable root issue/delegation Receipt、全维衰减、generation/ancestor fence、verified-signer Semantic authorization PARTIAL PASS；[B-CAPABILITY-002](../evidence/stage-b/b-capability-002-call-limit-ledger.md)：call-limit 消耗账本 PARTIAL PASS；[B-CAPABILITY-003](../evidence/stage-b/b-capability-003-namespace-hierarchy.md)：Namespace zero-padded 前缀子树 narrowing PARTIAL PASS；[B-CAPABILITY-004](../evidence/stage-b/b-capability-004-fault-matrix.md)：kill-window 故障矩阵最小前缀（issue/delegate/consume 四窗，commit `349fe9d`）PARTIAL PASS | 跨进程认证入口、通用 object/right registry、kill-9/torn-WAL/revoke 扩展窗、authority 侧 Namespace 派生 API |
| `B-SEMANTIC` | canonical SemanticEvent、签名、lineage、Admission/Durability 与 authority view | `IN_PROGRESS` | [B-SEMANTIC-001](../evidence/stage-b/b-semantic-001-durable-assertion-admission.md)：durable Assertion；[B-SEMANTIC-002A](../evidence/stage-b/b-semantic-002a-canonical-intent-spec-body.md)：canonical IntentSpec identity；[B-SEMANTIC-002B](../evidence/stage-b/b-semantic-002b-durable-spec-event-admission.md)：durable signed SpecEvent admission/migration；[B-SEMANTIC-003](../evidence/stage-b/b-semantic-003-outbox-owner-readback.md)：admission outbox owner-consistent transport readback；[B-SEMANTIC-004](../evidence/stage-b/b-semantic-004-outbox-ack-writer.md)：owner-bound monotonic transport ACK；[B-SEMANTIC-006](../evidence/stage-b/b-semantic-006-judgment-verification-retraction.md)：Judgment/Verification/Retraction admission PARTIAL PASS；[B-SEMANTIC-007](../evidence/stage-b/b-semantic-007-declassification.md)：declassification receipt 最小前缀 PARTIAL PASS；[B-SEMANTIC-008](../evidence/stage-b/b-semantic-008-trust-view.md)：只读 Trust View 最小前缀（`inspect_trust_view`，commit `0092707`）+ **retraction 观察**（`TrustViewSnapshot.retracted`/`retraction`，commit `43a95f3`，W21-S）PARTIAL PASS；[B-SEMANTIC-009](../evidence/stage-b/b-semantic-009-fault-matrix.md)：admission kill-window 故障矩阵最小前缀（Assertion/Judgment 四窗，commit `5c8f2de`）PARTIAL PASS；[B-TASK-007B1](../evidence/stage-b/b-task-007b1-authority-endpoint-proofs.md)：authority-assigned admission endpoint proof PARTIAL PASS | batch DAG、TrustPolicy/Gate/vector checkpoint、跨进程 endpoint 签名/attestation、declassification policy 解引用与 Spec/Judgment 路径、kill-9/torn-WAL 扩展窗 |
...
| `B-RUNTIME` | RuntimeAdapter 与 Tokio 有界 Fiber runtime | `PARTIAL_PASS` | [ADR-0001](./adrs/0001-stage-b-core-language-and-runtime.md)、[PoC-0001](../evidence/stage-b/poc-0001-tokio-fiber-runtime.md)、[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)：100K fiber、cancel/late-callback、structured join/detach、Activation meter（commit `c3b2a10`）、阻塞 I/O 负向（§6.7，`9b262f3`）、**`backpressure_wait`/`suspended` 生命周期**（§6.8，commit `2332ce7`）、**Activation meter 规模探针骨架**（§6.9，`24001d0`）+ **探针 race-free 修复**（commit `8d98b78`）+ **10K/100K activation-meter ignore 探针实跑**（§6.9.3–§6.9.4，commit `a82cd5b`/`2236e26`）+ **lifecycle 10K backpressure/suspended 探针**（§6.10，commit `8425be7`，W19-006）+ **lifecycle 100K 探针实跑**（§6.11，commit `aa71d6c`，W20-006）+ **lifecycle meter aggregate inspect 前缀**（§6.12，commit `ba9e3c9`，W21-006）PARTIAL PASS；[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) process crash propagation + runtime terminal 门 + **platform kill 合同层**（process `89b14a5`）+ **AlreadyTerminated + inspect 读回**（commit `0ca7251`，W19-P）+ **POSIX platform kill 最小前缀**（commit `207ddd2`，W20-P）+ **Windows platform kill adapter 前缀**（`WindowsPlatformKillAdapter` taskkill /F/T，commit `1a8baea`，W21-P）PARTIAL PASS | wake latency/fairness、runtime 侧 fiber 批量 cancel 联动、跨平台 supervisor spawn/suspend/kill、完整 BirthDecision |
...
| `B-TASK-006L` | SystemControl recovery handler、bounded rejection 与 Windows local IPC | `PARTIAL_PASS` | [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)：TaskAuthority/ServiceDirectory/Unix handler、`handle_for_ipc` typed rejection、Unix denied-response evidence、Windows named-pipe success/replay；本轮三平台 + MSRV Rust CI [32614979438](https://github.com/cty12356541/llmos/actions/runs/32614979438)、Pages [32614979403](https://github.com/cty12356541/llmos/actions/runs/32614979403) 与 Schema fuzz smoke [32614979395](https://github.com/cty12356541/llmos/actions/runs/32614979395) 已成功 | 认证模型已定：签名贯穿 + AuthorityClock（[ADR-0011](./adrs/0011-ipc-principal-auth-signature-passthrough.md)）；实现与三平台验证未决、metrics exporter、多入口 parity、全量生产 IPC caller 迁移。第五十九增量落地握手最小前缀（commit `b0badd5`，evidence §追加）：challenge-response + nonce 一次性注册表 + channel binding 服务端本地对照，`nlos.sabi.PrincipalHandshake` 入 REGISTRY 第 7 项（additive，冻结 6 项零字节改动）+ golden + 三语言生成物同步，真实 Unix socket roundtrip 7 项测试；AuthorityClock 独立落地为 [B-CLOCK-001](../evidence/stage-b/b-clock-001-authority-clock.md)（nlos-clock 单调水位 + 7 项故障矩阵，commit `57e6197`）。遗留：三服务 authorizer 接线、validity 接 AuthorityClock、跨 Principal 端到端。第六十增量（2026-08-30）三服务认证入口全部落地（SystemControl `d005f15` / WaitControl `8d3da50` / TakeoverControl `885ef23`，各自 opt-in feature、真 Unix socket 认证 roundtrip + 负路径测试、既有 conformance 路径零改动），validity 经 nlos-clock wall 域 + identity at_clock 变体（`4c1387f`）接入——遗留仅全量生产 caller 迁移与 NL 路径 |
...
| `B-SDK-LANG-EVAL` | 官方 SDK 语言集合与 Go/C# 优先兼容评估 | `READY` | [多语言 SDK 支持评估计划](./language-sdk-support-plan.md)；OperationControl 前置切片见 [B-SCHEMA-013](../evidence/stage-b/b-schema-013-operation-control-timer-worker.md) | 2026-08-04 起 Go/C# generation/golden 探针与独立 IPC PoC 后移至 `B-TASK`/EffectPermit 纵切面之后（议题 31/32：第四种语言不能证明核心成立，且不应推动 SABI 在 Task/Effect 语义稳定前过早冻结）；Java/Kotlin、Swift、C/C++ 需求驱动复审；2026-08-29 决策解封：Go/C# golden 探针车道末位排队（触发条件经 ADR-0014 冻结与 Slice K 启动实质满足） |
| `B-SANDBOX` | Wasmtime/WASI 与独立 host Process 隔离对比 | `PARTIAL_PASS`（结论已经 [ADR-0015](./adrs/0015-dual-layer-untrusted-execution-isolation.md) 定案） | [B-SANDBOX-001](../evidence/stage-b/b-sandbox-001-isolation-comparison.md)：exp8 独立实验（wasmtime+wasi 手写 WAT guest + host stress 子进程，nix 零 unsafe）四维实测：fuel 指令级精确 vs rlimit 秒级粒度 vs kill 轮询延迟；wasmtime 页粒度内存拒绝 vs macOS RLIMIT_AS 不强制（实测）；WASI default-deny capability 面收敛 vs 进程全环境继承；trap 恢复 vs 子进程崩溃隔离 | 隔离机制选型已由 [ADR-0015](./adrs/0015-dual-layer-untrusted-execution-isolation.md) 定案（双层隔离：独立 host Process + 进程内 Wasmtime 按信任档分层，candidate 晋升 canonical）；遗留按其复审触发器：单平台 macOS、Linux rlimit 补测、Seatbelt、Component Model/p2 未覆盖 |
| `B-PROCESS` | native Process supervisor 与平台资源/生命周期 adapter | `IN_PROGRESS` | [v0.5 Process 规范](../design/06-架构设计总纲-v0.5.md)；[B-PROCESS-001](../evidence/stage-b/b-process-001-durable-execution-binding-authority.md)：Process/AgentInstance/IsolationDomain durable generation/fence authority PARTIAL PASS；[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md)：crash/terminal + 批量 cancel 传播（commit `520cfce`）+ **platform kill 合同层最小前缀**（schema v5、`request_platform_kill` + stub adapter，commit `89b14a5`，3 测试）+ **AlreadyTerminated + inspect 读回**（commit `0ca7251`，W19-P，22 测试）+ **POSIX platform kill 最小前缀**（`PosixPlatformKillAdapter` ProcessId→pid 映射 + SIGTERM，commit `207ddd2`，W20-P，23 测试）+ **Windows platform kill adapter 前缀**（`WindowsPlatformKillAdapter` taskkill /F/T + 非 Windows stub，commit `1a8baea`，W21-P，25 测试）PARTIAL PASS | 完整 BirthDecision、多 authority prepare/activate、macOS/Windows/Linux spawn/suspend/kill、checkpoint、IsolationUnit 与 resource mapping、Windows 实机 platform kill |
| `B-PROCESS-002` | Fiber replay 最小前缀（ADR-0009 事件溯源续跑） | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-process-002-fiber-replay.md)：BindingEventProjection（binding→wait 事件流投影）、ResumableBinding 契约（框架 arming 前 fail-closed 校验）、resume_binding（satisfied/pending/already_woken/cancelled 分桶、幂等重放、durable 零副作用）、SnapshotResumable 占位；nlos-wait `list_waits_for_binding`；新增 fiber_replay 10 项测试，runtime-tokio 55+1 全绿；commit `c5144a8` | 三项设计已定（[ADR-0012](./adrs/0012-fiber-projection-registration-and-entry-snapshot.md)：登记式投影/入口快照/代次借 B-PROCESS-001）；跨进程 replay 认证前提已定（ADR-0011）、实现未决。第五十九增量 ADR-0012 全量落地（commit `1497220`）：channel v3/task v41 binding+fiber 代次列幂等迁移、register_queue_consumption/register_effect_binding 消费侧登记（镜像 register_wait）、BindingEventProjection 扩展投影 effect/queue 事件、SnapshotResumable 转正（入口快照 latest-only + 终态 GC + SnapshotUnavailable 接线）、fiber incarnation CAS + supersede sweep；四 crate 350 测试（新增 effect_fiber_registration/fiber_registration_resume/queue_consumption_registration/fiber_incarnation）；遗留：跨进程 replay、真实掉电、绑定语义解释文档化 |
| `B-RESOURCE` | ResourceAccount、Driver/Device、Reservation 与 activation reference monitor | `IN_PROGRESS` | [B-RESOURCE-001](../evidence/stage-b/b-resource-001-driver-reservation-binding-authority.md)：Driver generation/fence + AVAILABLE→RESERVED→ACTIVE binding + **多维 demand 谓词前缀**（固定三维 cpu_shares/memory_mib/io_weight binding 时逐维 fail-closed，schema v5→v6，commit `43d4014`，W22-R）；[B-RESOURCE-003](../evidence/stage-b/b-resource-003-consumption-high-water.md)：strict ACTIVE consume high-water + immutable ConsumptionReceipt；[B-RESOURCE-004](../evidence/stage-b/b-resource-004-quarantine-freeze.md)：缺少 effect-closed 证明时 QUARANTINED freeze + immutable QuarantineReceipt；[B-RESOURCE-005](../evidence/stage-b/b-resource-005-finalize-refund.md)：effect-closed finalize/refund、故障矩阵与 activation→consume→finalize 重启前缀；[B-TASK-007C1](../evidence/stage-b/b-task-007c1-resource-endpoint-proofs.md)：Driver gateway + Resource/Ledger authority endpoint proofs PARTIAL PASS | Task registry/operation admission 接线、多维 demand、完整 Ledger/risk/rebate、AdmissionPlan、ControllerBinding、真实 enforcement shim 与平台 Device adapter |
...
| `B-RESOURCE-006` | Resource cost-receipt 聚合回读 | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-resource-006-cost-receipt-aggregate.md)：owner-derived 只读聚合 + finalize kill-window 故障矩阵（IOERR/ENOSPC/PowerLoss/replay storm，commit `0eac704`，零 src 改动）+ **零 consumption 闭合边界**（commit `8b32d82`，§6，activate→finalize 无 consume 路径 `(0,0)` high-water）；`nlos-resource` 30 passed / 0 failed，clippy/fmt 干净 | TaskAuthority 消费接线、endpoint 签名 effect proof、真实掉电仍未实现；owner-side only，不包含 Task 侧桥接 |
| `B-TASK-008C2G-RES-COMMIT` | Task 侧 Resource cost-receipt 消费接线 | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-task-008c2g-resource-cost-commit.md)：owner-derived 完整聚合（TaskWriteSet 封存 Reservation 集合 + `inspect_cost_receipt` per FINALIZED Reservation），schema v39 双表 immutable 全量明细，Task 同事务嵌套行，重启 replay 只信任 Task 行不重读 owner；`nlos-task` 186 passed + workspace 全绿（500/0/2）；[ADR-0005](./adrs/0005-task-write-set-authority-first.md) authority-first 决策补记；第三十六增量补上混合 rung：`finalize_commit_v3_with_semantic_publications_and_resource_authority[_and_authority_lease]` 双 authority 预校验（plan READY + 每 Reservation FINALIZED）后单 Task 终态事务写入双嵌套收据集合，replay 只信任 Task 行（双 owner 空 fixture 零重读），7431372 混合 fail-closed 守卫移除，nlos-task 193 passed + workspace 全绿（507/0/2）；第三十七增量补上桥接 kill-window 故障注入矩阵：11 项测试 × 六窗口（pre-commit IOERR/ENOSPC、PowerLossAfter commit 点、torn WAL tail、replay storm、mixed 双侧同生同灭）× 双 rung，全部 durable prefix 收敛且 replay 逐字节幂等，零生产代码改动、未发现缺陷，nlos-task 204 passed + workspace 全绿（518/0/2） | 跨 authority 契约已定（ADR-0013 verify-then-commit+有界收敛）；真实掉电采分层阶梯策略（层 1 fs 语义矩阵优先）；CI 未接 |
...
| `B-CHANNEL-001` | Durable Channel endpoint authority | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-channel-001-endpoint-authority.md)：canonical commit `6b7285e` 已推送，Rust cross-platform/MSRV run [32649416604](https://github.com/cty12356541/llmos/actions/runs/32649416604)、Pages [32649416595](https://github.com/cty12356541/llmos/actions/runs/32649416595) 与 Schema fuzz smoke [32649416610](https://github.com/cty12356541/llmos/actions/runs/32649416610) 均成功；owner 派生 ChannelId/generation fence/CAS rotate、authority 派生 TaskParticipantId endpoint proof 与重启 replay；第三十九增量补上 Task 侧接线：`register_channel_participant`（owner proof 回读 → ChannelTopic code 4 注册）+ `ChannelTopicBinding`(code 7) effect endpoint，seal 解析/permit freeze 逐字节复验 owner 三元组，rotate 后 fail-closed；schema v40 表重建扩 CHECK 1..7（v24 模式幂等预检）；root 域 `/v1` 不变，旧 seal replay 逐字节一致，nlos-task 217 passed + workspace 全绿（531/0/2）；第四十增量补上 EffectPermit channel gate：`request_effect_permit_with_channel_authority` mint 前回读当前 generation proof 并逐字节复验 sealed 三元组，rotate 后 fail-closed，replay 只信 Task 行（空 authority fixture 反证），legacy 与 operation 变体不变，nlos-task 223 passed + workspace 全绿（537/0/2）；第四十一增量补上 combined 全-authority 构造器：导出 `Authorities<'a>` 边界结构体（六 Option 字段）+ `seal_task_write_set_with_authorities_struct` / `request_commit_permit_with_authorities_struct` additive 入口（内层零改动、既有梯子保留），混合 Operation+Channel 写集自此可 seal/permit freeze 双重复验/replay 字节相等（梯子不可达性已实证钉住为回归测试），nlos-task 229 passed + workspace 全绿（543/0/2）；第四十二增量三车道 struct 化收尾：`EffectPermitAuthorities` 双 gate 组合入口、`FinalizeSpec` finalize 吸收（钉住并修复两个梯子不可表达缺口）、15 个梯子构造器 `#[deprecated]`（lease 入口豁免），commits `e8a7256`/`ff6ccab`/`88a9770`，workspace 全绿（558/0/2）；第四十三增量 queue delivery 前缀 + 故障矩阵：schema v2（immutable entries + consume/trim 双 high-water + bytes 簿记）、`enqueue`（fence CAS + capacity admission `QueueFull` 零部分状态 + idempotency）/有序 `receive`（跨代全序）/单调 `ack`/显式 `compact`（钳制 consume，trigger 守卫 DELETE）、CorruptRecord 交叉校验、重启 replay（commits `7faae15`）；13 项 kill-window 故障矩阵（PowerLoss 双向/torn WAL 33 点穷举/rotate CAS 无中间代/replay storm，commit `c6c9ffd`）并抓出反例 D-1：`inspect_channel` head fence 交叉校验空转自比，已改为以最新 generation 行为权威对照并转正回归测试（commit `0c9b3f1`）；nlos-channel 27 passed / 0 failed | Topic routing/fanout、payer accounting、commit+wakeup 接线（依赖 B-PROCESS wait registry）、ack 消费身份绑定、跨进程/多机语义、真实掉电仍未实现；其中 Topic routing/fanout 与 payer binding 已由第四十四增量移交 `B-TOPIC-001`（[ADR-0007](./adrs/0007-topic-service-single-log-fanout.md)），Channel 增补 queue delivery 故障矩阵与 D-1 修复后 27 passed / 0 failed |
| `B-TOPIC-001` | Topic 服务层单 log fanout（ADR-0007） | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-topic-001-topic-service-single-log-fanout.md)：新 crate `nlos-topic`（schema v2），TopicId/SubscriptionId authority 派生，create 策略（RSM-FANOUT-001 全参数）fail-closed + replay，subscribe max_recipients admission，publish 恰好一次 enqueue + PENDING_ENQUEUE 崩溃窗口补投收敛（verify-then-commit），poll/advance per-subscriber 单调 CAS 慢消费者隔离，compact 钳制 min-live-cursor，republish cascade 预算 CAS（耗尽/深度越界 fail-closed）+ 父链环检测 + 预算审计不变量；14 项 kill-window 故障矩阵（PENDING_ENQUEUE 跨权威双向收敛、双侧 torn WAL、advance CAS 窗口、compact 钳制），无缺陷钉住；commits `89f966e`/`345a959`/`05ff1ff`，nlos-topic 32 passed / 0 failed，workspace clippy/fmt 全绿 第四十五至四十七增量关闭 ack 消费身份绑定（`3d42dc6`）、弃用 token-free 旧入口（`a356f97`）并执行 delivery attempts 隔离语义（`c74e5f4`，50 passed）；第五十三/五十四增量关闭匹配谓词（`852098a`）、retention 执行（`504a311`/`fa62a00`）、payer 计量 ledger（`fc64a20`，86 passed）；运行时自动 republish、扣费/信用（ledger 为记账承诺）、共享 channel 分摊、跨进程/多机、真实掉电 |
| `B-WAIT-001` | Durable wait registry（ADR-0008，B-PROCESS 家族前缀） | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-wait-001-durable-wait-registry.md)：新 crate `nlos-wait`（schema v1），register_wait 经 channel owner 回读绑定 fence 快照 + WaitId 派生 + 漂移冲突，notify_commits 单事务批量 CAS `PENDING→WOKEN` + 回执重放不重复翻转，cancel 按 WaitId fail-closed 状态机，binding_digest trigger 冻结 + 读时重derive，重启 replay；13 项 kill-window 故障矩阵（notify 跨权威窗口双向证明无部分翻转、IOERR/ENOSPC、PowerLoss 双向、torn WAL、replay storm、注入后 trigger 守卫仍有效），零缺陷钉住；commits `401c817`/`04839cc`/`7acded5`，nlos-wait 26 passed / 0 failed，workspace clippy/fmt 全绿 第五十一增量落地 runtime-tokio WakeSink 消费接线（`3b005e6`，commit+wakeup 单机闭环，runtime-tokio 37 passed）；第五十三增量落地 wait 侧 rehydration（`d3cb9a5`，runtime-tokio 45+1）；fiber 执行状态重建（B-PROCESS 检查点域）、binding 语义解释、per-channel notify 水位、跨进程等待本地 IPC 前缀已落地（`f5fed22`，跨 Principal 认证模型已定（ADR-0011）、实现未决）、真实掉电 |
| `B-CONTROL` | CLI/API/NL/GUI 共用 ControlCommand 与 Receipt | `PARTIAL_PASS` | [B-CONTROL-001](../evidence/stage-b/b-control-001-control-command-cli.md)：ControlCommand/Receipt 有界类型面，统一 dispatch 走 SystemControl 既有 handler 路径；CLI bin 真实 Unix socket roundtrip；in-process 与 CLI 两路径 Receipt 逐字节等价有测试钉住；[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md)：受限语法 NL + ExportMetrics + 四命令同义词（commit `e66167e`）+ **InspectProcess NL/CLI**（commit `e40de48`）+ **InspectTask socket parity + InspectResource NL/CLI**（commit `b75d8c5`，§W17-005）+ **六命令 EN/ZH 同义词扩展**（commit `ee7363a`，§W18-005）+ **inspect 同义词 `查看 健康`**（commit `3c12370`，§W19-005）+ **`检查健康`/`检查 健康`**（commit `c4567c2`，§W20-005）+ **`health status`/`健康状态`/`健康 状态`**（commit `76cf285`，§W21-005）；59+ 测试 | GUI 路径未接、控制操作面仍极小、pause/cancel 无 ControlCommand 变体、权限 UI 与多层手动调度、ResourceInspector 生产接线 |
| `B-ARTIFACT` | 内容寻址 Artifact、metadata、reconcile、GC | `IN_PROGRESS` | [B-ARTIFACT-001](../evidence/stage-b/b-artifact-001-content-addressed-store.md)：内容寻址 blob 五步写入协议 + SQLite metadata + 崩溃窗口/reconcile + cache 分域 + Windows 目录 fsync 最小前缀（commit `27b205f`，Windows 实机待 CI 复验）PARTIAL PASS；[B-ARTIFACT-002](../evidence/stage-b/b-artifact-002-staged-publication.md)：staged revision + Artifact 域内原子 publish + immutable publication receipt + v1→v2 迁移（33 测试）；[B-ARTIFACT-004](../evidence/stage-b/b-artifact-004-gc-orphan-collection.md) GC 执行；[B-ARTIFACT-005](../evidence/stage-b/b-artifact-005-retention-policy.md) retention policy；[B-ARTIFACT-006](../evidence/stage-b/b-artifact-006-provenance.md)：per-revision provenance 最小前缀（commit `446730f`）PARTIAL PASS；[B-TASK-007B1](../evidence/stage-b/b-task-007b1-authority-endpoint-proofs.md)：per-head authority endpoint proof PARTIAL PASS | Task registry/operation admission 接线、加密/lineage/backfill/legal hold、sync/对象存储后端、真实 ENOSPC 探针 |
| `B-SLICE-K` | Slice K：Package → Application → Task → Fiber → Operation → Receipt → 控制 | `PARTIAL_PASS` | [B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md)：`crates/nlos-slice-k` 全链贯通 + lifecycle disable/uninstall 演示（09b/09c，commit `fc5d7f1`）+ 竞争场景（`3500c9c`）+ **uninstall 后显式 orphan GC**（commit `daafa75`，§10）+ **background task/process binding inspect 接线**（commit `4849cd3`，§11，10 集成测试）+ **全链 registration inspect 断言**（commit `689d557`，§12，12 集成测试）+ **双 process binding inspect 读回**（commit `10719a3`，§13，W21-002，13 集成测试）+ demo EXIT=0 | 单机单进程演示级；TaskSpec 无自由字段致 application↔task 关联在 slice 层；NL 路径未接；Task/Process teardown 与 rollback/自动 GC 未做 |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Semantic durable recovery ledger（schema v42 表组 + CAS + due scan/resume + 告警面 + F1–F4 故障矩阵 + 台账损坏自愈） PARTIAL PASS（B-TASK-008C2G-UNIFIED-RECOVERY，W26-001，本地 16 项定向 + 全量门 218 二进制）
  → 统一 worker 双域驱动 + 按域 Faulted/health 分列 + 无 caller 重启收敛 PARTIAL PASS（B-TASK-008C2G-UNIFIED-RECOVERY，W26-002，本地 7 项定向）
  → 统一 TaskCommitReceipt 五变体读侧聚合 + 确定性 digest PARTIAL PASS（B-TASK-008C2G-UNIFIED-RECOVERY，W26-003，本地 4 项定向）
```
...
2026-09-13 W26 增量：[B-TASK-008C2G-UNIFIED-RECOVERY](../evidence/stage-b/b-task-008c2g-unified-recovery.md) 把 ADR-0013 的有界收敛半边补齐为 Artifact/Semantic 两域均自动——schema v42 Semantic 恢复台账（CAS/退避/Escalated/告警面 + F1–F4 故障矩阵 + 台账行丢失自愈重扫）、worker `durable_cycle` 双域驱动与按域 Faulted（无 caller 重启收敛到唯一终态）、统一 `TaskCommitReceipt` 五变体读侧聚合与确定性 digest。上段「下一验收门转为跨 authority prepare/finalize coordinator 与 recovery」中的 recovery 半边至此两域均自动；**下一验收门更新为 Resource/Operation 的 prepare/finalize 入口评估（W27+ 候选，届时若有三域样本再评估台账泛化）**。complete TaskWriteSet 仍未完成，本增量不改变该口径。
...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall/rollback 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)–[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md)）；运行中 Task activity-gate 拒绝 fresh uninstall/rollback（commit `e709753`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §W16-001）PARTIAL PASS；Slice K uninstall（commit `fc5d7f1`）+ **uninstall 后显式 orphan GC**（commit `daafa75`，§W17-001）+ **install 自动 orphan GC 前缀**（install-scoped pass 默认开启可旁路，commit `048d9ab`，§14，W22-001）PARTIAL PASS；**`[PKG-UPDATE-001]` SameMajor compatibility window**（commit `69aab73`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md) §7）+ **SameMinor compatibility window**（commit `82e4eb7`，§8，W19-001）PARTIAL PASS；W27-D 真实活动门、W28-E 自动 GC tick、W29-E migration runner/health check/原子切换、W30-D teardown 链、W33-A 工具链 + W33-C conformance kit + W33-B 第三方全周期样板（[W33-H 评审](../evidence/stage-b/reviews/w33h-road-b001-b002.md)：构造性证明成立、PARTIAL 近收口）已落；载荷执行面与 `nlos package install` CLI 待做；slice-k-demo 09d 登记 defect 已处置（[B-SLICE-K-001 §17](../evidence/stage-b/b-slice-k-001-end-to-end.md)，RISK-B-12 closed，分支 `fix/w34-09d-defect` 未 push） |
| `ROAD-B-002` Application 多 Process、后台 Task、UI Surface | 局部推进：后台 Task durable 登记最小前缀（schema v5、`register_background_task`/`inspect_background_tasks`，commit `7225313`，[B-APPLICATION-005](../evidence/stage-b/b-application-005-background-task-registration.md)）+ **多 Process binding 登记**（commit `8c6dd00`，[B-APPLICATION-006](../evidence/stage-b/b-application-006-process-binding.md)）+ **Slice K inspect 接线**（commit `4849cd3`，[B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md) §11）+ **全链 registration inspect 断言**（commit `689d557`，§12，W20-002）+ **双 process binding inspect 读回**（commit `10719a3`，§13，W21-002）PARTIAL PASS；W29-F 第二 Process spawn→kill 全链（双活三层 + 真 POSIX kill + 隔离 + replay）+ W30-D teardown/关联下沉 + **W32-F UI Surface 声明→呈现**（唯一硬缺口闭合，[W33-H 评审](../evidence/stage-b/reviews/w33h-road-b001-b002.md)）已落，PARTIAL；Windows 实杀（B6-2 同源）、生命周期 NL 动词、supervisor unregister 登记 |
| `ROAD-B-003` 双 Attempt、cancel/commit、handle 泄漏、snapshot、provider cache、effect fence | 局部推进：双 Attempt 唯一 CommitPermit、cancel/commit 线性化、effect fence 四态与 quarantine/reconcile、树状取消、permit-time 组绑定/漂移围栏及 Artifact staged publication 已有单节点 H3，B-TASK-001 已有三平台证据（B-TASK-001~005、B-ARTIFACT-002）；**W26 统一恢复面**（[B-TASK-008C2G-UNIFIED-RECOVERY](../evidence/stage-b/b-task-008c2g-unified-recovery.md)：schema v42 Semantic 恢复台账 + worker 双域自动收敛（无 caller 重启收敛/按域 Faulted/台账自愈）+ 统一 TaskCommitReceipt 读侧聚合，commits `e5203c7`..`2b51d28`，全量 218 二进制/1177 passed/0 failed/11 ignored、fmt/clippy 双 0）把 ADR-0013 有界收敛半边补齐为两域均自动（单机 H3；CI 待 push 后触发）PARTIAL PASS；**W27–W33 收官波闭合**：ADR-0017 定案并实现（Resource 有界 coordinator v43 + 三域 worker；Operation verify 半边负门）、[W30-A 六域混合写集闭环验收](../evidence/stage-b/b-task-008c2g-complete-writeset.md) + W30-C provider 降级/投机 fence（[B-DRIVER-001](../evidence/stage-b/b-driver-001-mock-provider-plane.md)）、W27-A/G8 三域恢复运维面 IPC/CLI/metrics 全可达；handle 泄漏/snapshot 漂移三平台复验随本增量 CI run |
| `ROAD-B-004` 10K/100K logical TaskNode、working-set、pressure/reclaim、rehydrate | 局部推进：ScaleProfile 骨架 + `TASK_PROFILE_10K` 实跑（commit `0bb6095`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md)）+ `TASK_PROFILE_100K` 常量与 `#[ignore]` 100K 注册探针（commit `1affa6b`）+ **pressure/reclaim 声明式骨架**（commit `8c73def`，§6）+ **working-set admission gate 最小前缀**（commit `7e01ce7`，§7）+ **reclaim advisory 前缀**（commit `1b41afb`，§8，W19-004）+ **reclaim 执行最小前缀**（首 phase `RebuildableCache` 规划，`WorkingSetReclaimExecution`，commit `dfdf6b0`，§9，W20-004）+ **reclaim execution outcome 前缀**（`WorkingSetReclaimOutcome` 仅 issued-permit 读回，commit `c83c6c2`，§10，W21-004）+ **`max_task_nodes` admission gate 前缀**（net-new 注册 consult + replay bypass，commit `e6ec3ef`，§11，W22-004）PARTIAL PASS；**W28–W31 声明面全链落地**：nlos-plan 状态权威（G1）+ manifest 模板面（G6/PLAN-OVERRIDE-001）+ TaskSpec 关联字段/维度正规化 + resolver（G4）+ 物化门（G3）+ residency + 两层调度器 + 10K/100K 逻辑 TaskNode 惰性有界实测 + working-set 比例矩阵 + rehydrate 三档实测（[W31-G 评审](../evidence/stage-b/reviews/w31g-road-b004-gates.md)：G1–G6 全 SATISFIED-WITH-BOUNDARIES，边界=debug/test profile 单平台等具名项） |
...
>
> 滚动细化纪律：W27–W34 已于 2026-09-20 全量细化到车道级（议题 35 经 [ADR-0016](./adrs/0016-task-plan-declaration-surface.md) 定案后解除 W28+ 主题级占位）；每波屏障（定向门 + 全仓测试 + fmt/clippy 双 0 + 本表 §3/§5/§6 同步 + 三平台 CI/Pages）后允许重排后续波车道，但已登记车道的变更/删除必须在增量日志留痕。本节不改变 §5「唯一主线 `IN_PROGRESS` 工作包」约束与 §7 更新协议；实现完成仍以增量日志与 §3/§4 行为准，本节只承诺编排不承诺完成。
>
> 派发纪律（2026-09-20 补充，真依赖提前解锁）：车道派发按真依赖就绪集推进、不按波次批次——(1) 入度以 **VERIFIED**（定向门通过 + 审查 clean）计零，仅 completed 不触发解锁；(2) 写集锁：写集与任一 in-flight 车道相交即不可派，共享文件（含本进度单）仍按渐进式披露 §6.1 单一 integrator 合并、禁止并行双写；(3) 就绪集内按关键路径优先派发（最宽铺开不保证最短 makespan，Graham 异常）；(4) 跨波晋升：后续波车道若依赖全部满足（含已 ACCEPTED 的决策类信息依赖，如 ADR-0016）且写集空闲，可晋升进当前波执行，须在增量日志留痕；依赖未决结论（如 W27-B 评估）的车道不得提前；(5) 波次屏障保留为验证/记账/重规划点（全仓门 + 本表同步 + 重排后续车道），不因提前解锁豁免，晋升车道随当前波屏障一并验收；(6) §5「唯一主线 `IN_PROGRESS`」不变——晋升车道并入当前波记账，不开第二主线。本纪律与 dash T2 调度器同语义（入度归零 + 写集锁 → 可派发集，dash spec §11），项目层先行采用。
>
> 工具链波次（仓库级工具，如 dash 插件）使用 T 编号（T1 起，见 2026-09-20 第九十增量登记），不占用本节 W 车道号。
...
| # | 事项 | 来源 |
|---|---|---|
| B4-1 | ~~议题 35 用户决策~~ **已解除**（2026-09-20 定案 B+C 组合，[ADR-0016](./adrs/0016-task-plan-declaration-surface.md) ACCEPTED） | 议题 35 §9 / ADR-0016 |
...
```text
ADR-0016（议题 35 已定案）→ B4-2 声明面（W28-A 状态面 / W28-B 模板面）→ { B4-3 Resolver、B4-4 惰性物化、B4-5 residency、B4-9 TaskNode benchmark、B2-3/Slice K 关联下沉 }
...
### 6.5.3 波次编排（W27–W34 全量车道级）

每波固定屏障（[PD-ORCH-004]）：全部车道定向门通过 → 全仓 `cargo test --workspace --no-fail-fast` + fmt/clippy 双 0 → §3/§5/§6 与增量日志同步 → 三平台 CI/Pages 复验并回填 run 链接。同波写集冲突处理：共享 crate 车道由单一 integrator 串行合并（表内标注「串行位」），其余车道独立 worktree/CARGO_TARGET_DIR 并行。

**W27（定向波）**——六车道写集两两不相交，可并行：
...
| W27-B | B3-2 Resource/Operation prepare/finalize 入口评估 | docs/management（ADR 草案/评估记录，无代码写集） | 采纳或否决决策 + 理由；采纳则给出 W28-C 实现切片定义；三域样本是否需要台账泛化一并裁定 |
...
**W28（声明面启动 + TaskWriteSet 前置波）**——依 ADR-0016 与 W27-B 结论：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W28-A | nlos-plan authority 骨架（B4-2 状态面；ADR-0016 决定 2）：自有 SQLite、域分隔 PlanId/TaskNodeId 派生、`plan_revisions` immutable digest 链、`plan_nodes` durable metadata 与状态机迁移凭证 | 新 crate `nlos-plan` | G1 证伪测试（已执行节点 revision 不可改写）；重启 replay + 幂等键；kill-window 精简矩阵 |
| W28-B | manifest tasks 模板段（B4-2 模板面；决定 1）：package manifest additive 扩段 + 验签 golden + 模板→TaskPlan proposal 编译（同一 schema） | nlos-application、nlos-artifact | G6：旧签名包仍可验装（负路径硬门）；新段 additive golden；PLAN-OVERRIDE-001 编译等价 |
| W28-C | Resource 跨 authority prepare/finalize coordinator（B3-2 实现半边，依 W27-B 采纳；否决则本车道按评估结论重定义） | nlos-task、nlos-resource 接线（nlos-task 串行位 1） | 重启收敛唯一终态；kill-window 矩阵沿用 B-TASK-008C2G-RES-COMMIT 模式 |
| W28-D | ControlCommand pause/resume/cancel 变体 + typed Receipt（B5-1 前半） | nlos-system-control、nlos-schema（additive `frozen: false` 条目） | NL/CLI/direct receipt 逐字节相等；三语言生成同步 |
| W28-E | 自动 GC 触发（B1-2）：周期/阈值触发 pass + 独立幂等键 + 健康计数 | nlos-artifact | 触发幂等；孤儿零误删回归；可停用旁路 |
| W28-F | Windows 实机 platform kill CI 复验（B6-2） | .github workflow / 文档 | Windows runner kill 矩阵绿；run 链接回填台账 |

**W29（接线与维度正规化波）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W29-A | TaskSpec 最小关联字段 additive 迁移 + ScaleProfile 维度正规化（决定 3/4；G5 前半）：`application_id` + plan revision 引用；`max_task_nodes`→TaskNode 计数、Task 注册保留第二维度 | nlos-task schema（串行位 2，波内屏障 1 先行） | 引用落库；旧路径零回归；TASK_PROFILE_10K 口径切换显式注明 |
| W29-B | Dependency Resolver（B4-3；决定 5）：typed selector→带版本/generation handle、环检测 fail-closed、immutable 解析 receipt | nlos-plan | G4 证伪测试；receipt 重放幂等 |
| W29-C | Operation 跨 authority prepare→activate 接线（B3-2 Operation 半边） | nlos-task、nlos-store 接线（nlos-task 串行位 3，波内屏障 2） | 重启 replay；cancel/generation fence 保持 |
| W29-D | kill/throttle/reclaim 变体 + 执行接线（B5-1 后半 + B5-2）：kill→SupervisorPidRegistry/platform kill、throttle→ResourceDemand、reclaim→WorkingSetReclaim | nlos-system-control、nlos-process、nlos-resource（读接线） | 每变体 typed Receipt 且执行路径真达对应 authority |
| W29-E | PKG migration runner + health check + 原子切换（B1-3） | nlos-application | 同 major 迁移演练 + 回滚路径 + 健康检查 |
| W29-F | 第二 Process spawn→platform kill 全链（B2-1） | nlos-process、nlos-slice-k | spawn→运行→kill→binding 终态→重启 replay |
| W29-G | Go/C# golden 探针解封执行（X-3 前半，末位） | sdk/ 探针 | 双语言 Envelope 家族 golden byte-equal |

**W30（ROAD-B-003 收口波）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W30-A | complete TaskWriteSet 六域闭环验收（B3-3）：Effect/Artifact/Semantic/Resource/Operation/Channel 混合写集 seal→唯一 permit→finalize→统一 TaskCommitReceipt | nlos-task 验收测试 | 全域 e2e + 故障矩阵；replay 只信 Task 行 |
| W30-B | 最小 provider driver plane（B3-4 前半）：确定性 fake provider + 真实 IPC + durable operation 面接线 | 新 crate（driver-mock 前缀） | register/dispatch/complete 全链 + 重启 replay |
| W30-C | provider cache 降级 + 投机副作用 fence 测试（B3-4 后半，依 W30-B，波内串行链） | 同上 + 测试 | ROAD-B-003 六项最后两项语义门全绿 |
| W30-D | Slice K Task/Process teardown + NL 接线 + 关联下沉验证（B2-3；Slice K 缺口 1 关闭） | nlos-slice-k | uninstall→teardown 链；NL 控制真实走 Slice K 全链 |
| W30-E | 三平台复验 + ROAD-B-003 Evidence 落档（B3-5 收口） | docs/evidence、本表 | 三平台 + MSRV + Pages run 链接；§6 B-003 行更新 |

**W31（ROAD-B-004 收口波）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W31-A | 惰性物化门完整实现（B4-4；G3）：ELIGIBLE/WAITING/MATERIALIZING 状态推进 + verify-then-commit 物化请求 + 窗口收缩 checkpoint/evict | nlos-plan、nlos-task 接线（波内屏障 1 先行） | G3 证伪测试；物化门崩溃窗口故障矩阵 |
| W31-B | working-set 比例 benchmark（B4-7）：10K/100K × active 比例矩阵 | nlos-task benchmark | 比例矩阵数据落 evidence；admission 不退化 |
| W31-C | checkpoint/rehydrate benchmark + reclaim 实执行（B4-8） | nlos-runtime-tokio、nlos-task | rehydrate 实测数据；reclaim 执行闭环 |
| W31-D | 10K/100K logical TaskNode benchmark（B4-9；G2/G5 后半） | nlos-plan benchmark | 100K METADATA_ONLY 惰性有界（RSS/metadata 上界）；Task 注册近似退役注明 |
| W31-E | Context residency 分级最小版（B4-5）：METADATA/COLD/WARM/HOT/RUNNING + evict | nlos-plan | 分级读回 + evict 边界测试 |
| W31-F | 分层 Scheduler 最小版（B4-6）：Global/Worker 两层 + 物化窗口调度 | nlos-plan 或 runtime 侧 | 窗口收缩联动 W31-A；调度决策可 inspect |
| W31-G | ROAD-B-004 门证据评审（G1–G6 逐门 Evidence review） | docs/evidence、本表 | 六门各有 review 记录；§6 B-004 行更新 |

**W32（ROAD-B-005/002 GUI 波）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W32-A | Tauri 壳 + SystemControl IPC 接入 + inspect 读侧（B5-4 读半边） | 新 `desktop/`（Tauri） | 真实 IPC 走 authenticated 入口；inspect 数据与 CLI 一致 |
| W32-B | GUI 授权操作 + Receipt 展示（B5-4 写半边） | desktop/ | pause/cancel/kill/throttle/reclaim 真实下发 ControlCommand |
| W32-C | GUI↔NL↔CLI Receipt parity 钉死（B5-6） | 测试 | 三路径 Receipt 逐字节相等测试入库 |
| W32-D | 可信权限 UI 最小版（B5-5 前半）：授权/预算/成本可见 | desktop/ | 权限变更 UI 可见且与 authority 一致 |
| W32-E | Resource Monitor 最小版（B5-5 后半）：OpenMetrics 消费 + 展示 | desktop/ | 消费既有 metrics 面，无新控制路径 |
| W32-F | UI Surface 声明与最小呈现（B2-2） | nlos-application、desktop/ | Application 声明 surface→窗口呈现最小链 |
| W32-G | 各层 inspect 补齐（B5-3：TaskGroup/TaskNode、ExecutionFiber、Topic/Operation） | nlos-system-control | 每层 inspect Receipt 齐；§6 B-005 行更新。**DONE**（2026-09-21：五视图 SABI v1.5 additive + 四真实适配器 + NL/CLI 三路 parity，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W32-G） |

**W33（ROAD-B-001 生态波 + 最小服务）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W33-A | Package SDK/打包工具链（B1-4）：`nlos package build/verify` CLI + 开发者文档 | 新 tools/ 或 sdk 扩展 | 样板包可构建、可验签 |
| W33-B | 第三方样板 Application 全周期演示（B1-5）：非内核视角开发→签名→安装→运行→更新→卸载 | examples/ + evidence | ROAD-B-001 构造性证据；全程不触内核内部 API |
| W33-C | conformance kit for packages（X-3 中） | tools/ | 包结构/manifest/验签一致性检查器 |
| W33-D | Notification 最小服务（X-1 前半）：Topic/Channel/Wait 薄服务层 + 订阅面 | 新 crate `nlos-notify`（薄层） | 订阅/投递/ack 全部经既有 Topic 权威 |
| W33-E | Search 最小服务（X-1 后半）：语义原子/断言索引 + 只读查询面 | 新 crate `nlos-search` | 只读查询；零 semantic 权威写入 |
| W33-F | 最小 Task Space（X-2）：任务列表/详情只读视图 | desktop/ | 复用 Task Manager 读侧 |
| W33-G | 最小 debugger 调试面（X-3 后）：fiber snapshot/replay inspect CLI + 事件流回放 | tools/ | 快照检查/回放可跑 |
| W33-H | ROAD-B-001/002 门证据评审 | docs/evidence、本表 | 两门 review 记录；§6 行更新 |

**W34（退出评审波）**：

| 车道 | 事项 | 主要写集 | 验收门 |
|---|---|---|---|
| W34-A | 六门证据矩阵补漏与 Evidence review（B1–B6 全门） | docs/evidence | 每门 review 记录齐；无缺门 |
| W34-B | claims/risks/evidence 全量回填 + Claim≤Evidence lint 全绿（X-4 收口） | management yaml | lint 全绿；反例保留 |
| W34-C | 未知风险清单 + P0 清零确认 | management | 清单显式；P0 为零或经批准豁免 |
| W34-D | 阶段 B 退出评审与明确批准（唯一保留用户门）+ §6 状态翻转 + 移交阶段 C 项登记 | 本表 §6 | 6.5.5 五条件逐一满足并留痕 |

### 6.5.4 决策点状态（2026-09-20 更新）

1. **议题 35 TaskPlan 声明面：已解除。** 2026-09-20 用户授权按编排建议直接采纳讨论推荐倾向：B（独立 `nlos-plan` authority）为主 + C（manifest 模板段）组合，A 否决、D 收缩路径；六项参数定案见 [ADR-0016](./adrs/0016-task-plan-declaration-surface.md)（ACCEPTED）。B4-2 起（W28-A/W28-B）车道 READY。
2. **最小服务裁剪边界：已确认默认口径。** X-1 Notification=Topic 薄层、Search=只读查询面；X-2 Task Space=只读视图；X-3 debugger=snapshot/replay inspect CLI——按 W33 车道所列最小版执行；后续收紧/放宽在波屏障经增量日志留痕。
3. **保留用户门**：W34-D 阶段 B 退出评审与明确批准；以及波次执行中新出现的不可逆/跨模块决策（按[管理机制 §7](./README.md#7-评审与决策机制)晋升 ADR 后再进实现）。
...
13. TS/Python conformance 对 SABI v1.2–v1.5 新臂/新视图的 golden 钉死——**DONE**（2026-09-21，merge `5c6f4c7`，[B-SCHEMA-002 §6](../evidence/stage-b/b-schema-002-cross-language-generation.md) 收口证据；U-14 退役；与 stage-c §C.3.2 #13 对齐）；
