...
> 本轮（2026-08-19 第十一增量，远端待复验）推进 `B-TASK-008C2G-IPC-POWERLOSS`：在实际 `TakeoverControl` IPC handler 上通过 fault VFS 注入 `PowerLossAfter { remaining: 0 }` 静默丢写；handler 返回的 signed record 在关闭并重开 authority 后不可见，coverage 回到 `Partial`，同一 request/idempotency key 重做后恢复为单行 signed、`LocallyCovered` durable record。本地 TakeoverControl 10 项、workspace 448 项、feature clippy/fmt 已通过；三平台 + MSRV 与 Pages 待推送后回填。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.94。
...
| `B-TASK-008C2G-IPC-CONFORMANCE` | TakeoverControl TypeScript/Python payload conformance 与 Windows named-pipe handler round-trip | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md)：feature-gated handler、两语言生成 payload、verified Receipt/signer 回读与第二连接 durable replay；三平台 + MSRV run 32192662820 已过，Pages run 32192662841 成功 | 并发多 caller、IPC crash/fault matrix、真实 Capability authorizer、principal-level peer attestation、长期 SDK packaging 仍未实现 |
| `B-TASK-008C2G-IPC-CONCURRENCY` | TakeoverControl 多 caller 并发线性化 | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.92：两个独立 duplex caller 并行提交同一 signed observation，单 durable row、verified record 等值、coverage `LocallyCovered`；本地 7 项通过，三平台 + MSRV run 32194086430 已过，Pages run 32194086576 成功 | 真实 Unix/named-pipe 多连接并发、IPC crash/fault matrix、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
| `B-TASK-008C2G-IPC-FAULT-MAPPING` | TakeoverControl IPC 故障映射与同 key 恢复 | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.93：fault VFS `IOERR`/`ENOSPC` → typed `DURABILITY + RETRY_SAME_IDEMPOTENCY_KEY`，失败无 durable row，解除后同 key 恢复为 signed/`LocallyCovered` 单行；本地 9 项、workspace 447 项、feature clippy/fmt 通过；三平台 + MSRV run [32196354141](https://github.com/cty12356541/llmos/actions/runs/32196354141)、Schema fuzz smoke run [32196354132](https://github.com/cty12356541/llmos/actions/runs/32196354132)、Pages run [32196354259](https://github.com/cty12356541/llmos/actions/runs/32196354259) 成功 | 进程崩溃/PowerLoss/WAL 撕裂尾部、真实 Unix/named-pipe 多连接压力、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
| `B-TASK-008C2G-IPC-POWERLOSS` | TakeoverControl IPC 静默丢写恢复 | `IN_PROGRESS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.94：`PowerLossAfter { 0 }` 下 handler 返回的幻影 record 在 authority 重开后不可见，coverage 回到 `Partial`，同 key 重做恢复单行 signed/`LocallyCovered`；本地 10 项、workspace 448 项、feature clippy/fmt 通过，远端待复验 | 真实进程崩溃/断电、WAL 撕裂尾部、真实 Unix/named-pipe 多连接压力、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
...
- 当前状态为 `PARTIAL_PASS`：并发多 caller、IPC crash/fault matrix、真实 Capability
  authority、principal-level peer attestation 和 npm/PyPI 长期 SDK packaging 仍是明确
  后续项。
...
- 本地 `cargo test -p nlos-takeover-control --features conformance-server` 10 项、`cargo test --workspace --quiet` 448 项（另 2 项既有 scale probe ignored）、feature clippy/fmt 与 diff 检查已通过；三平台 + MSRV 与 Pages 待本增量推送后回填。该模型不外推为真实进程 kill、断电或 WAL 撕裂尾部恢复。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC `PowerLossAfter` 静默丢写与同 key 恢复 IN_PROGRESS（B-TASK-008C2G-IPC-POWERLOSS，本地 10 项通过，远端待复验）
```
