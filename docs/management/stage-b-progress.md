...
- 真实 SABI server 接线与 server restart 组合已由 4.19/B-SCHEMA-011 补齐并通过三平台，deadline/cancel/uncertain 初始状态机由 4.20/B-SCHEMA-012 推进并通过三平台。通用 Operation control、完整 fault matrix、Receipt body 和 retention/GC 仍未完成，因此不升级 SDK 等级。
