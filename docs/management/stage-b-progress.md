...
>
> 当前进度同步（2026-08-20）：第十四增量已在 TypeScript/Python conformance 本地完成真实 IPC WAL 撕裂尾部截断、进程 kill、清理 `-shm` 后重启恢复与同 key 逐字节 replay；三平台 + MSRV 与 Pages 运行待本增量提交后回填。第十三增量已在 TypeScript/Python conformance 本地完成 8 个真实 Unix/named-pipe 并发 caller 的单 row/逐字节 durable record 证明，三平台 + MSRV run 32235500125、Pages run 32235500056 均成功；第十二增量已完成受控 kill-after-commit/restart 同 key 恢复与逐字节 replay，三平台 + MSRV run 32229735359、Pages run 32229735358 均成功；第十一增量已完成 IPC `PowerLossAfter` 静默丢写模型与同 key 恢复，三平台 + MSRV run 32197845869、Pages run 32197845859 均成功；第十增量已完成 IPC `IOERR`/`ENOSPC` typed durability 映射与同 key 恢复，三平台 + MSRV run 32196354141、Schema fuzz smoke run 32196354132、Pages run 32196354259 均成功；第九增量已完成 TakeoverControl 两 caller 并发线性化与单 row 证明，三平台 + MSRV run 32194086430 已通过，Pages run 32194086576 成功；第八增量的 TypeScript/Python payload/response conformance、双连接 replay 与 Windows named-pipe 已由 run 32192662820 三平台通过。
>
> 最后更新（当前检查点）：2026-08-20；旧的“最后更新：2026-08-19”行保留为上一检查点历史摘要。
...
| `B-TASK-008C2G-IPC-CONFORMANCE` | TakeoverControl TypeScript/Python payload conformance 与 Windows named-pipe handler round-trip | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md)：feature-gated handler、两语言生成 payload、verified Receipt/signer 回读与第二连接 durable replay；三平台 + MSRV run 32192662820 已过，Pages run 32192662841 成功 | 并发、fault、受控 crash/restart 已由后续独立行拆出；长时/高数量连接压力、真实 Capability authorizer、principal-level peer attestation、长期 SDK packaging 仍未实现 |
| `B-TASK-008C2G-IPC-CONCURRENCY` | TakeoverControl 多 caller 并发线性化 | `PARTIAL_PASS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.92：两个独立 duplex caller 并行提交同一 signed observation，单 durable row、verified record 等值、coverage `LocallyCovered`；本地 7 项通过，三平台 + MSRV run 32194086430 已过，Pages run 32194086576 成功 | 更高数量/长时真实端点压力、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
...
- 本地 `cargo fmt --all -- --check`、conformance server 编译、schema typecheck、两语言 TakeoverControl conformance（含 TS/Python WAL 场景）与 Python 编译检查已通过；远端三平台 + MSRV、Pages 运行待本增量原子提交后回填。该证据是可重复的 test-only WAL frame 撕裂模型，不外推为真实断电、随机信号崩溃、长时/高数量连接 soak、时间窗 anti-replay 或真实 Capability/principal attestation。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC WAL 撕裂尾部恢复 PARTIAL PASS（B-TASK-008C2G-IPC-WAL-TAIL，本地 TS/Python conformance 已过，远端待提交回填）
```
