...
>
> 当前进度同步（2026-08-23）：第十九增量外部 supervisor TS/Python 各 8 trials 在本地 macOS 通过；第二十增量 Semantic coordinator 故障矩阵 9 项、既有 coordinator 4 项、Semantic crate、workspace、Clippy/fmt 全部通过；第二十一增量补上 SystemControl bounded `SabiFailure`/脱敏 envelope 与 Windows named-pipe 成功/重放测试，本地 `nlos-system-control` 11 项和 workspace 回归通过，三平台 + MSRV Rust CI [32588868965](https://github.com/cty12356541/llmos/actions/runs/32588868965) 与 Pages [32588868943](https://github.com/cty12356541/llmos/actions/runs/32588868943) 成功。所有这些切片仍保持 `PARTIAL_PASS`，不改变真实 Capability authority、Principal peer attestation、trusted-clock anti-replay、真实断电或完整跨 authority TaskWriteSet 的未决状态。
...
| `B-TASK-006L` | SystemControl recovery handler、bounded rejection 与 Windows local IPC | `PARTIAL_PASS` | [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)：TaskAuthority/ServiceDirectory/Unix handler、bounded `SabiFailure` mapping + sanitized envelope、Windows named-pipe success/replay test；三平台 + MSRV Rust CI [32588868965](https://github.com/cty12356541/llmos/actions/runs/32588868965)、Pages [32588868943](https://github.com/cty12356541/llmos/actions/runs/32588868943) 已成功 | 真实 Capability authority、Principal-level peer attestation、trusted-clock anti-replay、transport caller 全量接入 failure envelope、metrics exporter、多入口 parity |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → bounded rejection mapping + Windows named-pipe success/replay                    PARTIAL PASS（B-TASK-006L；三平台 + MSRV run 32588868965、Pages 32588868943 已过）
