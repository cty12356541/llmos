...
>
> 当前进度同步（2026-08-19）：第八增量已在本地接通 TakeoverControl 的 TypeScript/Python payload/response conformance 与双连接 replay；Windows named-pipe 远程 CI 正在验证，未将本地结果冒充三平台完成。
>
...
| `B-TASK-008C2G-IPC-CONFORMANCE` | TakeoverControl TypeScript/Python payload conformance 与 Windows named-pipe handler round-trip | `IN_PROGRESS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md)：feature-gated handler、两语言生成 payload、verified Receipt/signer 回读与第二连接 durable replay；本地 macOS Unix socket 已过 | 三平台 CI、Windows named-pipe 实跑结果待回填；并发多 caller、IPC crash/fault matrix、长期 SDK packaging 仍未实现 |
...
- 详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md)；增量已补上 feature-gated TypeScript/Python payload conformance 与 Windows named-pipe handler round-trip 入口，仍待三平台 CI 回填；限制还包括无时间窗防重放（trusted clock 独立 authority）、ExactPeerAuthorizer 为 OS 凭证非 principal attestation、CorruptRecord 串映射与 store 静态消息结构耦合、并发竞争与 IPC 层崩溃注入。
...
- 本地 macOS 已通过 `npm run schema:lint`、`schema:check-generated`、`schema:typecheck`、
  ...
  与 fmt；workflow 已把两条新命令接入 Ubuntu/macOS/Windows job。
- 当前状态为 `IN_PROGRESS`：远程三平台 CI 尚未回填；并发多 caller、IPC crash/fault
  matrix、真实 Capability authority、principal-level peer attestation 和 npm/PyPI
  长期 SDK packaging 仍是明确后续项。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TS/Python conformance + Windows named pipe round-trip IN_PROGRESS（B-TASK-008C2G-IPC-CONFORMANCE，本地通过，待三平台 CI）
```
