...
> 本轮（2026-08-23 第二十一增量）推进 `B-TASK-006L` 的横向控制面缺口：`SystemControlError` 新增有界 `SabiFailure` 分类与脱敏 `failure_envelope`，覆盖契约、期限、权限、冲突、状态、durability、driver、not-found、fenced 和 unsupported；新增映射/重试安全/相关性保留测试。另新增真实 Windows named-pipe `RecoverySystemControl` 成功提交 + durable Receipt replay 测试（macOS 仅验证非 Windows 构建，Windows 编译/执行待远端 CI）。本地 `nlos-system-control` 11 项、workspace、Clippy/fmt 已通过；该增量仍不声称 transport-neutral handler 已自动接入每个 IPC server，也不等同真实 Capability authority、Principal peer attestation 或 trusted-clock anti-replay。详见 [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)。
...
>
> 当前进度同步（2026-08-23）：第十九增量外部 supervisor TS/Python 各 8 trials 在本地 macOS 通过；第二十增量 Semantic coordinator 故障矩阵 9 项、既有 coordinator 4 项、Semantic crate、workspace、Clippy/fmt 全部通过；第二十一增量补上 SystemControl bounded `SabiFailure`/脱敏 envelope 与 Windows named-pipe 成功/重放测试，本地 `nlos-system-control` 11 项和 workspace 回归通过。第二十一增量的 Windows 编译/执行尚待远端 CI；所有这些切片仍保持 `PARTIAL_PASS`，不改变真实 Capability authority、Principal peer attestation、trusted-clock anti-replay、真实断电或完整跨 authority TaskWriteSet 的未决状态。
...
| `B-TASK-006L` | SystemControl recovery handler、bounded rejection 与 Windows local IPC | `PARTIAL_PASS` | [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)：TaskAuthority/ServiceDirectory/Unix handler、bounded `SabiFailure` mapping + sanitized envelope、Windows named-pipe success/replay test（Windows CI 待复验） | 真实 Capability authority、Principal-level peer attestation、trusted-clock anti-replay、transport caller 全量接入 failure envelope、metrics exporter、多入口 parity、三平台复验 |
...
- 新增真实 Windows named-pipe `RecoverySystemControl` 成功提交与第二连接 durable Receipt replay 测试；macOS 仅验证非 Windows 构建，Windows 编译/执行待远端 CI。
- 既有 5 项 integration tests 继续覆盖拒绝路径、framed mutation/replay、ServiceDirectory negotiate 和真实 Unix endpoint typed get，详见 [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)。限制：transport-neutral `handle` 仍需由各 IPC caller 显式使用 `failure_envelope`，真实 Capability/peer authority、Principal attestation、trusted-clock anti-replay、外部 metrics exporter、多入口 parity 与三平台 CI 尚未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → bounded rejection mapping + Windows named-pipe success/replay                    PARTIAL PASS（B-TASK-006L；Windows CI 待复验）
