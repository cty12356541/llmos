...
> 本轮（2026-08-23 第二十八增量）补强 `B-TASK-008C2G-COORD` 的 mixed Effect + Semantic restart convergence：新增 5 项集成测试，持久化 mixed finalize envelope，完成 Semantic owner publication 与 optional Effect close，重开 TaskAuthority 后仅由公开 `converge_pending` 收敛为单条 TaskCommitReceipt/`FINALIZED`；该证据仍是单机 bounded recovery，远端 CI 待本批提交后回填。详见 [B-TASK-008C2G-COORD](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md) §5.3。

> 本轮（2026-08-23 第二十七增量）补强 `B-SCHEMA-014` 的跨语言 recovery contract：为 `ArtifactRecoveryOperationsSnapshot` 增加固定 deterministic protobuf golden，Python/TypeScript bytes 逐字节一致并覆盖 optional retry/ack、enum、failure authority/id 与 alerts 截断；远端 CI 待本批提交后回填。详见 [B-SCHEMA-014](../evidence/stage-b/b-schema-014-system-control-recovery-contract.md)。

> 本轮（2026-08-23 第二十六增量）补强 `B-TASK-006M`：`FlappingHealth` 验证单次 metrics export 使用同一 health snapshot，lifecycle/counter/gauge sink failure matrix 验证首错短路；metrics 专属测试现为 3 项（本轮新增 2 项），远端 CI 待本批提交后回填。详见 [B-TASK-006M](../evidence/stage-b/b-task-006m-recovery-metrics-export.md)。

> 本轮（2026-08-23 第二十五增量）补强 `B-TASK-008C2G-IPC`：新增 `TakeoverControl::handle_for_ipc`，普通 duplex 与 Unix 拒绝 caller 统一 typed failure envelope；crash-hook conformance server 保留 outcome-bearing `Result` 分支，未声称全仓零手工包装。远端 CI 待本批提交后回填。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §5.4。
...
> 当前进度同步（2026-08-23 后续四项）：第二十五增量的 `TakeoverControl::handle_for_ipc` 已接入普通 duplex/Unix 拒绝 caller；第二十六增量以 single health snapshot 与 lifecycle/counter/gauge failure matrix 将 metrics 专属测试补至 3 项；第二十七增量补上 SystemControl recovery metrics 的 Python/TypeScript deterministic golden；第二十八增量补上 mixed Effect + Semantic `converge_pending` restart convergence。四项本地门均通过，当前 `HEAD` 为 `cc2d59a`、`origin/main` 为 `68c1a86`，远端 CI 待本批提交后回填；仍不改变真实 Capability/Principal/trusted-clock 与完整 TaskWriteSet 的未决状态。
...
- feature-gated conformance server 仍使用 `handle` 的 `Result`，因为 crash-hook 需要成功提交结果来设置 `should_hold`；这是测试 outcome 边界，不声称所有手工包装均已消除。本地 TakeoverControl 10 项、feature check/clippy/fmt 通过，远端 CI 待本批提交后回填。
...
- lifecycle/counter/gauge sink failure matrix 在首个失败样本停止并返回 typed backend error，后续 sink 不再调用；metrics 专属测试共 3 项（本轮新增 2），远端 CI 待本批提交后回填。
...
- 该项只补强 schema/encoding conformance，不外推 concrete metrics exporter；远端 CI 待本批提交后回填。
...
- 仅调用公开 `converge_pending(1, 11)` 即恢复单条 Semantic publication、单条 TaskCommitReceipt、`FINALIZED` 与空 incomplete scan；目标测试二进制 5 项、targeted Clippy/fmt/diff 通过，仍是单机 bounded evidence，远端 CI 待本批提交后回填。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl `handle_for_ipc` typed adapter + ordinary caller migration PARTIAL PASS（B-TASK-008C2G-IPC-TYPED-ADAPTER，本地 10 项；conformance crash-hook server 保留 outcome-bearing Result，远端待回填）
  → Recovery metrics single-snapshot + sink failure matrix PARTIAL PASS（B-TASK-006M-METRICS-SNAPSHOT，metrics 专属 3 项、本轮新增 2；远端待回填）
  → SystemControl recovery metrics cross-language deterministic golden PARTIAL PASS（B-SCHEMA-014-METRICS-GOLDEN，Python/TypeScript fixed bytes；远端待回填）
  → Mixed Effect + Semantic pending restart convergence PARTIAL PASS（B-TASK-008C2G-COORD-SEM-MIXED-RESTART，本地 5 项；远端待回填）
```
