...
- 详见 [B-TASK-008C2G-FAULT](../evidence/stage-b/b-task-008c2g-takeover-fault-matrix.md)；限制：kill-9 ≠ 真实断电、macOS 本地、F4 全集（checkpoint/backup/migration 变体）未覆盖、v28/v29 lease-binding 列未逐列注入、三平台 CI 待运行。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Lease/takeover 表组 F1–F4 故障注入矩阵（kill-9 中断/commit 后崩溃/IoErr/ENOSPC/静默丢写/撕裂尾部/解除后继续） PARTIAL PASS（B-TASK-008C2G-FAULT，7 测试，macOS 本地）
