...
| `B-PARTICIPANT` | TaskAuthority participant registry generation/root/freeze 与 endpoint proof | `IN_PROGRESS` | [B-TASK-007A](../evidence/stage-b/b-task-007a-self-participant-registry.md)：authority-assigned TaskStore self participant、versioned root、permit-time atomic freeze；[B-TASK-007B1](../evidence/stage-b/b-task-007b1-authority-endpoint-proofs.md)：Artifact/Semantic durable endpoint proofs PARTIAL PASS | Task registration CAS、Driver/Resource/Channel endpoints、EffectPermit/Receipt binding、takeover fence、fault matrix |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TaskAuthority verified participant registration CAS                                    NEXT（B-TASK-007B2）
