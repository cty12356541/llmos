...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → 签名写路径 F1–F6 故障注入矩阵 + term-3 反重放不可构造性 PARTIAL PASS（B-TASK-008C2G-BARRIER-SIG 增量，7 测试，本地全绿；三平台复验随下次 push）
  → Cross-process IPC transport/防重放 + cross-term adoption NEXT（B-TASK-008C2G）
```
