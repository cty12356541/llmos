...
- 仍为 `PARTIAL_PASS`：TaskAuthority consumer 已由 `B-TASK-008C2G-SEM` 接线，单 authority lease 已覆盖 mixed、Semantic-only high-level 与 same-term adoption/reconcile terminal path；跨 authority prepare/consume/recovery、Trust View/vector checkpoint、NLOS principal/attestation、跨 term adoption 与多 Cell 仍未完成。
