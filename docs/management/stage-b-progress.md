...
| `B-SANDBOX` | Wasmtime/WASI 与独立 host Process 隔离对比 | `PARTIAL_PASS` | [B-SANDBOX-001](../evidence/stage-b/b-sandbox-001-isolation-comparison.md)：exp8 独立实验（wasmtime+wasi 手写 WAT guest + host stress 子进程，nix 零 unsafe）四维实测：fuel 指令级精确 vs rlimit 秒级粒度 vs kill 轮询延迟；wasmtime 页粒度内存拒绝 vs macOS RLIMIT_AS 不强制（实测）；WASI default-deny capability 面收敛 vs 进程全环境继承；trap 恢复 vs 子进程崩溃隔离 | 结论为 candidate（建议后续 ADR 定双层隔离）；单平台 macOS、Linux rlimit 行为引用未实测、Component Model 未覆盖 |
