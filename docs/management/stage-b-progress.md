...
| `B-RUNTIME` | RuntimeAdapter 与 Tokio 有界 Fiber runtime | `PARTIAL_PASS` | [ADR-0001](./adrs/0001-stage-b-core-language-and-runtime.md)、[PoC-0001](../evidence/stage-b/poc-0001-tokio-fiber-runtime.md)、[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)：100K fiber、cancel/late-callback、structured join/detach、Activation meter（commit `c3b2a10`）、阻塞 I/O 负向（§6.7，`9b262f3`）、**`backpressure_wait`/`suspended` 生命周期**（§6.8，commit `2332ce7`）、**Activation meter 规模探针骨架**（§6.9，`24001d0`）+ **探针 race-free 修复**（commit `8d98b78`）+ **10K ignore 探针实跑登记**（§6.9.3，commit `a82cd5b`，100K tier 未跑）PARTIAL PASS；[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) process crash propagation + runtime terminal 门 + **platform kill 合同层**（process `89b14a5`）PARTIAL PASS | wake latency/fairness、100K activation-meter 探针 `--include-ignored` 实跑、背压/挂起维 100K 规模验证、runtime 侧 fiber 批量 cancel 联动、真实 OS platform kill、跨平台 |
...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall/rollback 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)–[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md)）；运行中 Task activity-gate 拒绝 fresh uninstall/rollback（commit `e709753`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §W16-001）PARTIAL PASS；Slice K uninstall（commit `fc5d7f1`）+ **uninstall 后显式 orphan GC**（commit `daafa75`，§W17-001）PARTIAL PASS；**`[PKG-UPDATE-001]` compatibility window 最小前缀**（commit `69aab73`，[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md) §7）PARTIAL PASS；自动 GC 触发/真实 Task 探针接线与 migration runner/health check/原子切换未做 |
