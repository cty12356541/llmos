...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Takeover 完成与 successor assignment 激活（schema v37 整表拷贝 + 窄化 immutable trigger + complete_authority_takeover + 全签名门） PARTIAL PASS（B-TASK-008C2G-COMPLETION，9 测试，本地全绿；三平台随本次 push）
  → Successor term registry 重开/新 permit + cross-term adoption + TS/Python conformance + Windows named pipe round-trip NEXT（B-TASK-008C2G）
```
