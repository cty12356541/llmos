...
> 本轮（2026-08-21 第十五增量）推进 `B-TASK-008C2G-IPC-HIGH-CONNECTION-SOAK`：测试专用 server 支持 1–8 轮批次；TypeScript/Python 各启动 32 个真实 Unix/named-pipe 连接，保持连接后并发提交同一 idempotency key 的不同 caller/context，连续 4 轮共 128 个会话，所有 durable record 逐字节一致且最终单行 `LocallyCovered`。本地 Rust feature/workspace、schema typecheck、两语言 conformance、Clippy/fmt 已通过；三平台 + MSRV 与 Pages 运行待本增量提交后回填。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.98。
...
>
> 当前进度同步（2026-08-21）：第十五增量已在 TypeScript/Python conformance 本地完成 32 个真实 Unix/named-pipe 连接 × 4 轮（每语言 128 个会话）bounded soak；三平台 + MSRV 与 Pages 运行待本增量提交后回填。第十四增量已在 TypeScript/Python conformance 本地完成真实 IPC WAL 撕裂尾部截断、进程 kill、清理 `-shm` 后重启恢复与同 key 逐字节 replay；三平台 + MSRV run 32374724436、Pages run 32374724524 均成功。第十三增量已在 TypeScript/Python conformance 本地完成 8 个真实 Unix/named-pipe 并发 caller 的单 row/逐字节 durable record 证明，三平台 + MSRV run 32235500125、Pages run 32235500056 均成功；第十二增量已完成受控 kill-after-commit/restart 同 key 恢复与逐字节 replay，三平台 + MSRV run 32229735359、Pages run 32229735358 均成功；第十一增量已完成 IPC `PowerLossAfter` 静默丢写模型与同 key 恢复，三平台 + MSRV run 32197845869、Pages run 32197845859 均成功；第十增量已完成 IPC `IOERR`/`ENOSPC` typed durability 映射与同 key 恢复，三平台 + MSRV run 32196354141、Schema fuzz smoke run 32196354132、Pages run 32196354259 均成功；第九增量已完成 TakeoverControl 两 caller 并发线性化与单 row 证明，三平台 + MSRV run 32194086430 已通过，Pages run 32194086576 成功；第八增量的 TypeScript/Python payload/response conformance、双连接 replay 与 Windows named-pipe 已由 run 32192662820 三平台通过。
>
> 最后更新（当前检查点）：2026-08-21；旧的“最后更新：2026-08-20”行保留为上一检查点历史摘要。
...
- 本地 conformance server 编译、Rust feature 10 项、workspace 448 项（另 2 项既有 scale probe ignored）、schema typecheck、TS/Python conformance、Clippy/fmt 已通过；远端三平台 + MSRV、Pages 运行待本增量原子提交后回填。该证据是 bounded high-connection soak，不外推为分钟级/长时运行、真实断电、随机信号崩溃矩阵、时间窗 anti-replay 或真实 Capability/principal attestation。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC 32 连接 × 4 轮 bounded soak PARTIAL PASS（B-TASK-008C2G-IPC-HIGH-CONNECTION-SOAK，本地 TS/Python conformance 已过，远端待提交回填）
```
