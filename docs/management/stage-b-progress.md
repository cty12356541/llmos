...
>
> 最后更新：2026-08-16（已纳入 `B-TASK-008C2G-SEM` Semantic publication consumer、schema v25、nested receipt、本地混合 Effect + Semantic v3 终结 hook、slot/Receipt-bound success proof，以及 `B-TASK-008C2G-COORD` Semantic-only 跨 authority restart coordinator 和 schema v27 单 authority durable lease/term/fencing 原语；此前的 TaskWriteSet/authority 增量保持有效）
>
...
| `B-TASK-008C2G-COORD` | Semantic publication cross-authority restart coordinator | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md) | schema v26 mixed finalize envelope、单机 bounded convergence、本地 slot/Receipt-bound proof binding，以及 schema v27 单 authority durable lease/term/fencing primitive 已接线；IPC peer auth、lease-to-permit binding/adoption、完整故障矩阵、外部 provider proof/attestation、Trust View/vector checkpoint、多 Cell 与完整 TaskWriteSet 仍未实现 |
...
- TaskAuthority 写失败后的 owner publication exact replay 与 prefix 收敛已有 SQLite abort/VFS 局部证据；schema v26 又持久化 mixed finalize envelope，使 coordinator 可在重启后重建 v3 request；EffectClosedSuccess 也已有本地 slot/Receipt-bound proof binding；schema v27 增加单 authority durable lease/term/fencing primitive，并以测试证明续租、过期接管、旧 token 拒绝和重启回读。详见 [B-TASK-008C2G-COORD](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md)。仍为 `PARTIAL_PASS`：跨进程 auth、lease-to-permit binding/adoption、完整 kill-9/ENOSPC/torn-write 矩阵、外部 provider proof/attestation、Trust View/vector checkpoint、多 Cell 与完整 TaskWriteSet 仍未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Cross-process auth + lease-to-permit adoption/fault matrix + external proof          NEXT（B-TASK-008C2G）
```
