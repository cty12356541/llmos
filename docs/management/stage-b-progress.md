...
| `B-SLICE-K` | Slice K：Package → Application → Task → Fiber → Operation → Receipt → 控制 | `PARTIAL_PASS` | [B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md)：`crates/nlos-slice-k` 全链贯通 + lifecycle disable/uninstall 演示（09b/09c，commit `fc5d7f1`）+ 竞争场景（`3500c9c`）；9 集成测试 + demo EXIT=0 | 单机单进程演示级；TaskSpec 无自由字段致 application↔task 关联在 slice 层；NL 路径未接；Task/Process teardown 与 rollback/GC 未做 |
...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall/rollback 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)–[B-APPLICATION-004](../evidence/stage-b/b-application-004-rollback.md)）；运行中 Task activity-gate 拒绝 fresh uninstall/rollback（commit `e709753`，[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md) §W16-001）PARTIAL PASS；Slice K uninstall（commit `fc5d7f1`）；GC/真实 Task 探针接线与完整 `[PKG-UPDATE-001]` 未做 |
| `ROAD-B-002` Application 多 Process、后台 Task、UI Surface | 局部推进：后台 Task durable 登记最小前缀（schema v5、`register_background_task`/`inspect_background_tasks`，commit `7225313`，[B-APPLICATION-005](../evidence/stage-b/b-application-005-background-task-registration.md)）PARTIAL PASS；多 Process binding / UI Surface 未做 |
...
| `ROAD-B-004` 10K/100K logical TaskNode、working-set、pressure/reclaim、rehydrate | 局部推进：ScaleProfile 骨架 + `TASK_PROFILE_10K` 实跑（commit `0bb6095`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md)）+ `TASK_PROFILE_100K` 常量与 `#[ignore]` 100K 注册探针（commit `1affa6b`）PARTIAL PASS；TaskPlan/TaskNode 声明面、working-set 比例 benchmark、pressure/reclaim/rehydrate 未做；waiting Fiber 不能替代 TaskNode benchmark |
| `ROAD-B-005` Task Manager 多层手动控制与 NL/GUI/CLI 同路 | 局部推进：NL 四命令同义词 + ExportMetrics + **InspectProcess** NL/CLI（commit `e40de48`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W16-005）已有单节点 H3；Trusted GUI 与多层手动调度未做 |
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach、process crash 合同层 + runtime terminal 门（`a69e709`）、Activation meter（`c3b2a10`）、阻塞 I/O 负向（`9b262f3`）、**批量 cancel 传播**（process `520cfce`）、**`backpressure_wait`/`suspended` 生命周期**（runtime `2332ce7`，§6.8）已有单节点 H3；100K 探针下分维 metering 规模验证、platform kill、runtime 侧 fiber 批量 cancel 联动未做；仍为 `PARTIAL_PASS` |
