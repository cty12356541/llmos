...
> 本轮（2026-08-23 第二十四增量）补强 `B-TASK-008C2G-COORD` 的 Semantic pending restart scan：新增集成测试，先持久化 owner publication 前缀并模拟 TaskAuthority 重启，再由公开 `converge_pending` 仅凭 durable plan/expectation 完成 owner replay、Task receipt 消费和终态 finalize。5 项测试、targeted Clippy/fmt 已通过；仍是单机双 authority bounded evidence，不外推完整 TaskWriteSet 或跨 authority 原子提交。详见 [B-TASK-008C2G-COORD](../evidence/stage-b/b-task-008c2g-semantic-coordinator.md)。

> 本轮（2026-08-23 第二十三增量）补强 `B-TASK-006M`：新增 backend-neutral metrics catalog 顺序/名称契约测试，并验证 sink 首次失败后立即短路；使用真实临时 `SqliteTaskAuthority`，不引入平台 exporter 或诊断文本。2 项新增测试、`nlos-system-control` crate、fmt/Clippy 已通过，三平台 CI 待本次提交后复验。详见 [B-TASK-006M](../evidence/stage-b/b-task-006m-recovery-metrics-export.md)。
...
>
> 当前进度同步（2026-08-23）：第十九增量外部 supervisor TS/Python 各 8 trials 在本地 macOS 通过；第二十增量 Semantic coordinator 故障矩阵 9 项、既有 coordinator 4 项、Semantic crate、workspace、Clippy/fmt 全部通过；第二十一增量补上 SystemControl bounded `SabiFailure`/脱敏 envelope 与 Windows named-pipe 成功/重放测试；第二十二增量补上 `handle_for_ipc`、common SABI 三语言终止失败语义和真实 Unix denied-response；第二十三增量补上 Recovery metrics typed catalog/失败短路契约；第二十四增量补上 Semantic pending restart scan 与 `converge_pending` durable-prefix 收敛。Rust workspace、三语言 schema conformance、Clippy/fmt、metrics targeted tests 与 coordinator targeted tests 均通过；21st 的三平台 + MSRV Rust CI [32588868965](https://github.com/cty12356541/llmos/actions/runs/32588868965) 与 Pages [32588868943](https://github.com/cty12356541/llmos/actions/runs/32588868943) 成功，22nd–24th 远端复验待本次提交。所有这些切片仍保持 `PARTIAL_PASS`，不改变真实 Capability authority、Principal peer attestation、trusted-clock anti-replay、真实断电或完整跨 authority TaskWriteSet 的未决状态。
...
| `B-TASK-006L` | SystemControl recovery handler、bounded rejection 与 Windows local IPC | `PARTIAL_PASS` | [B-TASK-006L](../evidence/stage-b/b-task-006l-system-control-recovery-handler.md)：TaskAuthority/ServiceDirectory/Unix handler、`handle_for_ipc` typed rejection、Unix denied-response evidence、Windows named-pipe success/replay；三平台 + MSRV Rust CI [32588868965](https://github.com/cty12356541/llmos/actions/runs/32588868965)、Pages [32588868943](https://github.com/cty12356541/llmos/actions/runs/32588868943) 已成功 | 真实 Capability authority、Principal-level peer attestation、trusted-clock anti-replay、metrics exporter、多入口 parity、全量生产 IPC caller 迁移 |
| `B-TASK-006M` | Recovery metrics export contract | `PARTIAL_PASS` | [B-TASK-006M](../evidence/stage-b/b-task-006m-recovery-metrics-export.md)：typed catalog 顺序/稳定名称、live authority snapshot 与 sink failure 短路；本地 2 项新增测试通过，三平台 CI 待复验 | 具体 OpenMetrics/ETW/signpost adapter、scrape auth、retention/alert rules、三平台与生产 metrics sink |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Semantic pending restart scan PARTIAL PASS（B-TASK-008C2G-COORD-SEM-RESTART-SCAN，本地 5 项，远端复验待本次提交）
```
