...
| `B-RESOURCE-006` | Resource cost-receipt 聚合回读 | `PARTIAL_PASS` | [Evidence](../evidence/stage-b/b-resource-006-cost-receipt-aggregate.md)：owner-derived 只读聚合 + finalize kill-window 故障矩阵（IOERR/ENOSPC/PowerLoss/replay storm，commit `0eac704`，零 src 改动）；`nlos-resource` 29 passed / 0 failed，clippy/fmt 干净 | TaskAuthority 消费接线、endpoint 签名 effect proof、真实掉电仍未实现；owner-side only，不包含 Task 侧桥接 |
