...
> 本轮（2026-09-07 第七十九增量）波次 18 编排 resume 无进展中止、收尾员串行验收：六车道中仅 [W18-R] B-RESOURCE cost-receipt 零 consumption 闭合边界 canonical 落地（commit `8b32d82`，[B-RESOURCE-006](../evidence/stage-b/b-resource-006-cost-receipt-aggregate.md) §6）；`cargo test -p nlos-resource` → 30 passed / 0 failed；push `8b32d82`→`origin/main`。补记波次 17 遗漏：[W17-006] 探针 race-free 修复（commit `8d98b78`，[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md) §6.9 追加）。**未提交中止候选**（定向测试红/编译失败，不得冒充 DONE）：[W18-001] PKG-UPDATE compatibility window（`nlos-application` 编译失败：`pack_package_version` const `u64::from`）；[W18-002] slice-k inspect 接线（依赖 W18-001）；[W18-004] working-set admission gate（`working_set_admission_replay_bypasses_gate_at_cap` 1 failed）；[W18-005] NL 同义词扩展（`english_export_metrics_forms_parse` 1 failed）；[W18-006] 10K 探针实跑（仅 evidence/探针脏改、10K `--include-ignored` 未跑）。议题 35 未启动；CI/workspace 全仓门未在本增量复跑。
...
| `B-RUNTIME` | RuntimeAdapter 与 Tokio 有界 Fiber runtime | `PARTIAL_PASS` | [ADR-0001](./adrs/0001-stage-b-core-language-and-runtime.md)、[PoC-0001](../evidence/stage-b/poc-0001-tokio-fiber-runtime.md)、[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)：100K fiber、cancel/late-callback、structured join/detach、Activation meter（commit `c3b2a10`）、阻塞 I/O 负向（§6.7，`9b262f3`）、**`backpressure_wait`/`suspended` 生命周期**（§6.8，commit `2332ce7`）、**Activation meter 规模探针骨架**（§6.9，`24001d0`，`#[ignore]` 10K/100K 未实跑）+ **探针 race-free 修复**（commit `8d98b78`）PARTIAL PASS；[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md) process crash propagation + runtime terminal 门 + **platform kill 合同层**（process `89b14a5`）PARTIAL PASS | wake latency/fairness、100K 探针下分维 metering `--include-ignored` 实跑、背压/挂起维 100K 规模验证、runtime 侧 fiber 批量 cancel 联动、真实 OS platform kill、跨平台 |
...
| `B-SLICE-K` | Slice K：Package → Application → Task → Fiber → Operation → Receipt → 控制 | `PARTIAL_PASS` | [B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md)：`crates/nlos-slice-k` 全链贯通 + lifecycle disable/uninstall 演示（09b/09c，commit `fc5d7f1`）+ 竞争场景（`3500c9c`）+ **uninstall 后显式 orphan GC**（commit `daafa75`，§10，10 集成测试）+ demo EXIT=0 | 单机单进程演示级；TaskSpec 无自由字段致 application↔task 关联在 slice 层；NL 路径未接；Task/Process teardown 与 rollback/自动 GC 未做 |
...
| `ROAD-B-005` Task Manager 多层手动控制与 NL/GUI/CLI 同路 | 局部推进：NL 四命令同义词 + ExportMetrics + **InspectProcess** NL/CLI（commit `e40de48`）+ **InspectTask socket parity + InspectResource NL/CLI**（commit `b75d8c5`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W17-005）已有单节点 H3；Trusted GUI 与多层手动调度未做 |
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach、process crash 合同层 + runtime terminal 门（`a69e709`）、Activation meter（`c3b2a10`）、阻塞 I/O 负向（`9b262f3`）、**批量 cancel 传播**（process `520cfce`）、**platform kill 合同层**（process `89b14a5`）、**`backpressure_wait`/`suspended` 生命周期**（runtime `2332ce7`，§6.8）、**Activation meter 规模探针骨架**（runtime `24001d0`，§6.9，`#[ignore]` 10K/100K 未实跑）+ **探针 race-free 修复**（runtime `8d98b78`）已有单节点 H3；100K 探针 `--include-ignored` 实跑、背压/挂起维 100K 规模验证、真实 OS platform kill、runtime 侧 fiber 批量 cancel 联动未做；仍为 `PARTIAL_PASS` |
