...
| `B-IDENTITY` | Principal、ControlDomain、版本化 identity snapshot 与 signing key authority | `IN_PROGRESS` | [B-IDENTITY-001](../evidence/stage-b/b-identity-001-principal-key-authority.md)：authority-assigned identity、Ed25519 key validity/revocation、Semantic signature verification PARTIAL PASS；[B-IDENTITY-002](../evidence/stage-b/b-identity-002-key-rotation.md)：signing-key rotation 最小前缀 + kill-window 故障矩阵（commit `9777031`）PARTIAL PASS；[B-IDENTITY-003](../evidence/stage-b/b-identity-003-custody.md)：trusted-local key custody 最小前缀（commit `2a607b5`）PARTIAL PASS；[B-IDENTITY-004](../evidence/stage-b/b-identity-004-session-ingress.md)：trusted-local session ingress 最小前缀（commit `281871d`）PARTIAL PASS | 多 Principal domain merge/split、HSM/跨机 session、attestation ingress 扩展、grace-period 双签、三平台 CI/真实掉电 |
...
| `ROAD-B-004` 10K/100K logical TaskNode、working-set、pressure/reclaim、rehydrate | 局部推进：ScaleProfile 骨架 + `TASK_PROFILE_10K` 实跑（commit `0bb6095`，[B-TASK-SCALE-001](../evidence/stage-b/b-task-scale-001.md)）+ `TASK_PROFILE_100K` 常量与 `#[ignore]` 100K 注册探针（commit `1affa6b`）+ **pressure/reclaim 声明式骨架**（commit `8c73def`，§6）+ **working-set admission gate 最小前缀**（commit `7e01ce7`，§7）+ **reclaim advisory 执行前缀**（commit `1b41afb`，§8，W19-004）PARTIAL PASS；TaskPlan/TaskNode 声明面（议题 35 未启动）、working-set 比例 benchmark、soft reclaim 执行/`max_task_nodes`/rehydrate 未做；waiting Fiber 不能替代 TaskNode benchmark |
| `ROAD-B-005` Task Manager 多层手动控制与 NL/GUI/CLI 同路 | 局部推进：NL 四命令同义词 + ExportMetrics + **InspectProcess** NL/CLI（commit `e40de48`）+ **InspectTask socket parity + InspectResource NL/CLI**（commit `b75d8c5`，[B-CONTROL-003](../evidence/stage-b/b-control-003-nl-prefix.md) §W17-005）+ **六命令 EN/ZH 同义词扩展**（commit `ee7363a`，§W18-005）+ **inspect 同义词 `查看 健康`**（commit `3c12370`，§W19-005）已有单节点 H3；pause/cancel 无 ControlCommand 变体；Trusted GUI 与多层手动调度未做 |
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach、process crash 合同层 + runtime terminal 门（`a69e709`）、Activation meter（`c3b2a10`）、阻塞 I/O 负向（`9b262f3`）、**批量 cancel 传播**（process `520cfce`）、**platform kill 合同层**（process `89b14a5`）+ **AlreadyTerminated + inspect 读回**（process `0ca7251`，W19-P）、**`backpressure_wait`/`suspended` 生命周期**（runtime `2332ce7`，§6.8）+ **lifecycle 10K 探针**（runtime `8425be7`，§6.10，W19-006）+ **Activation meter 10K/100K ignore 探针实跑**（runtime `a82cd5b`/`2236e26`，§6.9.3–§6.9.4）已有单节点 H3；lifecycle 100K 规模验证、真实 OS platform kill、runtime 侧 fiber 批量 cancel 联动未做；仍为 `PARTIAL_PASS` |
