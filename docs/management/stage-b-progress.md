...
- 当前状态为 `PARTIAL_PASS`：受控 kill-after-commit/restart 已有本地证据；真实 Unix/named-pipe
  多连接压力、真实断电/WAL 撕裂尾部、随机/信号级崩溃矩阵、真实 Capability authority、
...
- 本地 `cargo test -p nlos-takeover-control --features conformance-server` 10 项、workspace 448 项、schema lint/generation/typecheck、既有 TS/Python IPC/directory conformance、两语言 TakeoverControl conformance、feature clippy/fmt 与 diff 均通过；远端三平台 + MSRV 运行待本增量提交后回填。该证据是受控 process kill/restart 窗口，不外推为随机崩溃、真实断电或 WAL 撕裂尾部恢复。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl IPC 提交后进程 kill/restart 与同 key 恢复 PARTIAL PASS（B-TASK-008C2G-IPC-CRASH-RESTART，本地 TS/Python conformance 已过，远端待回填）
```
