...
>
> 最后更新：2026-08-16（已纳入 `B-TASK-008C2G-SEM` Semantic publication consumer、schema v25、nested receipt、本地混合 Effect + Semantic v3 终结 hook、slot/Receipt-bound success proof，以及 `B-TASK-008C2G-COORD` Semantic-only 跨 authority restart coordinator、schema v27 durable lease/term/fencing 原语、schema v28 opt-in CommitPermit binding、Semantic-only high-level lease finalize、schema v29 same-term adoption/reconcile lease guard、local `FROZEN_FOR_TAKEOVER` takeover-fence pre-gate、schema v30 immutable local fence receipt/exact local root 与 schema v31 lease-bound local `TaskAuthorityAssignment` baseline、`B-SCHEMA-005` exact OS peer credential pre-gate；此前的 TaskWriteSet/authority 增量保持有效）
>
...
- 仍为 `PARTIAL_PASS`：TaskAuthority consumer 已由 `B-TASK-008C2G-SEM` 接线，单 authority lease 已覆盖 mixed、Semantic-only high-level 与 same-term adoption/reconcile terminal path，并增加新 term 的 local `FROZEN_FOR_TAKEOVER` fence pre-gate、schema v30 immutable local fence receipt/exact local fence-set root 与 schema v31 lease-bound local assignment baseline；跨 authority prepare/consume/recovery、NLOS principal/attestation、TakeoverReceipt、远端 barrier receipts、successor assignment 激活、跨 term adoption、Trust View/vector checkpoint 与多 Cell 仍未完成。
