...
> 本轮（2026-08-19 第九增量）推进 `B-TASK-008C2G-IPC-CONCURRENCY`：两个独立 IPC caller 在多线程 Tokio harness 中同时提交同一 signed observation（request/caller identity 不同），handler 与单写 SQLite authority 将其线性化为一条 durable row；两边都返回相同 verified record，coverage 保持 `LocallyCovered`。本地 7 项 TakeoverControl Rust 测试通过，三平台远端结果待回填。详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.92。
...
>
> 当前进度同步（2026-08-19）：第九增量已在本地完成 TakeoverControl 两 caller 并发线性化与单 row 证明；第八增量的 TypeScript/Python payload/response conformance、双连接 replay 与 Windows named-pipe 已由 run 32192662820 三平台通过。第九增量远端 CI 正在验证，未将本地并发结果冒充三平台完成。
>
...
| `B-TASK-008C2G-IPC-CONCURRENCY` | TakeoverControl 多 caller 并发线性化 | `IN_PROGRESS` | [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md) §4.92：两个独立 duplex caller 并行提交同一 signed observation，单 durable row、verified record 等值、coverage `LocallyCovered`；本地 7 项通过 | 三平台 CI 回填；真实 Unix/named-pipe 多连接并发、IPC crash/fault matrix、真实 Capability authorizer、principal-level peer attestation 仍未实现 |
...
- 本地 `cargo test -p nlos-takeover-control --quiet`（7 项）、workspace 445 项、feature
  clippy/fmt 已通过；当前状态 `IN_PROGRESS`，第九增量三平台 CI 待回填。该证据只覆盖
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TakeoverControl 多 caller 并发线性化 IN_PROGRESS（B-TASK-008C2G-IPC-CONCURRENCY，本地 7 项通过，待三平台 CI）
```
