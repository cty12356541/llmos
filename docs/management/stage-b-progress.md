...
| `B-PARTICIPANT` | TaskAuthority participant registry generation/root/freeze 与 endpoint proof | `IN_PROGRESS` | [B-TASK-007A](../evidence/stage-b/b-task-007a-self-participant-registry.md)：authority-assigned TaskStore self participant、versioned root、permit-time atomic freeze；[B-TASK-007B1](../evidence/stage-b/b-task-007b1-authority-endpoint-proofs.md)：Artifact/Semantic durable endpoint proofs；[B-TASK-007B2](../evidence/stage-b/b-task-007b2-verified-participant-registration.md)：direct authority readback + OPEN registry generation/root CAS PARTIAL PASS | Driver/Resource/Channel endpoints、operation prepare→activate、EffectPermit/Receipt binding、takeover fence、fault matrix |
...
| `B-RESOURCE` | ResourceAccount、Driver/Device、Reservation 与 activation reference monitor | `IN_PROGRESS` | [B-RESOURCE-001](../evidence/stage-b/b-resource-001-driver-reservation-binding-authority.md)：Driver generation/fence + AVAILABLE→RESERVED→ACTIVE binding PARTIAL PASS | 多维 demand、完整 Ledger/consume/finalize、AdmissionPlan、ControllerBinding、真实 enforcement shim 与平台 Device adapter |
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → Driver gateway + Resource/Ledger endpoint proofs                                       NEXT（B-TASK-007C1）
