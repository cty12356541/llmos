...
- TS/Python 真实 IPC 已验证 `DISPATCHED(0) → CANCEL_REQUESTED(1)`、cancel replay 与 query；独立 Tokio timer task 又验证 `REGISTERED(0) → CANCELLED_BEFORE_EFFECT(1)`，worker 精确一次成功、零失败。当前为本地 `PARTIAL PASS`，三平台回执待提交后补齐。详见 [B-SCHEMA-013](../evidence/stage-b/b-schema-013-operation-control-timer-worker.md)。
