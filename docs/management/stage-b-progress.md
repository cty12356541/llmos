...
> 本轮（2026-08-19 第七增量）推进 `B-TASK-008C2G-CROSS-TERM-ADOPTION`：schema v38 新增独立 immutable `task_cross_term_adoption_receipts`，`adopt_permit_across_takeover` 将旧 `QUARANTINED` permit 绑定到 completed takeover、exact fence root、reopened successor registry/assignment 和 live successor lease；核心 effect reconcile、close、v3 finalize 改用 successor registry binding，禁止新 EffectPermit/dispatch，且同 key/restart 逐位回放。新增 `cross_term_adoption.rs` 真实 takeover→registry reopen→adoption→reconcile→finalize 链；`nlos-task` 180 项、workspace 444 项通过（另 2 项既有 scale probe ignored），clippy/fmt 已通过。详见 [B-TASK-008C2G-CROSS-TERM-ADOPTION](../evidence/stage-b/b-task-008c2g-cross-term-adoption.md)。限制：远端 cleanup/endpoint attestation、Artifact/Semantic high-level 跨 term 路径、TS/Python conformance 与 Windows named pipe round-trip 仍未完成。
...
>
> 当前进度同步（2026-08-19）：首条第七增量已覆盖原六段历史摘要；schema v38 cross-term adoption 的本地证明链已实现并完成 nlos-task 回归，当前下一门是 TS/Python conformance 与 Windows named pipe round-trip。
>
...
- 详见 [B-TASK-008C2G-IPC](../evidence/stage-b/b-task-008c2g-takeover-control-ipc.md)；限制：Rust-only conformance（TS/Python 延后）、无时间窗防重放（trusted clock 独立 authority）、ExactPeerAuthorizer 为 OS 凭证非 principal attestation、CorruptRecord 串映射与 store 静态消息结构耦合、无 Windows named pipe handler round-trip/并发竞争/IPC 层崩溃注入。
...
- 详见 [B-TASK-008C2G-SUCCESSOR-REGISTRY](../evidence/stage-b/b-task-008c2g-successor-registry.md)；限制：只复制本地 participant tuple，未重新取得跨 authority endpoint proof/attestation；旧 permit 的核心 cross-term adoption 已由 [B-TASK-008C2G-CROSS-TERM-ADOPTION](../evidence/stage-b/b-task-008c2g-cross-term-adoption.md) 接通，Artifact/Semantic high-level 跨 term、TS/Python conformance、Windows named pipe round-trip 与 rotation 专属 VFS 故障矩阵仍未完成。
...
- 仍为 `PARTIAL_PASS`：本地核心 Effect 路径已接通，但 Artifact/Semantic high-level
  跨 term、远端 cleanup/endpoint attestation、TS/Python conformance、Windows named
  pipe round-trip、并发/崩溃及 rotation 专属故障矩阵仍未完成。
...
TaskGroup membership generation/root CAS + Admission/Removal Receipt                  PARTIAL PASS 候选（B-TASK-004）
  ...
  → TS/Python conformance + Windows named pipe round-trip NEXT（B-TASK-008C2G-IPC）
```
