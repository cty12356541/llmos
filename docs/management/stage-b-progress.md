...
| `B-SLICE-K` | Slice K：Package → Application → Task → Fiber → Operation → Receipt → 控制 | `PARTIAL_PASS` | [B-SLICE-K-001](../evidence/stage-b/b-slice-k-001-end-to-end.md)：`crates/nlos-slice-k`（commit `43b7d37`）以已落地 API 组装全链贯通——签包→验签→安装→Task/Attempt→CommitPermit→Fiber 执行 Driver Operation→stage/plan→converge→TaskCommitReceipt→cancel（CancelledBeforeEffect）→crash recovery（drop→reopen→converge_pending 幂等收敛）→inspect；demo bin 全链输出；3 集成测试 | 单机单进程演示级；TaskSpec 无自由字段致 application↔task 关联在 slice 层；NL 路径未接；多 Attempt 竞争/双提交竞态场景未含（ROAD-B-003 级证据待补）；Process authority 物化未接 |
...
| Exit gate | 当前结论 |
|---|---|
| `ROAD-B-001` 第三方 Application 安装/更新/卸载 | 局部推进：install/disable/update/uninstall 已有单节点 H3（[B-APPLICATION-001](../evidence/stage-b/b-application-001-installation-authority.md)、[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md)、[B-APPLICATION-003](../evidence/stage-b/b-application-003-uninstall.md)）；rollback/GC/运行中 Task 门与完整 `[PKG-UPDATE-001]` 生命周期策略未做 |
...
| `ROAD-B-006` 100K dormant Fiber、阻塞隔离、crash propagation、Activation meter | 局部推进：100K fiber、cancel/late-callback、structured join/detach、process crash propagation 合同层（[B-RUNTIME-002](../evidence/stage-b/b-runtime-002-fiber-scale.md)、[B-PROCESS-003](../evidence/stage-b/b-process-003-crash-propagation.md)）已有单节点 H3；Activation meter、阻塞 I/O 负向证明、runtime 联动未做；仍为 `PARTIAL_PASS` |
