...
- 三语言 Protobuf generation/compat、首轮 sanitizer fuzz smoke、Unix/Windows typed IPC client、ServiceDirectory 两跳调用与 common metadata/safe-retry 校验已通过局部验证；durable dedup/result、真实 IPC 重连、server restart 与 deadline/cancel 状态机已有三平台证据，OperationControl/timer worker 已取得本地局部证据。但正式 SDK facade、Receipt authority、CBOR 跨语言和长期 fuzz 未完成前，ADR 保持 `POC`；
