...
[B-SCHEMA-006](../../evidence/stage-b/b-schema-006-typescript-python-ipc-clients.md) 已加入 TypeScript/Node 与 Python asyncio candidate client，并在 macOS 通过两种语言调用 Rust conformance server 的真实 Unix socket 往返、compatibility preflight、backpressure 和 unavailable endpoint 测试。Windows named-pipe 分支、ServiceDirectory/common semantics、双向 peer auth 和 SDK 发布仍未完成。
