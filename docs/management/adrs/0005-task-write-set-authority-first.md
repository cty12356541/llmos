...
用户在 2026-08-16 选择 [ADR-0006](./0006-semantic-publication-receipt-owner.md) 候选 1：`B-SEMANTIC-005` 已由 SemanticAuthority 生成 owner-derived `SemanticPublicationReceipt` 与 local log-prefix checkpoint（schema v4）；`B-TASK-008C2G-SEM` 随后完成 TaskAuthority consumer、schema v25、READY/finalize 与 nested `SemanticTaskCommitReceipt.semantic_publications`，并将 nested receipt 接入含 Effect slot 的 v3 同事务混合终结 hook；`B-TASK-008C2G-COORD` 再补上 Semantic-only 跨 authority restart coordinator、schema v26 typed mixed-finalize envelope、本地 slot/Receipt-bound success proof binding、schema v27 单 authority durable lease/term/fencing primitive，以及 schema v28 opt-in CommitPermit/plain v3 terminal binding、mixed Effect + Semantic persisted-envelope finalize/replay、Semantic-only high-level finalize、schema v29 same-term adoption/reconcile lease guard、新 term `FROZEN_FOR_TAKEOVER` local fence pre-gate、schema v30 immutable local fence receipt 与完整映射时的 exact local fence-set root 计算、schema v31 lease-bound permit 的 local `TaskAuthorityAssignment` baseline、schema v32 旧 assignment `TakeoverPending` 与 pending `TaskAuthorityTakeoverReceipt` local prefix、schema v33 逐 endpoint barrier receipt observation、schema v34 canonical exact-fence member manifest 与只读 local coverage view。outbox ACK 继续不被提升为 publication proof；跨进程 principal/attestation、远端 barrier 验证与完成、successor assignment 激活、跨 term adoption、完整故障矩阵、外部 provider proof/attestation、Trust View/vector checkpoint 仍待后续门。详见 [Evidence](../../evidence/stage-b/b-semantic-005-publication-receipt-producer.md)、[B-TASK-008C2G-SEM](../../evidence/stage-b/b-task-008c2g-semantic-publication-consumer.md) 与 [B-TASK-008C2G-COORD](../../evidence/stage-b/b-task-008c2g-semantic-coordinator.md)。
...
进度补记（2026-08-24）：`B-RESOURCE-006`（owner 侧 `inspect_cost_receipt` 聚合回读，committed `3661112`）之后，用户于 2026-08-24 为 Task 侧 Resource cost-receipt 桥接固定选择 **authority-first 全量聚合**方案（Attempt `TASK-RESOURCE-COMMIT-01`，schema v39）：调用方不提供任何成本事实（无 activation/finalization ID、无 consumption sequence、无 usage/refund 数值），`finalize_commit_v3_with_resource_authority[_and_authority_lease]` 从 permit 绑定的 sealed `TaskWriteSet.resource_reservations`（按 `ReservationId` 排序）推导精确 Reservation 集，在打开 Task 终结事务**之前**逐项调用 committed `ResourceAuthority::inspect_cost_receipt`（要求 FINALIZED），比对 reservation/account/quote/call/operation/upper_bound 七项绑定后将完整 owner 聚合（activation + 全部有序 consumption + finalization/refund）以**两张 STRICT immutable 子表**（`task_resource_cost_receipts` 父表 + `task_resource_cost_consumptions` 子表）原样复制进同一 Task 终结事务（在 `insert_receipt` 之后、permit close/head advance 之前）；重放只从 Task 行读取（replay 权威 = Task rows），不再回读 Resource owner。请求/幂等身份就是既有 `FinalizeRequestV3` + finalize proof digest；不再存在 caller-selected receipts。明确边界：verify-then-commit **不是**跨 authority 原子性（owner 读在 Task 事务之外，两个 authority 仍是两个事务域）；混合 Semantic + Resource 权威终结rung、kill-window 故障矩阵/真实断电验证仍是退出条件/已知缺口；若复审发现 Resource binding 缺乏独立 authority 事实，按本 ADR 退出策略降级为 immutable reference/digest。该 attempt 为候选（未提交、未更新进度表），Evidence 见 [b-task-008c2g-resource-cost-commit](../../evidence/stage-b/b-task-008c2g-resource-cost-commit.md)。
