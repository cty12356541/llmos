...
[B-SCHEMA-012](../../evidence/stage-b/b-schema-012-deadline-cancel-state-machine.md) 新增 dispatch 前 no-effect 的 idempotent 原子完成路径，并把 deadline/cancel 的 pre-dispatch、post-dispatch partial/effect-unknown 转换接入真实 Rust↔TS/Python IPC；本地通过，远程三平台待验证。独立 Operation query/cancel payload、timer-driven worker 与 retention/GC 仍待完成。
