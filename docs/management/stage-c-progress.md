...
| 阶段级状态 | 含义 |
|---|---|
| `PLAN-DRAFT` | 编排计划草案，待维护者批准（决策点 0）；车道不可派发 |
| `IN_PROGRESS / NOT EXITED` | 决策点 0 通过后翻转；至 §C.5.5 全条件满足并获明确批准前不得翻 `EXITED` |
...
### C.5.4 决策点状态（2026-09-21，起草基线）

早期波（W35–W37）派发前需要维护者裁定的议题；规范留白处只给候选不发明：

0. **本计划批准（唯一保留用户门，先于一切）**：本文件草案 → 维护者审阅批准 → §C.2 翻转 `IN_PROGRESS / NOT EXITED`、本文件转 canonical（含 L0/README 入口同步——渐进式披露 §5 Create 纪律，归控制器集成提交）。
1. **多 Cell 拓扑 PoC 顺序与最小拓扑**（规范未规定，§26.1 只定职责分工）：候选——(a) 单机多进程 Cell×2 起步（failure detector/分区可注入、迭代快，跨机网络语义后置）；(b) 跨机双节点起步（网络分区/时钟语义一步到位，迭代慢）；(c) 控制面先行（quota/epoch/fencing 先于 Cell 本地件套）。附带子议题：`TaskAuthorityAssignment.consensus_commit_index`（§26.1 对象模型）隐含共识/日志基底，但 v0.5 未定共识算法——是否需要 ADR 定基底（或先以控制面单写者 + lease/fencing 语义覆盖）**必须随本决策点裁定**。裁定晋升 ADR 后 W37 方可细化。
2. **单 Cell 加固 vs 跨 Cell 主线优先序**（资源约束：单人维护，RISK-B-08）：候选——(a) 加固先行（W36 全量后再开 C-core，生产声明前置早闭环）；(b) C-core 先行（B 残余加固穿插 C-core 波屏障间）；(c) 按写集并行双线（本计划默认排布，须确认可承载）。
3. **移交项归置**：(i) 移交#15 拆分——「完整桌面」按 §28.4 属 D 册追踪（本计划建议：D 册，C 册只挂引用）；「Notification/Search 超最小面扩展」规范未指派阶段（§28.2 只要求最小版，已交付；超出面无阶段归属）——候选：C 册低顺位 / D 册 / 显式不排程（挂 backlog）；(ii) 移交#3 GUI 真机战役归 C 册执行还是 D 册首个战役；(iii) 移交#16 掉电层 3+ 时序（C 期内还是持续项）；(iv) 移交#2/#11 等已默认 C 册项的顺位确认。W34-D 已裁量的移交默认入 C 册，本决策点只处理歧义项。
4. **跨 Cell 提交语义 ADR 启动时机**：ADR-0013 复审触发器显式点名「Stage C 跨机提交语义定案」；候选——(a) W39 前置启动（C-SHARD 依赖其结论，派发纪律 (4) 信息依赖）；(b) W37 随拓扑 ADR 并行开档。2PC/reconciliation 混合评估按 ADR-0013 决定 3 原文。
5. **「federation」交付面解释确认**：§28.3 要求 federation 交付，§29.2 又把「全球联邦与跨组织清算」列为延后但保留接口。本计划采读法：C 交付 federation **机制面**（跨 Cell 名称/服务发现、迁移 intent、审计 checkpoint——§26.1 控制面职责原文），全球规模跨组织清算延后。请维护者确认或改采他读法（影响 C-MIGRATE 验收边界）。
