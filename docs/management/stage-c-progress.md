...
**W36（单 Cell 加固波，依决策点 2 排程）**：四车道任务门+终审已齐并**已合入 origin/main**（#5 `09c0832` / #7 `8b91419` / #10 `c45bd10` / #8 `8ed592a`；#6 已于 W35-P6 `4df55d6` 提前）。**W36 屏障未关**：三平台 job（ubuntu、macos、windows）与 MSRV 已在 `47986000adebb8bf58e03e6ebffa44ee041990f0` 成功（run [36239183617](https://github.com/cty12356541/llmos/actions/runs/36239183617)）；Pages run [36239183619](https://github.com/cty12356541/llmos/actions/runs/36239183619) 同 SHA 成功。Scale probe release 与 Scale probe (include-ignored) 为 skipped，夜间 schedule 仍开——不得声称屏障闭合。否决窗口仍开。本句不冻结 tip SHA。
...
**W38（lease/quota + 移交后片）**：本地 main 与 origin/main 对齐（ahead/behind 0；2026-09-25）；范围 `ee649d3..194b014` 已由 bound SSH ls-remote 核实在 origin；台账提交 `3c5cc9a` 亦在 origin；权威 tip 以 `git ls-remote origin refs/heads/main` 为准，本文件不冻结 tip SHA。已合入 origin 的前片：`C-LEASE` 单 Cell Quota+Capacity+ExclusiveDevice（`ac898ec`/`9be119c`/`ac2116c`）；boot generation（`12d89e1`/`bb9b7cb`）；#12 干净退出 + restore 过期代次测试（`cb51acb`/`d391ba7`）；#2 run/update/uninstall CLI（`4c67ede`/`000436d`/`e406fdb`）+ clippy 行数拆分（`e782c35`/`f3e83f6`）；#11 inspect + supervisor unregister（`ce06746`/`7e975d7`，**发现仍开**）。**未做 / 仍开**：C-SHARD、C-CELL **七件套全量**、Stage D 余量、#12 全生命周期余量、Device reset、reconcile 路由、夜间 schedule、#17 custody/gateway；各前片无独立 Evidence 文件——Claim=`PARTIAL_PASS`。三平台 job（ubuntu、macos、windows）与 MSRV 已在 `47986000adebb8bf58e03e6ebffa44ee041990f0` 成功（run [36239183617](https://github.com/cty12356541/llmos/actions/runs/36239183617)）；scale-probe job 为 skipped，故夜间 schedule 仍开。本地 fmt+clippy+workspace test 于 `e782c35` 绿（1628 passed）**≠** 该 run。不声称计划或阶段 C 已退出。

**W39（TaskAuthority 跨 Cell 波 + D 邻接前片）**：`C-SHARD` / `C-FANOUT` **未实现**（ADR-0019 仍非 VERIFIED）。D 前片（决策点 3 归 D 册）：五层只读 inspect `ee649d3`/`86fd713`；窗口开合终态 `d059ac2`/`49d3e14`；Surface `create`/`hide` `6165bc4`；表面呈现投影当前代际进程绑定 `80fd1e3`；`InspectApplication` `3560800`；当前代际后台任务投影 `74305de`——**`PARTIAL_PASS` / D 前片**，**不声称 Stage D 余量 / C-SHARD DONE**。
...
2026-09-22 落档（现已在 origin/main）：[ADR-0018](./adrs/0018-single-host-multiprocess-dual-cell-topology.md) `ACCEPTED` `c433d6f`；[ADR-0019](./adrs/0019-cross-cell-commit-semantics-extension.md) `CANDIDATE` `b512fa7`；[ADR-0020](./adrs/0020-control-plane-single-writer-and-consensus-timing.md) `CANDIDATE` `705d4d1`。**否决窗口仍至 W36 屏障**（三平台 job 与 MSRV 已在 `47986000adebb8bf58e03e6ebffa44ee041990f0` 成功，run [36239183617](https://github.com/cty12356541/llmos/actions/runs/36239183617)；scale-probe skipped，夜间 schedule 仍开，屏障未关）。ACCEPTED/CANDIDATE ≠ VERIFIED。
...
2026-09-26 续：四车道本地提交，均为 `PARTIAL_PASS` / 前片。不声称 C-LEASE、C-LIFECYCLE、Stage D、C-SHARD、七件套或 CI 绿。推送结果不在本段预写。

| 车道 | 提交 | 写集要点 |
|---|---|---|
| Capacity `TARGET_PREPARED` | `cd2205e5839cb0d28c1a0a8c045125f3fb376e0d` | `GLOBAL_RESERVED→TARGET_PREPARED`；归还仍等 `RETURNED` 才退池；无 `ACTIVE` |
| 拆卸采纳已提交 kill | `3d8da8073d4a438b01a409ed13bf48c35fdff15a` | crash 标记未写时沿用已提交 platform-kill 回执；反向 NL 时间戳同源未做 |
| 表面进程绑定投影 | `80fd1e3d5bad28cdee60b4d21b3f0ae64fb8c1cf` | `present_surfaces` 投影当前代际 `inspect_process_bindings`；无新 IPC |
| Quota 失联隔离 | `7573260c1bc76b32c8e7ea1e38850acb40840ff9` | epoch 推进把未对清 `QuotaLease` 转入 `QUARANTINED`，不退回 AVAILABLE |

定向验证（本机，隔离 `CARGO_TARGET_DIR`）：`nlos-lease` 7 passed / 0 failed，clippy `-D warnings` exit 0；`application_teardown` 3 passed / 0 failed，`nlos-slice-k` clippy exit 0；`surface_presentation_side` 2 passed / 0 failed，`llmos-desktop` clippy `--features dev-fixture` exit 0。全仓（隔离 target，docs 提交前的代码树）：`cargo fmt --check` exit 0；`cargo clippy --workspace --all-targets -- -D warnings` exit 0；`cargo test --workspace --no-fail-fast` **1674 passed / 0 failed / 27 ignored**（313 个 test result 行合计）。**≠** 三平台 CI / 夜间 schedule。

2026-09-26 续：两车道本地提交，均为 `PARTIAL_PASS` / D 前片。不声称 Stage D 余量、C-LEASE、C-LIFECYCLE、C-SHARD、七件套或 CI 绿。推送结果不在本段预写。

| 车道 | 提交 | 写集要点 |
|---|---|---|
| InspectApplication 接线 | `3560800e429425271e7c7e276571cfd494d69b6c` | 既有 `ControlCommand::InspectApplication` + `ApplicationAuthorityInspector`；未配置 `application_root` 仍是未接线 `NOT_FOUND` |
| 后台任务呈现投影 | `74305defb5be9bff5dfb624b0dda7d20646881a1` | `present_surfaces` 投影当前代际 `inspect_background_tasks`；stale 代际不呈现 |

定向验证（本机，隔离 `CARGO_TARGET_DIR`，desktop 独立 workspace）：`cargo fmt --all -- --check` exit 0；`application_inspect_side` 2 passed / 0 failed；`surface_presentation_side` 2 passed / 0 failed；`cargo clippy --features dev-fixture --all-targets -- -D warnings` exit 0。**≠** 三平台 CI / 夜间 schedule。#12 反向时间戳同源与完整复活链、Device reset、C-SHARD、七件套、supervisor pid 发现、TTL/`FENCED`、Capacity `ACTIVE`、slice-k reconcile 路由仍开。

2026-09-26：Rust cross-platform verification run [36239183617](https://github.com/cty12356541/llmos/actions/runs/36239183617) 在 `47986000adebb8bf58e03e6ebffa44ee041990f0` 上 conclusion=success。三平台 job（ubuntu、macos、windows）与 MSRV 1.97 成功；Scale probe release 与 Scale probe (include-ignored) 为 skipped，故夜间 schedule 仍开。Pages run [36239183619](https://github.com/cty12356541/llmos/actions/runs/36239183619) 同 SHA 成功。权威 tip 以 `git ls-remote origin refs/heads/main` 为准，本段不冻结 tip SHA。**仍开**：C-SHARD、Cell 七件套、Stage D 余量、Device reset、reconcile 路由、#12 余量。`PARTIAL_PASS` 维持。不声称计划或阶段 C 已退出，不声称 W36 屏障闭合。本段只登记 run 36239183617；其后因本台账提交新起的 CI run 不在此预写、不声称已绿。
