...
### 6.1 子 Agent 编排与依赖感知并行

多步实现、验证或跨模块工作不得由单个 Agent 顺序包办，也不得无边界并发。编排流程：

```text
显式依赖图 → 波次划分 → 并行执行 → 波屏障验收 → 集成与回执
```

`[PD-ORCH-001]` 多步实现/验证 MUST 委派给子 Agent 执行：每个子 Agent 拥有独立 Task/Attempt 身份、声明的 read-set/write-set 和验收条件；父 Agent 只负责编排、验收与集成，不得以自身顺序执行替代委派。

`[PD-ORCH-002]` 启动波次前 MUST 声明显式依赖图：每条车道标明 blocked-by/blocks 关系与共享对象；依赖未知时按有依赖处理，禁止执行中隐式发现依赖。

`[PD-ORCH-003]` 相互无依赖且 write-set 不相交的车道 MUST 划入同一波次并行执行，不得人为串行拖慢；共享 canonical 文件或存在数据依赖的写入 MUST 串行，或收敛到单一 integrator 合并，禁止多车道并发写同一权威对象。

`[PD-ORCH-004]` 波次之间设有屏障：下一波必须等本波全部车道通过验收后才能启动；单车道失败只阻塞其依赖车道，无关车道继续执行，失败车道产物按候选处理。

`[PD-ORCH-005]` 子 Agent 只继承衰减后的 write-set 与权限，MUST NOT 自动扩大范围；失败、取消或未通过验收的车道输出保留为 Evidence/候选，不得自动进入 canonical 文档或提交。

`[PD-ORCH-006]` 每条车道完成时 MUST 产出准确回执：记录实际运行的验证命令与结果、commit/push/CI 状态逐级确认，未运行项显式标注；Claim 不得超过 Evidence，禁止用单车道结果冒充集成验证或发布状态。
...
- [ ] 多步工作按 §6.1 委派子 Agent，依赖图、并行波次、单一 integrator 和波屏障边界明确，车道回执准确；
