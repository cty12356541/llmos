...
- 阶段 C 权威进度单与编排：[stage-c-progress.md](./stage-c-progress.md)（2026-09-22 决策点 0 批准生效；W35 起波次编排 §C.5.3；决策点裁定与否决窗口 §C.5.4；W36–W39 合入与推送状态见 §C.8；本地 main 与 origin/main 对齐（ahead/behind 0）；范围 `ee649d3..194b014` 已由 bound SSH ls-remote 核实在 origin；台账提交 `3c5cc9a` 亦在 origin；权威 tip 以 `git ls-remote origin refs/heads/main` 为准，本文件不冻结 tip SHA；`PARTIAL_PASS` 前片；C-SHARD / Cell 七件套 / Stage D 余量 / 三平台 CI / 夜间 schedule 仍开，不声称 CI 绿或屏障闭合）
...
- 跨进程 Principal 认证签名贯穿：[ADR-0011](./adrs/0011-ipc-principal-auth-signature-passthrough.md)
