...
| 架构 | 方案 A:演进现有 CLI 管道(统一模型+适配器+多目标渲染,纯 stdlib) | B:MCP 中心(面板在会话外,连 MCP 反而复杂);C:全 TUI 重写(破坏零依赖与模型-渲染分离) |
...
- **渲染 golden 对拍**:固定模型 fixture → 各渲染目标输出逐字节比对;
...
MCP server 形态;全 TUI 交互(滚动/过滤/展开);行动指向(可派发集)与信任层(台账↔git 交叉校验、CI 状态)——内容主次决策中列为二期;多机/团队共享状态;HTML 交互与外链资源;issue/PR 数据源适配器。
