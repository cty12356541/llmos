...
> 状态：`DESIGN+IMPLEMENTED`（W33-A / B1-4；证据见 [B-ARTIFACT-007](../evidence/stage-b/b-artifact-007-package-sdk.md)；`install` 子命令 W35-P2 / 移交#2 前片，见 [B-APPLICATION-007 §7](../evidence/stage-b/b-application-007-third-party-sample.md)；`run`/`update` 子命令 W38-P2 / 移交#2 后片）
>
...
nlos-package update <PKGFILE> --root <DIR> [--compat …]     # W38-P2：update_application 消费端（见 §6.3）
```
...
### 6.3. update：从 CLI 走通更新权威（W38-P2）

```text
$ nlos-package update sample-v2.nlospkg --root state.d
VERIFIED …
manifest_digest …
package … version …
signer … key …
UPDATE …
decision updated
application … package … generation 2 version … entries … installer …
executables hello
compat same-major
```

流程：与 `install` 同一套 verify → materialize → install-scoped GC，随后调用既有 `ApplicationAuthority::update_application`（[B-APPLICATION-002](../evidence/stage-b/b-application-002-update.md)）：receipt digest-binding、兼容窗口（默认 `same-major`，可选 `--compat same-minor`）、单事务 CAS 推进代际。不发明新包格式。

幂等纪律：同 root / 同目标 verification receipt 再跑 ⇒ `decision replayed`，代际不双跳。未安装 / 同内容（`UpdateManifestUnchanged`）/ 兼容窗违规 / 终态冲突 ⇒ exit 7。

### 退出码

| 码 | 含义 |
|---|---|
| 0 | 成功（verify 输出 VERIFIED/REPLAYED；conformance 输出 CONFORMANT；install 输出 INSTALL + decision；run 输出 RUN + decision；update 输出 UPDATE + decision） |
| 1 | 用法错误 |
| 2 | 输入畸形：manifest/密钥文件/包文件解析或形状（重复 entry 名、悬空依赖、截断包…）；conformance 亦用于文件不可读；run 的 package-id 非 32 hex；update 的 `--compat` 非法 |
...
| 7 | 权威拒绝（`install`：receipt 未知/终态冲突/幂等冲突/时序倒置；`run`：未安装/非 installed/缺物化/driver 面拒绝；`update`：未安装/同内容/兼容窗违规/终态冲突） |
...
- `install`/`run`/`update` 已收口：卸载的消费端 CLI（`uninstall` 子命令）与样例自身接线仍是后续车道；已安装应用 executable 载荷的执行经 `nlos-slice-k::execute_application_payload` 内核车道（W35-P2 前片），`nlos-package run` 是其 CLI 消费端，`nlos-package update` 消费既有 `update_application`（W38-P2）。
